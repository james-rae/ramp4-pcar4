import { F as h, g as n, G as p, E as f } from "./main-B5C1KQ4n.js";
...
class W extends A {
  ...
  added() {
    ...
    }, this.schemaEH = this.$iApi.event.on(p.MAP_BASEMAPCHANGE, (i) => {
      i.schemaChanged && this.checkActive();
    }), this.$iApi.geo.map.created ? this.checkActive() : this.$iApi.event.once(p.MAP_CREATED, () => {
      this.checkActive();
    });
  }
  /**
  ...
  checkActive() {
    ...
    t.alwaysOn || t.extentSetIds.includes(this.$iApi.geo.map.getExtentSet().id) ? t.active || (t.setActive(!0), this.extentEH = this.$iApi.event.on(p.MAP_EXTENTCHANGE, (i) => {
