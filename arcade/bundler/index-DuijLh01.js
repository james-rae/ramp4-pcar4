import "tiny-emitter";
import { k as m, C as u, G as p, l as f, L as g, T as L, m as w, F as A } from "./main-D5OlKCXm.js";
...
class $ extends d {
  ...
  async reorderFogLayer() {
    ...
    if (!e || !i)
      return;
    const t = this.$iApi.geo.layer.layerOrderIds(), a = t.indexOf(i.id), r = t.indexOf(e.id);
    r < a && r > -1 && a > -1 && this.$iApi.geo.map.reorder(e, a, !1);
  }
  async add(i) {
  ...
  async remove(i) {
    ...
    if (!e)
      return;
    const t = Date.now(), a = await this.getHilightLayer();
    a && setTimeout(() => {
      this.lastAdd < t && !a.getGraphicCount() && (e.opacity = this.offOpacity);
    }, 300);
  }
  async reloadHilight(i) {
...
class y extends d {
  ...
  async add(i) {
    ...
    if (e && e.esriLayer && e.isLoaded && e instanceof w) {
      ...
      this.$iApi.geo.map.esriView?.whenLayerView(e.esriLayer)?.then(function(a) {
        a.highlight(t.map((r) => e.getEsriGraphic(r.id)));
      });
    }
  }
  async remove(i) {
...
class v extends A {
  ...
  async getGraphicsByKey(i, e, t) {
    ...
    if (!a)
      return [];
    let r = a.graphics.map((s) => ({
      ...this.deconstructGraphicKey(s.id),
      og: s
    }));
    return i && (r = r.filter((s) => s.origin === i)), e && (r = r.filter((s) => s.uid === e)), t && (r = r.filter((s) => s.oid === t)), r.map((s) => s.og);
  }
  /**
