import { defineComponent as F, ref as w, onMounted as ae, openBlock as t, createElementBlock as a, createElementVNode as g, normalizeClass as U, Fragment as Z, renderList as K, normalizeStyle as ve, createCommentVNode as j, toDisplayString as S, computed as M, resolveDirective as q, withDirectives as A, withModifiers as pe, createVNode as se, onBeforeMount as Q, watch as P, onBeforeUnmount as W, unref as L, createBlock as R, inject as X, resolveDynamicComponent as we, createTextVNode as le, resolveComponent as $e, withCtx as ce } from "vue";
import { _ as ie, e as ee, N as ne, L as Te, O as Me, G as J } from "./main-D5OlKCXm.js";
...
import { T as Ee } from "./toggle-switch-control-B0lYrvzN.js";
import "pinia";
import "screenfull";
...
const Ie = {
  ...
  setup(v) {
    ...
    }), (c, p) => v.result.loaded ? (t(), a("div", Ie, [
      g("div", {
  ...
  setup(v) {
    const b = ee(), i = M(() => b.properties), c = v, p = () => {
      const $ = c.layer;
  ...
  setup(v, { emit: b }) {
    ...
    return Q(() => {
      ...
      p.value?.addEventListener("blur", $), p.value?.addEventListener("keyup", f);
    }), W(() => {
      d.value.forEach((h) => h()), p.value?.removeEventListener("blur", $), p.value?.removeEventListener("keyup", f);
    }), (h, B) => {
  ...
  setup(v) {
    const { t: b } = V(), i = X("iApi"), c = v, p = (r, d, n, e) => {
      const o = r.find((E) => E[d].toLowerCase() === n.toLowerCase());
      o && delete e[o.name];
    }, $ = () => {
      const r = Object.assign({}, c.identifyData.data);
      p(c.fields, "type", "geometry", r), i?.ui.exposeOids || p(c.fields, "type", "oid", r), i?.ui.exposeMeasurements || (p(c.fields, "name", "shape_length", r), p(c.fields, "name", "shape_area", r));
      const d = {};
      c.fields.forEach((e) => {
      ...
      return e.innerHTML = r.trim(), e.firstElementChild?.tagName == "A" ? (e.firstElementChild.className = n, e.innerHTML) : me(r, {
        className: n,
        target: "_blank",
        validate: {
          url: (E) => /^https?:\/\//.test(E)
  ...
  setup(v) {
    const b = ne(), i = v, c = X("iApi"), p = w([]), $ = ee(), { t: f } = V(), _ = w(""), u = w("none"), r = w(), d = () => b.getLayerByUid(i.uid), n = M(() => $.properties), e = M(() => $.defaultTemplates), o = M(() => d()?.supportsFeatures ?? !1), E = M(() => d()?.mapLayer ?? !1), O = M(() => {
      const s = d();
      let y = s && i.data.loaded ? s.nameValue(i.data.data) : c.$i18n.t("details.items.title");
      return c.ui.isPlainText(y) && (y = c.ui.escapeHtml(y)), y;
    }), D = (s) => {
      if (typeof s == "string") {
        const y = "underline text-blue-700 break-all", k = document.createElement("div");
        return k.innerHTML = s.trim(), k.firstElementChild?.tagName == "A" ? (k.firstElementChild.className = y, k.innerHTML) : me(s, {
          className: y,
          target: "_blank",
          validate: {
            url: (H) => /^https?:\/\//.test(H)
  ...
  setup(v) {
    ...
    }, p = X("iApi"), $ = ee(), f = ne(), _ = v, { t: u } = V(), r = w(!1), d = w(p.fixture.get("details")), n = w(!0), e = w(!1), o = w(0), E = w(20), O = w([]), D = w([]), m = M(() => $.activeGreedy), I = M(() => $.properties), h = M(() => o.value + E.value), B = () => f.getLayerByUid(_.uid), N = () => _.results.find((l) => l.uid === _.uid), z = M(() => N()?.loaded ?? !1), C = M(() => N()?.requestTime), s = M(
    ...
    return ae(() => {
      O.value.push(
        p.event.on(J.LAYER_REMOVE, (l) => {
          const x = p.panel.get("details");
          _.uid === l.uid && x && x.close();
        })
      ), O.value.push(
        p.event.on(J.PANEL_CLOSED, (l) => {
          l.id === "details" && ge();
        })
      ), O.value.push(
        p.event.on(J.PANEL_MINIMIZED, (l) => {
          l.id === "details" && _e();
        })
      ), O.value.push(
        p.event.on(J.MAP_BASEMAPCHANGE, (l) => {
      ...
      D.value.forEach((l) => l()), O.value.forEach((l) => p.event.off(l)), b.value?.removeEventListener("blur", i), b.value?.removeEventListener("keyup", c);
    }), (l, x) => {
      const ke = q("truncate"), te = q("tippy"), xe = q("focus-item"), Le = q("focus-list");
      return z.value && m.value === 0 ? (t(), a("div", {
        key: 0,
        ...
        oe.value ? (t(), a("div", pt, [
          g("label", mt, S(L(u)("details.togglehilight.title")), 1),
          se(Ee, {
            config: {
              value: n.value,
              disabled: !1
  ...
  setup(v) {
    const { t: b } = V(), i = X("iApi"), c = ee(), p = w([]), $ = w([]), f = w([]), _ = w(!1), u = w(""), r = M(() => c.activeGreedy), d = M(() => c.payload), n = M(() => c.properties), e = (m) => {
      u.value = m;
    }, o = (m) => f.value.find((I) => I.uid === m), E = (m) => {
      m !== void 0 && (c.activeGreedy = m.length === 0 ? 0 : m[0].requestTime, f.value = m, O(m));
    }, O = (m) => {
      if (u.value) {
        const I = o(u.value);
        I ? I.loading.then(() => {
          I.requestTime === r.value && (I.items.length > 0 ? (c.activeGreedy = 0, _.value = !1) : D(m));
        }) : D(m);
      } else
        D(m);
    }, D = (m, I) => {
      ...
      if (I)
        ...
        const C = c.properties, s = m.map((k) => [
    ...
    return Q(() => {
      ...
      p.value.forEach((m) => i.event.off(m)), $.value.forEach((m) => m());
    }), (m, I) => {
...
}), ms = /* @__PURE__ */ ie(zt, [["__scopeId", "data-v-150a1d19"]]);
export {
  ms as default
};
