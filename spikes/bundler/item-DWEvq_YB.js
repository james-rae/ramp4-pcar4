import { defineComponent as Q, ref as V, onMounted as ce, resolveDirective as N, openBlock as l, createElementBlock as a, withModifiers as k, withDirectives as x, createElementVNode as o, unref as t, withKeys as se, normalizeClass as C, inject as ge, computed as U, toRaw as M, resolveComponent as me, createVNode as Le, withCtx as de, createTextVNode as B, toDisplayString as w, Fragment as le, renderList as ne, normalizeStyle as pe, createCommentVNode as v, watch as ke, createStaticVNode as re, createBlock as Z, resolveDynamicComponent as xe } from "vue";
import { p as we, o as u, X as Ce, a as ye, $ as I, G as K, _ as ae, N as Me, n as Se, a1 as f, a2 as G, S as Te, a3 as O, a4 as ee } from "./main-Caf1HWWJ.js";
import "pinia";
...
const Ee = ["type", "aria-label", "checked", "disabled", "content"], te = /* @__PURE__ */ Q({
  ...
  setup(e) {
    ...
    const b = (i) => !i.symbologyStack.some((d) => d.visibility), $ = () => {
      if (s.value instanceof we)
        ...
        if (b(s.legendItem) ? (s.legendItem.setSymbologyVisibility(void 0, !1), s.legendItem.setSymbologyVisibility(s.value.uid, !0), s.legendItem.toggleVisibility(!0)) : s.legendItem.setSymbologyVisibility(s.value.uid, !s.value.lastVisbility), b(s.legendItem) && s.legendItem.toggleVisibility(!1), s.legendItem.layer?.supportsFeatures) {
          ...
          let d = "";
          i.length === 0 ? d = "1=2" : i.length < s.legendItem.symbologyStack.length && (d = i.join(" OR ")), s.legendItem.layer?.setSqlFilter(Ce.SYMBOL, d);
        }
        m.value = !0;
      }
    };
  ...
  setup(e) {
    const { t: y } = oe(), s = ge("iApi"), m = V(), b = V(!1), $ = ye(), i = V($.mobileView), d = e, T = () => {
      d.legendItem.layerControlAvailable(I.Symbology) && d.legendItem.toggleSymbology();
    }, P = () => {
      d.legendItem.layerControlAvailable(I.Datatable) && A("grid") && s.event.emit(K.GRID_TOGGLE, d.legendItem.layer);
    }, q = () => {
      d.legendItem.layerControlAvailable(I.Settings) && A("settings") && s.event.emit(K.SETTINGS_TOGGLE, d.legendItem.layer);
    }, X = () => {
      if (d.legendItem.layerControlAvailable(I.Metadata) && A("metadata")) {
      ...
      d.legendItem.layerControlAvailable(I.BoundaryZoom) && d.legendItem?.layer?.zoomToLayerBoundary();
    }, F = () => {
      d.legendItem.layerControlAvailable(I.Remove) && s.geo.map.removeLayer(d.legendItem.layerUid);
    }, D = () => {
      j.value && (M(d.legendItem.layer).reload(), m.value.open = !1);
    }, j = U(() => d.legendItem.layerControlAvailable(I.Reload) && d.legendItem instanceof u && M(d.legendItem.layer)?.canReload), Y = (z) => {
  ...
  setup(e) {
    ...
    i.legendItem instanceof u && M(i.legendItem.layer)?.sublayers.length > 0), L = (r) => r === G.Expand || r === G.Visibility ? i.legendItem.controlAvailable(r) : i.legendItem.layerControlAvailable(r), R = (r) => Ve(r), E = () => {
      ...
      if (L(I.Symbology)) {
        const r = i.legendItem.toggleSymbology();
        b.updateAlert(m(`legend.alert.symbology${r ? "Expanded" : "Collapsed"}`));
      }
    }, ue = () => {
      ...
      i.legendItem.reload(), setTimeout(() => {
        ...
        if (n) {
          const h = r.isSublayer ? r.parentLayerId : r.layerId, p = X.value.find((S) => S.id === h);
          p !== void 0 && be(p);
        }
      ...
      const r = M(i.legendItem);
      let n = 0;
      if (r.type === f.Error) {
        i.legendItem.toggleHidden(!0);
        const h = setInterval(() => {
          const p = r.layer;
          (p && (p.layerExists || p.initiationState === ee.NEW || p.initiationState === ee.TERMINATING || p.initiationState === ee.TERMINATED) || n > 1200) && (clearInterval(h), p && b.geo.map.removeLayer(p), y.removeLayerConfig(r.layerId), b.fixture.get("legend")?.removeLayerItem(r.layerId)), n++;
        }, 250);
      } else {
        i.legendItem.error();
        const h = setInterval(() => {
          const p = r.parentLayerId || r.layerId, S = b.geo.layer.getLayer(p);
      ...
      i.legendItem.loadPromise.then(() => {
        ...
        Promise.all(
          M(i.legendItem.symbologyStack.map((r) => r.drawPromise))
        ).then(() => {
          ...
          x((l(), a("div", {
            ...
            e.legendItem.type !== t(f).Item ? (l(), a("div", _e, [
              e.legendItem.type === t(f).Placeholder ? (l(), a("svg", et, n[12] || (n[12] = [
                re('<path d="M0 0h24v24H0V0z" fill="none" data-v-9c9a8adc></path><path d="M0 0h24v24H0V0z" fill="none" data-v-9c9a8adc></path><circle cx="15.5" cy="9.5" r="1.5" data-v-9c9a8adc></circle><circle cx="8.5" cy="9.5" r="1.5" data-v-9c9a8adc></circle><circle cx="15.5" cy="9.5" r="1.5" data-v-9c9a8adc></circle><circle cx="8.5" cy="9.5" r="1.5" data-v-9c9a8adc></circle><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm0-2.5c2.33 0 4.32-1.45 5.12-3.5h-1.67c-.69 1.19-1.97 2-3.45 2s-2.75-.81-3.45-2H6.88c.8 2.05 2.79 3.5 5.12 3.5z" data-v-9c9a8adc></path>', 7)
              ]))) : (l(), a("svg", tt, n[13] || (n[13] = [
                re('<path d="M0 0h24v24H0V0z" fill="none" data-v-9c9a8adc></path><path d="M0 0h24v24H0V0z" fill="none" data-v-9c9a8adc></path><circle cx="15.5" cy="9.5" r="1.5" data-v-9c9a8adc></circle><circle cx="8.5" cy="9.5" r="1.5" data-v-9c9a8adc></circle><circle cx="15.5" cy="9.5" r="1.5" data-v-9c9a8adc></circle><circle cx="8.5" cy="9.5" r="1.5" data-v-9c9a8adc></circle><path d="M 20,12C 20,7.6 16.4,4 12,4C 7.6,4 4,7.6 4,12C 4,16.4 7.6,20 12,20C 16.4,20 20,16.4 20,12 Z M 22,12C 22,17.5 17.5,22 12,22C 6.5,22 2,17.5 2,12C 2,6.5 6.5,2.00001 12,2.00001C 17.5,2.00001 22,6.5 22,12 Z M 15.5,8C 16.3,8 17,8.7 17,9.5C 17,10.3 16.3,11 15.5,11C 14.7,11 14,10.3 14,9.5C 14,8.7 14.7,8 15.5,8 Z M 10,9.5C 10,10.3 9.3,11 8.5,11C 7.7,11 7,10.3 7,9.5C 7,8.7 7.7,8 8.5,8C 9.3,8 10,8.7 10,9.5 Z M 12,14C 13.7524,14 15.2943,14.7212 16.1871,15.8129L 14.7697,17.2302C 14.3175,16.5078 13.2477,16 12,16C 10.7523,16 9.68254,16.5078 9.23024,17.2302L 7.81291,15.8129C 8.7057,14.7212 10.2476,14 12,14 Z" data-v-9c9a8adc></path>', 7)
              ])))
            ])) : v("", !0),
            ...
            e.legendItem instanceof t(u) && t(T) ? x((l(), a("div", {
              ...
              e.legendItem.infoType === t(O).Title && e.legendItem.content ? (l(), a("h3", st, w(e.legendItem.content), 1)) : e.legendItem.infoType === t(O).Title ? (l(), a("h3", dt, w(e.legendItem.name), 1)) : e.legendItem.infoType === t(O).Text ? (l(), a("p", rt, w(e.legendItem.content), 1)) : e.legendItem.infoType === t(O).Image ? (l(), a("img", {
                key: 3,
...
}), Cl = /* @__PURE__ */ ae(Ot, [["__scopeId", "data-v-9c9a8adc"]]);
export {
  Cl as default
};
