import{is as X,le as D,it as j,aB as $,iw as R,aC as v,iL as B,iM as O,iR as F,iK as V,iN as q,pO as U,iy as z,hE as r,iB as k,hD as d,hF as f,iG as c,iA as u,iP as G,iz as K}from"./main-Dq7sl68_.js";import{k as M}from"./marked.esm-B5D8JARp.js";import"./preload-helper-DMGCcr4D.js";const Y=`<?xml version="1.0" encoding="UTF-8"?>
...
`,Z={key:0},ee={class:"flex justify-center"},te={key:0,class:"flex flex-col justify-center text-center"},ne=["innerHTML"],ae=["innerHTML"],le={key:3,class:"flex flex-col justify-center text-center"},se={class:"text-xl mt-20"},oe={key:1,class:"p-5"},me=X({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(i){const l=D(),{t:o}=j(),x=O("iApi"),I=$(),n=i,w=R(()=>l.status),y=R(()=>l.response),C=$(!1),p=v({}),P=v([]),T=v([]);B(()=>{l.status="loading",S(),P.push(x.event.on(F.LAYER_REMOVE,e=>{n.payload.layer?.uid===e.uid&&n.panel.close()})),T.push(V(()=>n.payload.layer.uid,(e,t)=>{e!==t&&(l.status="loading",S())}))}),q(()=>{P.forEach(e=>x.event.off(e)),T.forEach(e=>e())});const S=()=>{C.value=n.payload.layer!==void 0&&!n.payload.layer.isRemoved,n.payload.type==="xml"?N(n.payload.url,[]).then(e=>{if(l.status="success",e!==null){const t=document.createElement("div");t.appendChild(g(`${e.firstElementChild.outerHTML}`)),(n.payload.catalogueUrl||n.payload.url)&&t.appendChild(g(`<h5 class="text-xl font-bold mb-3">${o("metadata.xslt.metadata")}</h5>`)),n.payload.catalogueUrl&&t.appendChild(g(`<p><a style="color: blue;" href="${n.payload.catalogueUrl}" target="_blank">${o("metadata.xslt.cataloguePage")}</a></p>`)),t.appendChild(g(`<p><a style="color: blue;" href="${n.payload.url}" target="_blank">${o("metadata.xslt.metadataPage")}</a> (xml)</p>`)),l.response=t.outerHTML}}):n.payload.type==="html"?b(n.payload.url).then(e=>{l.status=e.status,l.response=e.response}):n.payload.type==="md"&&b(n.payload.url).then(e=>{l.status=e.status,l.response=M(e.response)})},N=(e,t)=>{let a;return n.payload.xmlType&&n.payload.xmlType==="DCAT"?a=x.language==="en"?Q:W:a=x.language==="en"?Y:J,a=a.replace(/\{\{([\w.]+)\}\}/g,(s,m)=>o(m)),p[e]?Promise.resolve(_(p[e],a,t)):b(e).then(s=>(p[e]=s.response,_(p[e],a,t)))},_=(e,t,a)=>{let s=null;if(window.XSLTProcessor){const m=new window.XSLTProcessor,L=new DOMParser,E=L.parseFromString(e,"text/xml"),A=L.parseFromString(t,"text/xml");m.importStylesheet(A),a&&a.forEach(h=>m.setParameter("",h.key,h.value||"")),s=m.transformToFragment(E,document);const H={className:"underline text-blue-700 break-all",target:"_blank",validate:{url:h=>/^https?:\/\//.test(h)}};n.payload.treatXmlAsMarkdown?s.firstChild.innerHTML=M.parse(s.firstChild.innerHTML):s.firstChild.innerHTML=U(s.firstChild.innerHTML,H)}return s},b=e=>new Promise(t=>{const a=new XMLHttpRequest;a.open("GET",e,!0),a.responseType="text",a.onload=()=>{a.status===200?t({status:"success",response:a.response}):t({status:"error",response:"Could not load results from remote service."})},a.onerror=()=>{t({status:"error",response:"Could not load results from remote service."})},a.send()});function g(e){const t=document.createElement("div");return t.innerHTML=e,t}return(e,t)=>{const a=K("panel-screen");return r(),z(a,{panel:i.panel,ref_key:"el",ref:I},{header:k(()=>[G(c(u(o)("metadata.title"))+": "+c(i.payload.layerName),1)]),content:k(()=>[C.value?(r(),d("div",Z,[f("div",ee,[w.value=="loading"?(r(),d("div",te,c(u(o)("metadata.loading")),1)):i.payload.type==="xml"&&w.value=="success"?(r(),d("div",{key:1,innerHTML:y.value,class:"flex flex-col justify-center max-w-full xml-content"},null,8,ne)):(i.payload.type==="html"||i.payload.type==="md")&&w.value=="success"?(r(),d("div",{key:2,innerHTML:y.value,class:"flex flex-col justify-center max-w-full metadata-view"},null,8,ae)):(r(),d("div",le,[t[0]||(t[0]=f("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1)),f("span",se,c(u(o)("metadata.error")),1)]))])])):(r(),d("div",oe,[f("span",null,c(u(o)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{me as default};
