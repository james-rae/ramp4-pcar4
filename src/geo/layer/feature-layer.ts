import { AttribLayer, InstanceAPI, OidIdentifyItem } from '@/api/internal';
        ...
        Promise.all([clientBlocker, serverBlocker])
            .then(() => {
                ...
                hitBucket.forEach(hitOid => {
                    // TODO see if we need reactive wrapper still
                    //     otherwise make .blocker public and underscore on identifyitem to stop TS from crying when things get reactive
                    /*
...
*/
                    ...
                    result.items.push(
                        reactive(new OidIdentifyItem(hitOid, this))
                    );
                });
