import{dK as A,fy as H,dL as j,e0 as R,e1 as O,dO as T,gg as C,e6 as V,eb as X,e5 as B,e7 as F,dP as D,dT as q,dU as k,dS as o,e8 as U,dZ as m,d_ as p,dW as d,dV as h}from"./main-COW1-tZE.js";import{m as G}from"./marked.esm-DmSrlTiT.js";import"./preload-helper-ExcqyqRp.js";const K=`<?xml version="1.0" encoding="UTF-8"?>
...
`,Y={key:0},Z={class:"flex justify-center"},z={key:0,class:"flex flex-col justify-center text-center"},J=["innerHTML"],Q=["innerHTML"],ee={key:3,class:"flex flex-col justify-center text-center"},te={class:"text-xl mt-20"},ne={key:1,class:"p-5"},oe=A({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(r){const s=H(),{t:l}=j(),u=R("iApi"),M=O(),a=r,f=T(()=>s.status),b=T(()=>s.response),v=O(!1),x=C({}),y=C([]),S=C([]);V(()=>{_(),y.push(u.event.on(X.LAYER_REMOVE,e=>{a.payload.layer?.uid===e.uid&&a.panel.close()})),S.push(B(()=>a.payload.layer.uid,(e,t)=>{e!==t&&_()}))}),F(()=>{y.forEach(e=>u.event.off(e)),S.forEach(e=>e())});const _=()=>{v.value=a.payload.layer!==void 0&&!a.payload.layer.isRemoved,a.payload.type==="xml"?I(a.payload.url,[]).then(e=>{if(s.status="success",e!==null){const t=document.createElement("div");t.appendChild(g(`${e.firstElementChild.outerHTML}`)),(a.payload.catalogueUrl||a.payload.url)&&t.appendChild(g(`<h5 class="text-xl font-bold mb-3">${l("metadata.xslt.metadata")}</h5>`)),a.payload.catalogueUrl&&t.appendChild(g(`<p><a style="color: blue;" href="${a.payload.catalogueUrl}" target="_blank">${l("metadata.xslt.cataloguePage")}</a></p>`)),t.appendChild(g(`<p><a style="color: blue;" href="${a.payload.url}" target="_blank">${l("metadata.xslt.metadataPage")}</a> (xml)</p>`)),s.response=t.outerHTML}}):a.payload.type==="html"?w(a.payload.url).then(e=>{s.status=e.status,s.response=e.response}):a.payload.type==="md"&&w(a.payload.url).then(e=>{s.status=e.status,s.response=G(e.response)})},I=(e,t)=>{let n=u.language==="en"?K:W;return n=n.replace(/\{\{([\w.]+)\}\}/g,(i,c)=>l(c)),x[e]?Promise.resolve($(x[e],n,t)):w(e).then(i=>(x[e]=i.response,$(x[e],n,t)))},$=(e,t,n)=>{let i=null;if(window.XSLTProcessor){const c=new window.XSLTProcessor,L=new DOMParser,N=L.parseFromString(e,"text/xml"),E=L.parseFromString(t,"text/xml");c.importStylesheet(E),n&&n.forEach(P=>c.setParameter("",P.key,P.value||"")),i=c.transformToFragment(N,document)}return i},w=e=>new Promise(t=>{const n=new XMLHttpRequest;n.open("GET",e,!0),n.responseType="text",n.onload=()=>{n.status===200?t({status:"success",response:n.response}):t({status:"error",response:"Could not load results from remote service."})},n.onerror=()=>{t({status:"error",response:"Could not load results from remote service."})},n.send()});function g(e){const t=document.createElement("div");return t.innerHTML=e,t}return(e,t)=>{const n=D("panel-screen");return o(),q(n,{panel:r.panel,ref_key:"el",ref:M},{header:k(()=>[U(m(p(l)("metadata.title"))+": "+m(r.payload.layerName),1)]),content:k(()=>[v.value?(o(),d("div",Y,[h("div",Z,[f.value=="loading"?(o(),d("div",z,m(p(l)("metadata.loading")),1)):r.payload.type==="xml"&&f.value=="success"?(o(),d("div",{key:1,innerHTML:b.value,class:"flex flex-col justify-center xml-content"},null,8,J)):(r.payload.type==="html"||r.payload.type==="md")&&f.value=="success"?(o(),d("div",{key:2,innerHTML:b.value,class:"flex flex-col justify-center max-w-full metadata-view"},null,8,Q)):(o(),d("div",ee,[t[0]||(t[0]=h("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1)),h("span",te,m(p(l)("metadata.error")),1)]))])])):(o(),d("div",ne,[h("span",null,m(p(l)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{oe as default};
