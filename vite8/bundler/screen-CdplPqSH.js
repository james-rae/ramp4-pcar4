import { B as e, pt as t } from "./main-dpIMyHMK.js";
...
var O = [
	...
	setup(n) {
		let a = f("iApi"), { t: d } = E(), y = _([]), O = _({}), H = _({}), U = _([]), W = _([]), G = _([]), K = i(() => a.animate), q = () => {
			...
			let n = a.geo.layer.getLayer(e.id), r = y.value.indexOf(e);
			if (n === void 0 || r === -1) return;
			let i = r - t;
			if (i < 0 || i >= y.value.length) return;
			let o = y.value[i].orderIdx;
			a.geo.map.reorder(n, o), a.updateAlert(d("layer-reorder.layermoved", {
				name: e.name,
				index: o
			}));
			let s = t === 1 ? "up" : "down";
			setTimeout(() => {
				O.value[e.id + "-" + s]?.focus();
			}, 0);
