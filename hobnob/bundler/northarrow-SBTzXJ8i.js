import { $ as e, V as t, X as n, ct as r, lt as i, s as a, tt as o, y as s } from "./main-Ck4jBxAm.js";
import { t as c } from "./config-vKemIUe8.js";
...
var x = class extends a {
	...
	setup(a) {
		...
		g(() => {
			F.value = f.config.map.tileSchemas, D?.value && (M.value = `<img width='25' src='${D.value}'>`), w.geo.map.esriView?.ready && L(w.geo.map.getExtent()), P.push(w.event.on(t.MAP_EXTENTCHANGE, b(L, 300)));
		}), h(() => {
		...
		let L = async (t) => {
			...
			if (a || a === void 0 && !u.isWebMercator()) {
				let t = new e("pole", {
					x: -96,
					y: 90
				}), a = await w.geo.proj.projectGeometry(u, t), d = w.geo.map.mapPointToScreenPoint(a);
				if (d.screenY < 0) {
					j.value = !0;
					let e = {
