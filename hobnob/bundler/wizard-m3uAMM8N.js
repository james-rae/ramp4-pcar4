import { J as e, ct as t, m as n, q as r, s as i } from "./main-Ck4jBxAm.js";
...
var s = class extends i {
	...
	async fetchFileInfo(e, n, r) {
		switch (r ||= await this.$iApi.geo.layer.files.fetchFileData(e, n), n) {
			case t.GEOJSON: return this.getGeojsonInfo(e, r);
			case t.SHAPEFILE: return this.getShapfileInfo(e, r);
			case t.CSV: return this.getCsvInfo(e, r);
			default: console.error(`Unsupported file type passed to fetchFileInfo - '${n}'`);
		}
	}
	async getGeojsonInfo(e, n) {
	...
	async fetchServiceInfo(e, n, r, i) {
		switch (n) {
			case t.FEATURE: return this.getFeatureInfo(e);
			case t.MAPIMAGE: return this.getMapImageInfo(e, r);
			case t.TILE: return this.getTileInfo(e);
			case t.IMAGERY: return this.getImageryInfo(e);
			case t.WFS: return this.getWfsInfo(e, i);
			case t.WMS: return this.getWmsInfo(e, r);
		}
	}
	async getFeatureInfo(e) {
		let n = await o.get(e, { params: { f: "json" } });
		return {
			config: {
				id: `${t.FEATURE}#${++this.layerCount}`,
				url: e,
				layerType: t.FEATURE,
				name: n.data.name,
	...
	async getMapImageInfo(e, n) {
		let r = await o.get(e, { params: { f: "json" } });
		return {
			config: {
				id: `${t.MAPIMAGE}#${++this.layerCount}`,
				url: e,
				layerType: t.MAPIMAGE,
				name: r.data.mapName,
				sublayers: [],
				state: {
					opacity: 1,
	...
	async getWfsInfo(t, n) {
		let { offset: r, limit: i } = new e(t).queryMap, a = await this.$iApi.geo.layer.ogc.loadWfsData(t, -1, parseInt(r) || 0, parseInt(i) || 1e3, void 0, !1, n);
		return this.getGeojsonInfo(t.match(/\/([^/]+)\/items/)?.[1] || "Layer", a);
	}
	async getWmsInfo(e, n) {
		let r = await this.$iApi.geo.layer.ogc.parseCapabilities(e);
		return {
			config: {
				id: `${t.WMS}#${++this.layerCount}`,
				url: e,
				layerType: t.WMS,
				name: e,
	...
	guessFormatFromURL(e) {
		switch (e.match(/\.(zip|csv|geojson|json)$/)?.[1]) {
			case "zip": return t.SHAPEFILE;
			case "csv": return t.CSV;
			case "geojson":
			case "json": return t.GEOJSON;
		}
		return e.match(/\/ImageServer\/?$/gi) ? t.IMAGERY : e.match(/\/collections\//gi) ? t.WFS : e.match(/arcgis\/rest\/services\//gi) ? e.match(/\/\d+\/?$/g) ? t.FEATURE : t.MAPIMAGE : e.match(/service=|version=|\/wms/gi) ? t.WMS : "";
	}
}, l = {
	en: {
	...
	added() {
		this.$iApi.panel.register({ wizard: {
			screens: { "wizard-screen": () => a(import("./screen-D9yhRB1b.js")) },
			button: {
