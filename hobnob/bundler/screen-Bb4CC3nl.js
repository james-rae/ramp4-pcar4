import { B as e, R as t, V as n, W as r, Y as i, it as a, nt as o, o as s, rt as c, st as l } from "./main-Ck4jBxAm.js";
import { n as u } from "./config-vKemIUe8.js";
import { t as d } from "./_plugin-vue_export-helper-C6QCLu9I.js";
import { n as ee, t as te } from "./table-state-manager-3UV4gjlb.js";
...
var z = ["onClick"], Te = { class: "md-icon-small flex w-full" }, B = { class: "flex-1 truncate whitespace-nowrap overflow-hidden pr-4" }, Ee = /* @__PURE__ */ v({
	...
	props: ["params"],
	setup(e) {
		let t = e, { t: r } = I(), i = y("iApi"), o = w(), s = async () => {
			let e = t.params.data, r = e.rvUid, o = i.geo.layer.getLayer(r), s = be(t.params.layerCols, o), c = await o.getGraphic(e[s], { getAttribs: !0 });
			i.event.emit(n.DETAILS_TOGGLE, {
				data: c.attributes,
				uid: r,
				format: a.ESRI
			}, !0), t.params.isTeleport && i.scrollToInstance();
		};
		return S(() => {
	...
	setup(a) {
		...
		let s = [
			...
			c.SINGLE,
			c.INTEGER
		], u = y("iApi"), d = r(), ne = e(), p = f(() => ne.mobileView), v = w(!p.value), T = w(), k = le("gridContainer"), { t: L, locale: be } = I(), xe = () => ie()?.proxy?.$forceUpdate(), R = a, z = w({
			id: "dummy",
			layerIds: [],
			state: new te(),
			fieldMap: {}
		...
		}, Je = f(() => X.value.some((e) => e.layerState === l.LOADED && e.visibility)), Ye = (e) => {
			B.value = e.api;
			let t = z.value.state.title;
			t ||= u.geo.layer.getLayer(R.gridId)?.name || R.gridId, Ne.value = t, Jt(), q.value.length > 0 && Z.value.autoSizeAllColumns(), Ke(), qe(), Z.value.addEventListener("rowDataUpdated", Ke), W.value.push(u.event.on(n.FILTER_CHANGE, ({ uid: e, filterKey: t }) => {
				t !== o.GRID && e && X.value.map((e) => e.uid).includes(e) && Q();
			})), W.value.push(u.event.on(n.LAYER_VISIBILITYCHANGE, ({ layer: e }) => {
				e.uid && X.value.map((e) => e.uid).includes(e.uid) && Q();
			})), W.value.push(u.event.on(n.LAYER_RELOAD_END, (e) => {
				e.loadPromise().then(() => {
					X.value.map((e) => e.uid).includes(e.uid) && Q();
				});
			})), W.value.push(u.event.on(n.LANG_CHANGE, () => {
				Z.value.redrawRows({ force: !0 });
			})), W.value.push(u.event.on(n.MAP_EXTENTCHANGE, de(() => {
				z.value.state.filterByExtent && Q();
			}, 100))), W.value.push(u.event.on(n.LAYER_REMOVE, (e) => {
			...
			let e = new i(), t = We.value.slice().map((e) => e.getPromise());
			We.value.push(e), await Promise.all(t), await Promise.all(X.value.map(async (e) => {
				e && e.visibility ? await e.getFilterOIDs([o.GRID], z.value.state.filterByExtent ? u.geo.map.getExtent() : void 0).then((t) => {
					ze.value[e.uid] = t;
				}) : ze.value[e.uid] = [];
			...
			X.value.filter((e) => e.mapLayer).forEach((e) => {
				if (!z.value.state.applyToMap) e.setSqlFilter(o.GRID, "");
				else {
					let t = ln(e.id);
					e.setSqlFilter(o.GRID, t);
				}
			});
		}, ln = (e) => {
			...
			[
				...
				if (e.special) rn(r, K.value, z.value.state);
					...
					r.cellRenderer = t.template === "" ? nt : u.component(t.template), r.autoHeight = !0, s.indexOf(e.type) > -1 ? (tn(r, z.value.state), r.filter = "agNumberColumnFilter", r.cellRendererParams = { type: "number" }) : e.type === c.DATE ? ($t(r, z.value.state), r.filter = "agDateColumnFilter", r.minWidth = 400, r.cellRendererParams = { type: "date" }) : e.type === c.STRING && (n ? en(r, i.rows, z.value.state) : nn(r, z.value.state), r.filter = "agTextColumnFilter", r.cellRendererParams = { type: "string" }), K.value.push(r);
				}
