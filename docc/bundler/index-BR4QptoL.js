import "tiny-emitter";
import { F as C, s as y, l as G, G as T, P as W, t as z, L as B, v as H, w as R } from "./main-BHVuCadI.js";
...
const Q = "path", Z = 12, ee = "M14.4 6L14 4H5v17h2v-7h5.6l.4 2h7V6z", te = "#ff0000ff", oe = 5, re = 6, ie = {
  ...
  __name: "northarrow",
  setup(S) {
    const t = G(), o = y(), e = j("iApi"), f = r(), n = E(() => o.arrowIcon), M = E(() => o.poleIcon), l = r(0), s = r(0), c = r(!1), P = r(`<svg xmlns="http://www.w3.org/2000/svg" fit=""  width="25" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" focusable="false">
    ...
    const I = async (i) => {
      ...
      const p = e.$vApp.$el.querySelector(".inner-shell"), h = f.value.querySelector(".northarrow").getBoundingClientRect().width, u = e.$vApp.$el.querySelector(".appbar")?.clientWidth || 0, x = i.sr;
      if (m || typeof m > "u" && !x.isWebMercator()) {
        const d = new W("pole", { x: -96, y: 90 }), L = await e.geo.proj.projectGeometry(x, d), v = e.geo.map.mapPointToScreenPoint(L);
        if (v.screenY < 0) {
          c.value = !0;
          const a = {
            screenX: p.clientWidth / 2,
            screenY: p.clientHeight
          };
          l.value = Math.atan(
            (v.screenX - a.screenX) / (a.screenY - v.screenY)
          ) * 180 / Math.PI, s.value = p.clientWidth / 2 + p.clientHeight * Math.tan(l.value * Math.PI / 180) - h / 2, s.value = Math.max(
            u - h / 2,
            Math.min(e.geo.map.getPixelWidth() - h / 2, s.value)
          );
        ...
        c.value = !0, l.value = 0, s.value = u + (p.clientWidth - u - h) / 2;
    };
    return (i, m) => (V(), D("div", {
