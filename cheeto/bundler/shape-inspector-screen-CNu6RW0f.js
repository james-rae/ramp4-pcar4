import { $ as e, et as t } from "./main-C-ApdCak.js";
import { i as n, w as r } from "./esri-vcEQ1sbb.js";
import { t as i } from "./_plugin-vue_export-helper-B3ysoDQm.js";
import { S as a, b as o, c as s, f as ee, g as te, i as ne, t as re, u as ie, x as ae, y as oe } from "./store-CbXVpAzw.js";
import { t as se } from "./color-control-Cd7ceWES.js";
import { t as ce } from "./download-icon-CfXMdF48.js";
import { n as le, r as ue } from "./shape-io-C4MmbuPn.js";
...
var Le = { class: "rv-header-title" }, Re = {
	...
	setup(i) {
		let Ce = i, { t: D, locale: dr } = Fe(), O = we("iApi"), k = re(), A = v(Ce.initialTab), j = v(!1), M = v([]), N = v([]), P = v([]), F = v(""), fr = v([]), pr = v(null), I = v(!1), L = v(!1), R = v(!1), mr = 0, z = [
			{
				id: "details",
		...
		], B = u(() => k.getSelectedGraphic()), V = u(() => B.value?.id ?? B.value?.attributes?.id), H = u(() => Ae(B.value?.geometry)), U = u(() => B.value?.attributes), W = u(() => a(U.value)), G = u(() => oe(U.value)), K = u(() => ae(U.value)), hr = u(() => B.value?.type ?? U.value?.type ?? H.value?.type), gr = u(() => D(`draw.details.shapeType.${hr.value ?? "unknown"}`)), _r = u(() => `${gr.value} - ${Y.value > 0 ? D("draw.inspector.subtitle.segments", { count: Y.value }) : D("draw.inspector.subtitle.selected")}`), q = u(() => [
			"polyline",
			"polygon",
			"rectangle"
		].includes(hr.value ?? "")), vr = u(() => q.value && N.value.length > 0), yr = u(() => P.value.length), br = u(() => H.value?.type === "polygon"), xr = u(() => G.value.distance > 0), Sr = u(() => o(U.value, k.identifyBufferMode)), J = u(() => {
			let e = V.value;
			...
			try {
				let i = await Kr(e), a = t === "x" ? i.x : i.y;
				return Gr(a, n, r);
			} catch {
				return Ur(e);
			}
			...
			k.setSelectedGraphicStyleSettings(ie(k.styleSettings)), k.setSelectedGraphicBufferSettings(s(k.bufferSettings)), k.setSelectedGraphicIdentifyBufferMode(k.identifyBufferMode);
		}, mi = (e) => {
			if (e === "default") {
				pi();
				return;
		...
		return Pe(() => Ce.tabRequestId, () => {
			...
			return _(), Se(n, { panel: i.panel }, {
				...
				content: C(() => [p("div", ze, [p("div", {
					...
					}, [(_(!0), f(l, null, y(x(ne), (e) => (_(), f("option", {
						key: e.value,
						value: e.value
