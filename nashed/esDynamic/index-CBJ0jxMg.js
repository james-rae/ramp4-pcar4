import { bx as ke, nJ as R, nK as Le, bC as j, bF as k, bG as le, bN as i, bT as o, fC as _, bU as Z, bV as W, fD as G, iG as me, fI as M, bP as z, i4 as ne, bI as I, bK as F, bS as C, bR as ie, ii as pe, bD as V, lu as oe, fG as Y, fH as P, bH as X, bL as q, bQ as L, bE as ee, nL as ve, kF as $e, iB as Ie, nM as Te, fK as Q, bO as ae, bJ as Se, bM as de, ha as He, hb as Ae } from "./main-dhqBKhJU.js";
import { H as ze } from "./hilight-defs-DzKgjtLG.js";
import { T as Me } from "./toggle-switch-control-kpM55am3.js";
class De {
  id;
  name;
  template;
...
const Ce = {
  ...
  setup(g) {
    ...
    return le(() => {
      ...
      _("div", qe, [
        g.result.loaded ? (i(), o("div", Re, z(g.result.items.length), 1)) : M("", !0)
      ])
    ])) : (i(), o("div", Ve, l[0] || (l[0] = [
  ...
  setup(g, { emit: e }) {
    const { t } = V(), s = oe(), l = k(), r = () => {
      l.value._tippy.hide();
    }, u = (x) => {
      x.key === "Tab" && l.value?.matches(":focus") && l.value._tippy.show();
    }, y = e, v = g, f = k(""), c = k([]), d = k(!1), a = k(!1), m = (x) => s.getLayerByUid(x), $ = (x) => {
  ...
  setup(g) {
    const e = oe(), t = g, s = ee("iApi"), l = k([]), r = R(), { t: u } = V(), y = k(""), v = k("none"), f = k(), c = () => e.getLayerByUid(t.uid), d = I(
      () => r.properties
    ), a = I(
      () => r.defaultTemplates
    ), m = I(() => c()?.supportsFeatures ?? !1), $ = I(() => c()?.mapLayer ?? !1), D = I(() => {
      const n = c()?.nameField;
      let h = n && t.data.loaded ? t.data.data[n] : s.$i18n.t("details.items.title");
      return s.ui.isPlainText(h) && (h = s.ui.escapeHtml(h)), h;
    }), E = (n) => {
      if (typeof n == "string") {
        const h = "underline text-blue-700 break-all", S = document.createElement("div");
        return S.innerHTML = n.trim(), S.firstElementChild?.tagName == "A" ? (S.firstElementChild.className = h, S.innerHTML) : ve(n, {
          className: h,
          target: "_blank",
          validate: {
            url: (B) => /^https?:\/\//.test(B)
      ...
      if (n.supportsFeatures) {
        const h = n.oidField;
        n.getIcon(t.data.data[h]).then((S) => {
          y.value = S;
        });
      }
    }, x = I(() => {
      ...
      const h = t.data.data[n.oidField], S = () => {
        const O = { getGeom: !0 };
...
}, wt = { class: "flex" }, kt = ["aria-label"], Lt = ["content", "aria-label", "disabled"], $t = { class: "px-3 text-center flex-grow" }, It = ["content", "aria-label", "disabled"], Tt = { key: 3 }, St = { key: 0 }, Ht = ["content"], At = ["onClick"], zt = {
  key: 1,
  class: "text-center"
}, Mt = {
  key: 4,
  ...
  setup(g) {
    const e = k(), t = () => {
    ...
    }, l = ee("iApi"), r = R(), u = oe(), y = g, { t: v } = V(), f = k(!1), c = k(l.fixture.get("details")), d = k(!0), a = k(!1), m = k(0), $ = k(20), D = k([]), E = k([]), b = I(() => r.activeGreedy), T = I(
      () => r.properties
    ), x = I(() => m.value + $.value), H = () => u.getLayerByUid(y.uid), N = I(() => y.results.find((w) => w.uid === y.uid)?.loaded ?? !1), A = I(() => y.results.find((w) => w.uid === y.uid)?.requestTime), U = I(
      () => f.value && (!a.value && h().length > 1 || a.value && h().length > $.value)
    ), n = I(() => {
      const p = H();
      return p && T.value[p.id] && T.value[p.id].name ? T.value[p.id].name : p?.name ?? "";
    }), h = () => {
      const p = y.results.find((w) => w.uid === y.uid);
      return p ? p.items : [];
    }, S = I(() => h()[m.value]), O = I(() => {
      ...
      if (d.value && N && p.length > 0 && O.value)
        if (a.value)
          ...
          const w = p[m.value];
          w && c.value.hilightDetailsItems([w], y.uid);
        }
      else
    ...
    return le(() => {
      D.value.push(
        l.event.on(
          Q.LAYER_REMOVE,
          (p) => {
            const w = l.panel.get("details-panel");
            y.uid === p.uid && w && w.close();
          }
        )
      ), D.value.push(
      ...
      E.value.push(
        P(
          () => h(),
          () => {
            fe(), S.value === void 0 && c.value.removeDetailsHilight();
          },
          {
            deep: !1,
            immediate: !0
      ...
      const _e = F("truncate"), te = F("tippy"), xe = F("focus-item"), we = F("focus-list");
      return N.value && b.value === 0 ? (i(), o("div", {
        ...
        ])) : (i(), o("div", Mt, z(L(v)("details.item.no.data")), 1))
      ], 4)) : (i(), o("div", {
        key: 1,
...
}), Et = /* @__PURE__ */ ne(Dt, [["__scopeId", "data-v-0cb6a8dd"]]), Ct = { class: "relative h-full" }, Ft = { class: "detailsContentSection overflow-y-auto h-full" }, Nt = /* @__PURE__ */ j({
  ...
  setup(g) {
    const { t: e } = V(), t = ee("iApi"), s = R(), l = k([]), r = k([]), u = k([]), y = k(!1), v = k(""), f = k(!1), c = I(() => s.activeGreedy), d = I(() => s.payload), a = I(
      () => s.properties
    ), m = (b) => {
      v.value = b, f.value = !0;
    }, $ = (b) => {
      ...
      s.activeGreedy = T, s.slowLoadingFlag = !1, u.value = b, D(b);
    }, D = (b) => {
      if (f.value) {
        ...
        if (T !== -1) {
          const x = u.value[T];
          x.loading.then(() => {
            x.requestTime === c.value && (x.items.length > 0 ? (s.activeGreedy = 0, f.value = !1, y.value = !1) : E(b));
          });
        } else
          E(b);
      } else
      ...
      const T = b.map(
        (H) => H.loading.then(
          () => H.items.length > 0 ? Promise.resolve(H) : Promise.reject()
        )
      ), x = b.length === 0 ? 0 : b[0].requestTime;
      Promise.any(T).then((H) => {
        if (H.requestTime !== c.value)
          return;
        ...
        content: de(() => [
          _("div", Ct, [
            ...
            _("div", Ft, [
              ...
              }, z(u.value.length >= 1 ? L(e)("details.layers.results.empty") : L(e)("details.layers.results.empty.noLayers")), 3)) : (i(), q(Et, {
                key: 0,
                uid: v.value,
                results: u.value
              }, null, 8, ["uid", "results"]))
...
//# sourceMappingURL=index-CBJ0jxMg.js.map
