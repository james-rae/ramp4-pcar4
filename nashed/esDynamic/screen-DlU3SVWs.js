...
}), jt = /* @__PURE__ */ ct(ea, [["__scopeId", "data-v-8aa4f1a3"]]);
var On = { exports: {} };
(function(d) {
  d.exports = /******/
  function(o) {
...
const na = /* @__PURE__ */ Sr(ta), ra = { key: 0 }, aa = { class: "text-base font-bold" }, oa = {
  ...
  setup(d, { emit: o }) {
    ...
    function C() {
      y.value = new ResizeObserver(function() {
        ...
        l.$vApp.$el.querySelector(".vue-treeselect__control")
      ), y.value.observe(
        l.$vApp.$el.querySelector(".vue-treeselect__menu")
      );
    }
    const g = () => {
      const b = l.$vApp.$el.querySelector(
        ".vue-treeselect__menu"
      )?.clientHeight, S = l.$vApp.$el.querySelector(
    ...
    const L = () => {
      if (E.value) {
        ...
        b && b.setAttribute(
          "aria-label",
          e("wizard.configure.sublayers.select")
        );
      }
    };
    return ut(() => {
...
}), ye = /* @__PURE__ */ ct(Ra, [["__scopeId", "data-v-4d422132"]]), wa = { class: "step relative flex flex-col px-12" }, ja = { class: "stepper-header flex pb-24" }, Fa = {
  key: 1,
...
}), Ft = /* @__PURE__ */ ct(Ba, [["__scopeId", "data-v-7693cdcc"]]), $a = { class: "py-12 h-auto stepper" }, Ha = /* @__PURE__ */ qe({
  __name: "stepper",
  props: {
    activeStep: {
      type: Number,
  ...
  setup(d) {
    ...
    const b = async ($) => {
      ...
      $?.preventDefault(), C.value = r.value.guessFormatFromURL(
        A.value
      ), o.goToStep(ze.FORMAT);
    }, P = async ($) => {
      $?.preventDefault(), p.value = !0, m.value = !1, w.value = !0, D.value = X() ? T.find(
        (J) => J.value === C.value
      )?.label : I.find(
        (J) => J.value === C.value
      )?.label;
      try {
        g.value = X() ? await r.value.fetchFileInfo(
          A.value,
          C.value,
          z.value
        ) : await r.value.fetchServiceInfo(
          A.value,
          C.value,
          o.nested
      ...
      if (o.nested = $, V.value = [], v.value += 1, C.value === ae.MAPIMAGE) {
        ...
        const B = new Set(
          (g.value?.config?.sublayers ?? []).map((J) => J.id)
        );
      ...
      const J = (K) => !K.layers || K.layers.length === 0 ? B.has(K.name) : K.layers.every(
        (de) => J(de)
      ), q = (K) => {
        J(K) ? V.value.push(K.name) : K.layers && K.layers.forEach(q);
      }, oe = $.value.layersRaw[0];
      oe && oe.layers && oe.layers.forEach(
        (K) => q(K)
      ), V.value = Array.from(new Set(V.value));
    }, Qe = ($, B) => {
      const J = (q) => {
        const oe = $.value.layersRaw.filter(
          (K) => K.parentLayerId === q
        );
        if (oe.length > 0)
          for (const K of oe)
      ...
      g.value.config.colour = $.colors.hex.substring(0, 7), Cr(() => {
        a.value.querySelector(
          ".vacp-copy-button"
        ).style.backgroundColor = g.value?.config.colour;
      });
    }, je = () => {
        ...
        content: ge(() => [
                  ...
                  H("form", {
                    ...
                    pe(ye, {
                      ...
                      "validation-messages": {
                        ...
                        )}.${L.value ? " " + G(l)("wizard.format.warn.cors") + "." : ""}${" " + G(l)("wizard.format.warn.vpn") + "."}`
                      },
              ...
              pe(Ft, {
                ...
                default: ge(() => [
                  H("form", {
                    ...
                    g.value?.configOptions.includes(
                      "tooltipField"
                    ) ? (k(), Ie(ye, {
                      key: 2,
                      type: "select",
          ...
          R.value ? (k(), te("div", {
            ...
            class: Je([
              ...
              M.value ? "bg-green-100 !border-green-900" : "bg-red-100 !border-red-900"
            ])
...
}), qa = /* @__PURE__ */ ct(ka, [["__scopeId", "data-v-52cf99d7"]]);
export {
  qa as default
};
//# sourceMappingURL=screen-DlU3SVWs.js.map
