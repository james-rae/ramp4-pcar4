import { bC as Q, bD as ae, bF as V, bG as ce, bK as N, bN as l, bT as a, bS as w, fC as o, bQ as t, ii as x, ij as ie, fD as C, iz as Le, iy as u, ls as ke, bE as ge, fA as me, bI as U, lt as b, kM as M, bJ as ye, bR as xe, bM as re, bO as B, bP as p, fK as W, i4 as oe, bU as le, bV as ne, iG as we, fI as v, lu as pe, i2 as Ce, fH as Me, lv as f, lw as Z, lx as de, bL as G, ix as Se, ly as O, kF as Te, lz as ze, lA as ee } from "./main-dhqBKhJU.js";
...
const Ee = ["type", "aria-label", "checked", "disabled", "content"], te = /* @__PURE__ */ Q({
  ...
  setup(e) {
    ...
    const I = (s) => !s.symbologyStack.some(
      (r) => r.visibility
    ), $ = () => {
      if (i.value instanceof Le)
        i.legendItem.toggleVisibility();
      else if (i.legendItem instanceof u) {
        if (I(i.legendItem) ? (i.legendItem.setSymbologyVisibility(void 0, !1), i.legendItem.setSymbologyVisibility(i.value.uid, !0), i.legendItem.toggleVisibility(!0)) : i.legendItem.setSymbologyVisibility(
          i.value.uid,
          !i.value.lastVisbility
        ), I(i.legendItem) && i.legendItem.toggleVisibility(!1), i.legendItem.layer?.supportsFeatures) {
          const s = i.legendItem.symbologyStack.filter((T) => T.lastVisbility === !0).map((T) => T.definitionClause || "");
          let r = "";
          s.length === 0 ? r = "1=2" : s.length < i.legendItem.symbologyStack.length && (r = s.join(" OR ")), i.legendItem.layer?.setSqlFilter(ke.SYMBOL, r);
        }
        m.value = !0;
      }
    };
    return (s, r) => {
      ...
      return l(), a("div", {
        class: "relative",
        onMouseover: r[3] || (r[3] = x(() => {
        }, ["stop"]))
      }, [
        w(o("input", {
          type: e.isRadio ? "radio" : "checkbox",
          "aria-label": t(y)(
            e.checked ? `legend.visibility.hide${e.label}` : `legend.visibility.show${e.label}`
          ),
          onClick: r[0] || (r[0] = x((D) => $(), ["stop"])),
          checked: e.checked && m.value,
          onKeypress: r[1] || (r[1] = ie(x(() => {
          }, ["prevent"]), ["enter"])),
          onKeyup: r[2] || (r[2] = ie(x((D) => $(), ["stop"]), ["enter"])),
          class: C([[
  ...
  setup(e) {
    const { t: y } = ae(), i = ge("iApi"), m = V(), I = V(!1), $ = me(), s = V($.mobileView), r = e, T = () => {
      r.legendItem.layerControlAvailable(b.Symbology) && r.legendItem.toggleSymbology();
    }, D = () => {
      r.legendItem.layerControlAvailable(b.Datatable) && A("grid") && i.event.emit(W.GRID_TOGGLE, r.legendItem.layer);
    }, q = () => {
      r.legendItem.layerControlAvailable(b.Settings) && A("settings") && i.event.emit(W.SETTINGS_TOGGLE, r.legendItem.layer);
    }, J = () => {
      ...
      r.legendItem.layerControlAvailable(b.Remove) && i.geo.map.removeLayer(r.legendItem.layerUid);
    }, P = () => {
      j.value && (M(r.legendItem.layer).reload(), m.value.open = !1);
    }, j = U(() => r.legendItem.layerControlAvailable(b.Reload) && r.legendItem instanceof u && M(r.legendItem.layer)?.canReload), Y = (z) => {
          ...
          default: re(() => [
            o("a", {
              ...
              class: C(["flex leading-snug items-center text-left w-auto", {
                disabled: !e.legendItem.layerControlAvailable(
                  t(b).Metadata
                ) || !A("metadata")
              }]),
              onClick: J,
...
}), De = /* @__PURE__ */ oe(Re, [["__scopeId", "data-v-67b6d0a9"]]), Pe = { key: 0 }, je = {
  key: 0,
  ...
  setup(e) {
    ...
    return ce(() => {
      y.legendItem.loadPromise.then(() => {
        y.legendItem.layerUid !== void 0 && Promise.all(
          M(y.legendItem.symbologyStack).map(
            (m) => m.drawPromise
          )
        ).then(() => {
      ...
      i.value.length > 1 ? (l(), a("div", je, [
        (l(!0), a(le, null, ne(i.value.slice(0, 3).reverse(), ($, s) => (l(), a("div", {
          class: C(["absolute", [
            s == 0 ? "symbol-0" : s == 1 ? "left-3 symbol-1" : "left-6 symbol-2"
          ]]),
...
}), Je = /* @__PURE__ */ oe(Qe, [["__scopeId", "data-v-1f1eb112"]]), Ye = { class: "relative" }, Xe = ["content"], _e = {
  key: 0,
  ...
  setup(e) {
    const y = pe(), i = me(), { t: m } = ae(), I = ge("iApi"), $ = V(), s = e, r = Ce(), T = r.multilineItems, D = (s.legendItem instanceof u && s.legendItem.maxLines) ?? r.maxLines, q = V(i.mobileView), J = U(() => y.layerConfigs), H = V([]), F = V(!1), P = V(!1), j = U(() => s.legendItem instanceof u ? M(s.legendItem.layer)?.layerType : ""), Y = U(() => s.legendItem instanceof u && M(s.legendItem.layer)?.canModifyLayer), A = U(() => {
      if (s.legendItem instanceof u) {
        const d = M(s.legendItem.layer);
    ...
    s.legendItem instanceof u && M(s.legendItem.layer)?.sublayers.length > 0), L = (d) => d === Z.Expand || d === Z.Visibility ? s.legendItem.controlAvailable(d) : s.legendItem.layerControlAvailable(
      d
    ), R = (d) => Ve(d), E = () => {
      s.legendItem.children.length === 0 || !L(Z.Expand) || (s.legendItem.toggleExpanded(), I.updateAlert(
        m(
          `legend.alert.group${s.legendItem.expanded ? "Expanded" : "Collapsed"}`
        )
      ));
    }, X = () => {
      if (L(b.Symbology)) {
        const d = s.legendItem.toggleSymbology();
        I.updateAlert(
          m(`legend.alert.symbology${d ? "Expanded" : "Collapsed"}`)
        );
      }
    }, ue = () => {
      L(b.Datatable) && K() && I.event.emit(
        W.GRID_TOGGLE,
        s.legendItem.layer
      );
    }, ve = (d) => {
      ...
      s.legendItem.reload(), setTimeout(() => {
        const d = s.legendItem;
        let n = !0;
        if (d.layer)
          M(d.layer).reload(), n = !1;
        else if (d.isSublayer && d.parentLayerId) {
          const h = I.geo.layer.getLayer(
            d.parentLayerId
          );
          h && (M(h).reload(), n = !1);
        }
        if (n) {
          const h = d.isSublayer ? d.parentLayerId : d.layerId, k = J.value.find(
            (S) => S.id === h
          );
          k !== void 0 && Ie(k);
        }
      ...
      const d = M(s.legendItem);
      let n = 0;
      if (d.type === f.Error) {
        s.legendItem.toggleHidden(!0);
        const h = setInterval(() => {
          const k = d.layer;
          (k && (k.layerExists || k.initiationState === ee.NEW || k.initiationState === ee.TERMINATING || k.initiationState === ee.TERMINATED) || n > 1200) && (clearInterval(h), k && I.geo.map.removeLayer(k), y.removeLayerConfig(d.layerId), I.fixture.get("legend")?.removeLayerItem(d.layerId)), n++;
        }, 250);
      } else {
        s.legendItem.error();
        const h = setInterval(() => {
          const k = d.parentLayerId || d.layerId, S = I.geo.layer.getLayer(k);
      ...
      s.legendItem.loadPromise.then(() => {
        ...
        Promise.all(
          M(
            s.legendItem.symbologyStack.map(
              (d) => d.drawPromise
            )
          )
        ).then(() => {
      ...
      return e.legendItem.hidden ? v("", !0) : (l(), a("div", {
        ...
        o("div", Ye, [
          w((l(), a("div", {
            ...
            content: c.value && L(t(Z).Expand) ? t(m)(
              e.legendItem.expanded ? "legend.group.collapse" : "legend.group.expand"
            ) : e.legendItem instanceof t(u) && e.legendItem.type === t(f).Item && L(t(b).Datatable) && K() ? t(m)("legend.layer.data") : "",
            "truncate-trigger": ""
          }, [
            e.legendItem.type !== t(f).Item ? (l(), a("div", _e, [
              e.legendItem.type === t(f).Placeholder ? (l(), a("svg", et, n[12] || (n[12] = [
                de('<path d="M0 0h24v24H0V0z" fill="none" data-v-66e5a8b6></path><path d="M0 0h24v24H0V0z" fill="none" data-v-66e5a8b6></path><circle cx="15.5" cy="9.5" r="1.5" data-v-66e5a8b6></circle><circle cx="8.5" cy="9.5" r="1.5" data-v-66e5a8b6></circle><circle cx="15.5" cy="9.5" r="1.5" data-v-66e5a8b6></circle><circle cx="8.5" cy="9.5" r="1.5" data-v-66e5a8b6></circle><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm0-2.5c2.33 0 4.32-1.45 5.12-3.5h-1.67c-.69 1.19-1.97 2-3.45 2s-2.75-.81-3.45-2H6.88c.8 2.05 2.79 3.5 5.12 3.5z" data-v-66e5a8b6></path>', 7)
              ]))) : (l(), a("svg", tt, n[13] || (n[13] = [
                de('<path d="M0 0h24v24H0V0z" fill="none" data-v-66e5a8b6></path><path d="M0 0h24v24H0V0z" fill="none" data-v-66e5a8b6></path><circle cx="15.5" cy="9.5" r="1.5" data-v-66e5a8b6></circle><circle cx="8.5" cy="9.5" r="1.5" data-v-66e5a8b6></circle><circle cx="15.5" cy="9.5" r="1.5" data-v-66e5a8b6></circle><circle cx="8.5" cy="9.5" r="1.5" data-v-66e5a8b6></circle><path d="M 20,12C 20,7.6 16.4,4 12,4C 7.6,4 4,7.6 4,12C 4,16.4 7.6,20 12,20C 16.4,20 20,16.4 20,12 Z M 22,12C 22,17.5 17.5,22 12,22C 6.5,22 2,17.5 2,12C 2,6.5 6.5,2.00001 12,2.00001C 17.5,2.00001 22,6.5 22,12 Z M 15.5,8C 16.3,8 17,8.7 17,9.5C 17,10.3 16.3,11 15.5,11C 14.7,11 14,10.3 14,9.5C 14,8.7 14.7,8 15.5,8 Z M 10,9.5C 10,10.3 9.3,11 8.5,11C 7.7,11 7,10.3 7,9.5C 7,8.7 7.7,8 8.5,8C 9.3,8 10,8.7 10,9.5 Z M 12,14C 13.7524,14 15.2943,14.7212 16.1871,15.8129L 14.7697,17.2302C 14.3175,16.5078 13.2477,16 12,16C 10.7523,16 9.68254,16.5078 9.23024,17.2302L 7.81291,15.8129C 8.7057,14.7212 10.2476,14 12,14 Z" data-v-66e5a8b6></path>', 7)
              ])))
            ])) : v("", !0),
            e.legendItem.type === t(f).Item && e.legendItem instanceof t(u) && e.legendItem.layer.legend.length > 0 ? (l(), a("div", {
              ...
              w((l(), a("button", {
                ...
                onClick: x(X, ["stop"]),
                class: C([
                  L(t(b).Symbology) ? "cursor-pointer" : "cursor-default"
                ]),
            ...
            e.legendItem instanceof t(u) && t(T) ? w((l(), a("div", {
              ...
              e.legendItem.infoType === t(O).Title && e.legendItem.content ? (l(), a("h3", it, p(e.legendItem.content), 1)) : e.legendItem.infoType === t(O).Title ? (l(), a("h3", rt, p(e.legendItem.name), 1)) : e.legendItem.infoType === t(O).Text ? (l(), a("p", dt, p(e.legendItem.content), 1)) : e.legendItem.infoType === t(O).Image ? (l(), a("img", {
                key: 3,
            ...
            e.legendItem.type === t(f).Item && e.legendItem instanceof t(u) && e.legendItem.layerOffscale ? (l(), a("div", kt, [
              ...
              legendItem: e.legendItem,
              disabled: e.legendItem instanceof t(u) && !e.legendItem.layerControlAvailable(
                t(b).Visibility
              ),
...
}), Ft = /* @__PURE__ */ oe(Ot, [["__scopeId", "data-v-66e5a8b6"]]);
export {
  Ft as default
};
//# sourceMappingURL=item-CzbHHnJN.js.map
