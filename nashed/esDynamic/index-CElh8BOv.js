import { kg as I, iB as O, kh as d, ha as E, ki as P, iE as k, kj as F, ik as L, kk as R, bx as T, bC as z, bD as H, bE as N, bF as f, bI as v, ip as x, bG as D, fK as m, fF as j, bH as W, bK as q, bT as y, fC as c, fD as K, bS as X, bQ as S, iG as G, bN as g, i4 as Q } from "./main-dhqBKhJU.js";
class U extends I {
  ...
  async addMapGraphicLayer() {
    ...
    const e = new k(
      ...
      "overview-graphic"
    ), a = this.overviewmapStore.borderColour ?? "#FF0000", i = this.overviewmapStore.borderWidth ?? 1, t = this.overviewmapStore.areaColour ?? "#000000", s = this.overviewmapStore.areaOpacity ?? 0.25, n = `${t}${Math.round(s * 255).toString(
      16
    )}`;
    e.style = new F({
      fill: { colour: n },
   ...
   */
  findBasemap(e) {
    const a = this._basemapStore.find(
      (i) => i.id === e
    );
    if (a)
      return a;
  ...
  async mapDrag(e) {
    if (e.native.pointerType === "mouse") {
      if (e.action === "start")
        ...
        this.overviewGraphicLayer.getEsriGraphic(
          "overview-graphic"
        ).geometry = t, e.action === "end" && (this.$iApi.geo.map.zoomMapTo(
          this.$iApi.geo.geom.geomEsriToRamp(t),
          void 0,
          !1
   ...
   */
  updateOverview(e) {
    const a = this.overviewmapStore.expandFactor, i = this.zoomMapTo(
      e.expand(a),
      void 0,
      !1
    ), t = this.overviewGraphicLayer.getLocalGraphic("overview-graphic");
...
const Z = { class: "relative h-full w-full overflow-hidden" }, J = { class: "absolute h-30 w-30 top-0 right-0" }, ee = ["content", "aria-label"], te = /* @__PURE__ */ z({
  __name: "overviewmap",
  setup(u) {
    const e = d(), { t: a } = H(), i = N("iApi"), t = L(), s = f(), n = v(
      () => t.activeBasemapConfig
    ), _ = v(() => e.mapConfig), b = v(
      () => e.basemaps
    ), A = v(() => e.startMinimized);
    let r = x(new U(i));
    ...
    D(() => {
      i.geo.map.viewPromise.then(async () => {
        w(), r.createMap(
          _.value,
          s.value.querySelector(".overviewmap")
        ), await r.viewPromise, await r.addMapGraphicLayer(), p.value = A.value;
        let h = r.updateOverview(
          i.geo.map.getExtent()
        );
        l.push(
          i.event.on(
      ...
      l.forEach((h) => i.event.off(h)), r.destroyMap();
    });
    const $ = async (h) => {
      ...
      try {
        ...
        r.created || e.updateInitialBasemap(o.id), r.created && r.viewPromise.then(
          () => r.setBasemap(o.id)
        );
      } catch {
        r.created || e.updateInitialBasemap(n.value.id), r.viewPromise.then(
          () => r.setBasemap(n.value.id)
        );
      }
    };
    return (h, o) => {
        ...
        c("div", {
          ...
          c("div", J, [
            X((g(), y("button", {
              ...
              content: S(a)(
                p.value ? "overviewmap.expand" : "overviewmap.minimize"
              ),
              "aria-label": S(a)(
                p.value ? "overviewmap.expand" : "overviewmap.minimize"
              )
...
}), ae = /* @__PURE__ */ Q(te, [["__scopeId", "data-v-7ca7037e"]]), ie = { en: { "overviewmap.expand": "Expand Overview", "overviewmap.minimize": "Minimize Overview" }, fr: { "overviewmap.expand": "Développer l'aperçu", "overviewmap.minimize": "Réduire l'aperçu" } };
class oe extends Y {
  added() {
    Object.entries(ie).forEach(
      (s) => this.$iApi.$i18n.mergeLocaleMessage(...s)
...
//# sourceMappingURL=index-CElh8BOv.js.map
