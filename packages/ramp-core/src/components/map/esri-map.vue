...
import GeoJsonLayer from 'ramp-geoapi/dist/layer/GeoJsonLayer';
...
export default class EsriMap extends Vue {
    ...
    onMapConfigChange(newValue: RampMapConfig, oldValue: RampMapConfig) {
        ...
        const happyLayer: GeoJsonLayer = (window as any).RAMP.geoapi.layers.createGeoJSONLayer(rampHappyLayerConfig , happy, systemMagic);
        this.map.addLayer(happyLayer);
        happyLayer.isLayerLoaded().then(() => {
            // test some filters
            happyLayer.setSqlFilter('mytest', `name = 'Right Eye'`);
            happyLayer.applySqlFilter();
        });
    }
}
</script>
