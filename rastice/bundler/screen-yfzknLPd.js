...
import { h as A, a as te, G as oe, _ as re, J as se, K as ne } from "./main-CggyT1N6.js";
...
const ae = { class: "rv-geosearch-bar relative h-26 mx-8 mb-8" }, le = ["placeholder", "value", "aria-label"], ie = { class: "absolute inset-y-0 right-8 grid w-10 place-content-center" }, ce = ["aria-label", "content"], pe = /* @__PURE__ */ V({
  ...
  disabled: "",
  hidden: ""
}, me = { class: "sm:w-1/2 h-26 sm:mx-16 flex" }, he = ["value", "aria-label"], be = {
  value: "",
  disabled: "",
  hidden: ""
  ...
  __name: "top-filters",
  setup(T) {
    const { t: s } = F(), p = q("iApi"), c = A(), g = C(), n = C(), l = C(), b = C([]), u = C([]), o = C([]), e = w(() => c.queryParams), d = w(() => p.language), L = (h) => c.setProvince(h), $ = (h) => c.setType(h), B = () => {
      L({}), $({});
    }, P = () => {
      c.initService(p.language, p.fixture.get("geosearch").config);
      const h = b.value.find((m) => e.value.province === m.name)?.code, f = u.value.find((m) => e.value.type === m.name)?.code;
      c.getProvinces.then((m) => {
        b.value = m, L({
          province: m.find((E) => E.code === h)?.name,
          forceReRun: !0
        });
      }), c.getTypes.then((m) => {
        u.value = m, $({
          type: m.find((E) => E.code === f)?.name,
          forceReRun: !0
        });
      });
    }, v = () => {
    ...
    return Q(() => {
      ...
      o.value.forEach((h) => h()), n.value?.removeEventListener("blur", v), n.value?.removeEventListener("keyup", j), l.value?.removeEventListener("blur", M), l.value?.removeEventListener("keyup", I);
    }), (h, f) => {
      const m = R("truncate"), E = R("tippy");
      return t(), r("div", {
