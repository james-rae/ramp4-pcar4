import { bQ as a, bR as o, bS as m, ew as v, Q as R, G as O, T as _, jZ as S, j_ as q, j$ as w, X as p, ds as F, fP as M, ic as V, id as g, gL as I } from "./main-DZxT3i7S.js";
const u = -1;
let c = class extends v {
  constructor(e) {
    super(e), this._from = null, this._to = null, this._final = null, this._current = [], this._time = 0, this.duration = R("mapview-transitions-duration"), this.effects = [];
  }
  set effect(e) {
    if (this._get("effect") !== (e = e || "")) {
      this._set("effect", e);
      try {
        this._transitionTo(b(e));
      } catch (t) {
        this._transitionTo([]), O.getLogger(this).warn("Invalid Effect", { effect: e, error: t });
      }
    }
  }
  get final() {
  ...
  canTransitionTo(e) {
    try {
      return this.scale > 0 && y(this._current, b(e), this.scale);
    } catch {
      return !1;
    }
  }
  ...
  _transitionTo(e) {
    this.scale > 0 && y(this._current, e, this.scale) ? (this._final = e, this._to = _(e), C(this._current, this._to, this.scale), this._from = _(this._current), this._time = 0) : (this._from = this._to = this._final = null, this._current = e), this._set("effects", this._current[0] ? _(this._current[0].effects) : []);
  }
  _applyTimeTransition(e) {
    if (!(this._to && this._from && this._current && this._final)) return;
    this._time += e;
    const t = Math.min(1, this._time / this.duration);
    for (let i = 0; i < this._current.length; i++) {
      const s = this._current[i], n = this._from[i], l = this._to[i];
      s.scale = E(n.scale, l.scale, t);
      for (let r = 0; r < s.effects.length; r++) {
        const f = s.effects[r], h = n.effects[r], T = l.effects[r];
        f.interpolate(h, T, t);
      }
    }
  ...
  _updateForScale(e) {
    if (this._set("scale", e), this._current.length === 0) return;
    const t = this._current, i = this._current.length - 1;
    let s, n, l = 1;
    if (t.length === 1 || e >= t[0].scale) n = s = t[0].effects;
    else if (e <= t[i].scale) n = s = t[i].effects;
    else for (let r = 0; r < i; r++) {
      const f = t[r], h = t[r + 1];
      if (f.scale >= e && h.scale <= e) {
        l = (e - f.scale) / (h.scale - f.scale), s = f.effects, n = h.effects;
        break;
      }
    }
    for (let r = 0; r < this.effects.length; r++)
      this.effects[r].interpolate(s[r], n[r], l);
  }
};
function b(e) {
  const t = S(e) || [];
  return $(t) ? [{ scale: u, effects: t }] : t;
}
function y(e, t, i) {
  return !e[0]?.effects || !t[0]?.effects ? !0 : !((e[0]?.scale === u || t[0]?.scale === u) && (e.length > 1 || t.length > 1) && i <= 0) && q(e[0].effects, t[0].effects);
}
function C(e, t, i) {
  const s = e.length > t.length ? e : t, n = e.length > t.length ? t : e, l = n[n.length - 1], r = l?.scale ?? i, f = l?.effects ?? [];
  for (let h = n.length; h < s.length; h++) n.push({ scale: r, effects: [...f] });
  for (let h = 0; h < s.length; h++) n[h].scale = n[h].scale === u ? i : n[h].scale, s[h].scale = s[h].scale === u ? i : s[h].scale, w(n[h].effects, s[h].effects);
}
function E(e, t, i) {
  return e + (t - e) * i;
}
...
let d = class extends v {
  ...
  fadeIn() {
    return this._fadeInResolver || (this.opacity = 1, this.computedOpacity = 0, this._fadeOutResolver?.(), this._fadeOutResolver = null, this._fadeInResolver = p()), this._fadeInResolver.promise;
  }
  fadeOut() {
    return this._fadeOutResolver || (this.opacity = 0, this._fadeInResolver?.(), this._fadeInResolver = null, this._fadeOutResolver = p()), this._fadeOutResolver.promise;
  }
  transitionStep(e, t) {
    const i = R("mapview-transitions-duration"), s = i ? 1 / i : 0;
    if (s === 0) this.computedOpacity = this.opacity, this.computedVisible = this.visible;
    else {
      const n = this._fadeOutResolver || !this.visible ? 0 : this.opacity, l = this.computedOpacity;
      if (l === n) this.computedVisible = this.visible;
      else {
        const r = e * s;
        this.computedOpacity = l > n ? Math.max(n, l - r) : Math.min(n, l + r), this.computedVisible = this.computedOpacity > 0;
      }
    }
    this.transitioning || (this._fadeInResolver?.(), this._fadeOutResolver?.(), this._fadeOutResolver = this._fadeInResolver = null);
  }
};
a([o()], d.prototype, "computedOpacity", void 0), a([o()], d.prototype, "computedVisible", void 0), a([o()], d.prototype, "opacity", void 0), a([o()], d.prototype, "visible", void 0), a([o()], d.prototype, "transitioning", null), a([o()], d.prototype, "_fadeOutResolver", void 0), a([o()], d.prototype, "_fadeInResolver", void 0), d = a([m("esri.views.2d.engine.transitions.FadeTransition")], d);
class A extends F {
  constructor() {
    super(...arguments), this._transitionables = null, this._clips = null, this._fadeTransition = null, this._isReady = !1, this._opacity = 1, this.parent = null, this._stage = null, this._visible = !0;
  }
  get computedOpacity() {
  ...
  get fadeTransitionEnabled() {
    return this._fadeTransition !== null;
  }
  set fadeTransitionEnabled(t) {
    !this._fadeTransition && t ? (this._fadeTransition = new d({ opacity: this.opacity, visible: this.visible }), this.addTransitionable(this._fadeTransition)) : this._fadeTransition && !t && (this.removeTransitionable(this._fadeTransition), this._fadeTransition = null);
  }
  ...
  removeTransitionable(t) {
    t.endTransition(), this._transitionables && M(this._transitionables, t), this.requestRender();
  }
  fadeIn() {
...
let G = class extends A {
  constructor() {
    super(...arguments), this._childrenSet = /* @__PURE__ */ new Set(), this._needsSort = !1, this._children = [], this._childrenObservable = new V(), this._effectView = null, this._highlightGradient = null;
  }
  get blendMode() {
  ...
  get children() {
    return g(this._childrenObservable), this._children;
  }
  get clips() {
    return this._clips;
  }
  ...
  set effect(t) {
    (this._effectView || t) && (this._effectView || (this._effectView = new c(), this.addTransitionable(this._effectView)), this._effectView.effect = t, this.requestRender());
  }
  get highlightGradient() {
    return this._highlightGradient;
  }
  set highlightGradient(t) {
    this._highlightGradient = t, this.requestRender();
  }
  get hasBlending() {
  ...
  get isReady() {
    return this.children.every((t) => t.isReady);
  }
  get sortFunction() {
    return this._sortFunction;
  }
  set sortFunction(t) {
    this._sortFunction = t, t && (this._needsSort = !0);
  }
  doRender(t) {
  ...
  addChildAt(t, i = this.children.length) {
    ...
    this._needsSort = !0;
    const s = t.parent;
    return s && s !== this && s.removeChild(t), i >= this.children.length ? this.children.push(t) : this.children.splice(i, 0, t), this._childrenSet.add(t), t.parent = this, t.stage = this.stage, this !== this.stage && (t.clips = this.clips), this.requestRender(), this._childrenObservable.notify(), t;
  }
  contains(t) {
    return g(this._childrenObservable), this._childrenSet.has(t);
  }
  removeAllChildren() {
  ...
  removeChildAt(t) {
    ...
    return this._childrenSet.delete(i), this !== this.stage && (i.clips = null), i.stage = null, i.parent = null, this._childrenObservable.notify(), i;
  }
  beforeRender(t) {
    super.beforeRender(t), this.sortFunction && this._needsSort && (this.children.sort(this.sortFunction), this._needsSort = !1, this._childrenObservable.notify());
  ...
  _createTransforms() {
    return { displayViewScreenMat3: I() };
  }
  onAttach() {
    super.onAttach();
    const t = this.stage;
  ...
  isTransitioning() {
    return super.isTransitioning() || this.children.some((t) => t.transitioning);
  }
};
export {
  c as a,
...
//# sourceMappingURL=Container-C7h2dwkX.js.map
