import { dL as I, dC as D, s as F, dE as V, cu as j, G as P, I as R, U as L, dz as _, cn as E, dB as A, a5 as G, a7 as Q, ik as M, il as J, b1 as Z, im as U, dw as Y, dx as B, dD as z, dy as H, r as K } from "./main-DZxT3i7S.js";
import { e as W, n as X } from "./date-Cqvy-TgA.js";
import { m as ee } from "./FeatureStore-zvkXSx1l.js";
import { V as te } from "./QueryEngine-DVmSGTfj.js";
import { c as ie } from "./number-CzPiexfI.js";
import { a as ne, u as se } from "./clientSideDefaults-ppWQ35EG.js";
function re(r, i, e) {
  i = I(i)?.toLowerCase(), e = I(e)?.toLowerCase();
  const t = r.map((o) => o.toLowerCase()), n = i ? r[t.indexOf(i)] : null, a = e ? r[t.indexOf(e)] : null;
  return { longitudeFieldName: n || r[t.indexOf(ae.find((o) => t.includes(o)))], latitudeFieldName: a || r[t.indexOf(oe.find((o) => t.includes(o)))] };
}
const oe = ["lat", "lat83", "latitude", "latitude83", "latdecdeg", "lat_dd", "y", "ycenter", "point_y"], ae = ["lon", "lng", "lng83", "long", "long83", "longitude", "longitude83", "longdecdeg", "long_dd", "x", "xcenter", "point_x"], T = /^\s*"([\S\s]*)"\s*$/, b = /""/g, O = `
`, le = [",", " ", ";", "|", "	"];
function* S(r, i, e) {
  let t = 0;
  for (; t <= r.length; ) {
    const n = r.indexOf(i, t), a = r.slice(t, n > -1 ? n : void 0);
...
function ce(r, i, e) {
  r = r.trim(), i = i?.trim();
  const t = [], n = Array.from(/* @__PURE__ */ new Set([e?.delimiter, ...le])).filter((o) => o != null);
  for (const o of n) {
    const l = N(r, o).length, c = N(i, o).length ?? l;
    l > 1 && t.push({ weight: Math.min(l, c), delimiter: o });
  }
  const a = t.sort(({ weight: o }, { weight: l }) => l - o).map(({ delimiter: o }) => o);
  for (const o of a) {
    const l = v(r, o).names, c = re(l, e?.longitudeField, e?.latitudeField);
...
function* C(r, i, e, t = () => /* @__PURE__ */ Object.create(null)) {
  ...
  e: for (const y of n) {
    const m = q(y, e);
    for (const p of m) if (a += o + p, o = "", l += k(p), l % 2 == 0) {
      if (l > 0) {
        const h = T.exec(a);
        if (!h) {
          c = t(), d = 0, a = "", l = 0;
        ...
        c[i[d]] = h[1].replaceAll(b, '"'), d++;
      } else c[i[d]] = a, d++;
      a = "", l = 0;
    } else o = e;
    l === 0 ? (yield c, c = t(), d = 0) : o = O;
  }
}
function v(r, i) {
  const e = N(r, i).filter((n) => n != null), t = e.map((n) => I(n));
...
function ue(r, i, e, t, n) {
  ...
  for (let c = 0; c < e.length; c++) {
    const d = e[c], y = t[c];
    if (d === n.longitudeFieldName || d === n.latitudeFieldName) a.push({ name: d, type: "esriFieldTypeDouble", alias: y });
    else {
      let m;
      switch (de(l.map((p) => p[d]))) {
        case "integer":
      ...
      a.push({ name: d, type: m, alias: y, length: D(m) });
    }
  }
  return a;
}
...
const me = se("esriGeometryPoint"), pe = ["csv"], he = [0, 0];
class ye {
  constructor(i, e) {
    this.x = i, this.y = e;
  }
...
class Ee {
  ...
  async load(i, e = {}) {
    this._loadOptions = i;
    const [t] = await Promise.all([this._fetch(e.signal), this._checkProjection(i?.parsingOptions?.spatialReference)]), n = ge(t, i);
    ...
    if (n.layerDefinition.extent = o, l) {
      ...
      n.layerDefinition.timeInfo.timeExtent = [c, d];
    }
    return n;
  }
  async applyEdits() {
  ...
  async refresh(i) {
    this._loadOptions.customParameters = i, this._snapshotTask?.abort(), this._snapshotTask = V(this._snapshotFeatures), this._snapshotTask.promise.then((n) => {
      this._queryEngine.featureStore.clear(), n && this._queryEngine.featureStore.addMany(n);
    }, (n) => {
      this._queryEngine.featureStore.clear(), j(n) || P.getLogger("esri.layers.CSVLayer").error(new F("csv-layer:refresh", "An error occurred during refresh", { error: n }));
  ...
  _createQueryEngine(i) {
    const { objectIdField: e, fields: t, extent: n, timeInfo: a } = i.layerDefinition, o = new ee({ geometryType: "esriGeometryPoint", hasM: !1, hasZ: !1 });
    return new te({ fieldsIndex: _.fromLayerJSON({ fields: t, dateFieldsTimeReference: { timeZoneIANA: E } }), geometryType: "esriGeometryPoint", hasM: !1, hasZ: !1, timeInfo: a, objectIdField: e, spatialReference: n.spatialReference || { wkid: 4326 }, featureStore: o });
  }
  async _createFeatures(i) {
    ...
    let l = [];
    const c = [], d = a.fields.filter((s) => s.name !== n).map((s) => s.name);
    let y = 0;
    const m = {};
    ...
    const p = C(i, d, this._delimiter, ne(m, n));
    for (const s of p) {
      const u = this._parseCoordinateValue(s[e]), g = this._parseCoordinateValue(s[t]);
      if (g != null && u != null && !isNaN(u) && !isNaN(g)) {
        s[e] = u, s[t] = g;
        for (const f in s) if (f !== e && f !== t) if (a.isDateField(f)) s[f] = X(s[f]);
        else if (a.isNumericField(f)) {
          const x = w(s[f]);
          isNaN(x) ? s[f] = null : s[f] = x;
        } else s[f] != null && (s[f] = fe(s[f]));
        s[n] = y, y++, l.push(new ye(g, u)), c.push(s);
      }
    }
    if (!G({ wkid: 4326 }, o)) if (Q(o)) for (const s of l) [s.x, s.y] = M(s.x, s.y, he);
    else l = J(U, l, Z.WGS84, o, null, null);
    const h = [];
    for (let s = 0; s < l.length; s++) {
      const { x: u, y: g } = l[s], f = c[s];
      f[n] = s + 1, h.push(new Y(new B([], [u, g]), f, null, f[n]));
    }
    return h;
  }
...
function ge(r, i) {
  ...
  if (!d) throw new F("csv-layer:location-fields-not-found", "Unable to identify latitude and longitude fields from the CSV file", { firstLine: o, secondLine: l, parsingOptions: e });
  t.locationInfo = d, t.delimiter = c;
  const { names: y, aliases: m } = v(o, c), p = ue(r, t.delimiter, y, m, t.locationInfo);
  if (e.fields?.length) {
    const s = new _(e.fields);
    for (const u of p) {
      const g = s.get(u.name);
      g && Object.assign(u, g);
    }
  }
  ...
  if (t.locationInfo && (t.locationInfo.latitudeFieldName = h.get(t.locationInfo.latitudeFieldName).name, t.locationInfo.longitudeFieldName = h.get(t.locationInfo.longitudeFieldName).name), n.timeInfo) {
    const s = n.timeInfo;
    if (s.startTimeField) {
      const u = h.get(s.startTimeField);
      u ? (s.startTimeField = u.name, u.type = "esriFieldTypeDate") : s.startTimeField = null;
    }
    if (s.endTimeField) {
      const u = h.get(s.endTimeField);
      u ? (s.endTimeField = u.name, u.type = "esriFieldTypeDate") : s.endTimeField = null;
    }
    if (s.trackIdField) {
      const u = h.get(s.trackIdField);
      s.trackIdField = u ? u.name : null;
    }
...
//# sourceMappingURL=CSVSourceWorker-DA19qU2p.js.map
