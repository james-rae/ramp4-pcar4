import { a5 as h, cq as w, cr as m, cs as g, ct as I, cv as f, cu as d, G as S, bQ as p, bR as T, bS as V } from "./main-DZxT3i7S.js";
import { r as v, n as _ } from "./imageUtils-23ndbz6A.js";
import { j as x, y as q } from "./LayerView-BSi2ZKlw.js";
import { i as M } from "./RefreshableLayerView-hfJGJOy7.js";
const R = [0, 0];
let s = class extends M(v(x(q))) {
  constructor() {
  ...
  get tileMatrixSet() {
    ...
    if (t && h(t.tileInfo?.spatialReference, this.view.spatialReference)) return t;
  ...
  attach() {
    ...
    e && (this._tileInfoView = new w(e), this._fetchQueue = new m({ tileInfoView: this._tileInfoView, concurrency: 16, process: (t, i) => this.fetchTile(t, i), scheduler: this.scheduler, priority: g.MAPVIEW_FETCH_QUEUE }), this._tileStrategy = new I({ cachePolicy: "keep", resampling: !0, acquireTile: (t) => this.acquireTile(t), releaseTile: (t) => this.releaseTile(t), tileInfoView: this._tileInfoView }), this.addAttachHandles(this._updatingHandles.add(() => [this.layer?.activeLayer?.styleId, this.tileMatrixSet], () => this.doRefresh())), super.attach());
  }
  detach() {
  ...
  supportsSpatialReference(e) {
    return this.layer.activeLayer.tileMatrixSets?.some((t) => h(t.tileInfo?.spatialReference, e)) ?? !1;
  }
  async doRefresh() {
    if (this.attached) {
  ...
  async fetchTile(e, t = {}) {
    const i = "tilemapCache" in this.layer ? this.layer.tilemapCache : null, { signal: a, resamplingLevel: n = 0 } = t;
    if (!i) return this._fetchImage(e, a);
    const r = new f(0, 0, 0, 0);
    let c;
    try {
      await i.fetchAvailabilityUpsample(e.level, e.row, e.col, r, { signal: a }), c = await this._fetchImage(r, a);
    } catch (l) {
      if (d(l)) throw l;
      if (n < 3) {
        const u = this._tileInfoView.getTileParentId(e.id);
        if (u) {
          const o = new f(u), y = await this.fetchTile(o, { ...t, resamplingLevel: n + 1 });
          return _(this._tileInfoView, y, o, e);
        }
      }
      throw l;
    }
    return _(this._tileInfoView, c, r, e);
  }
  canResume() {
  ...
  _getTileMatrixSetBySpatialReference(e) {
    return e.tileMatrixSets?.find((t) => h(t.tileInfo?.spatialReference, this.view.spatialReference));
  }
};
p([T({ readOnly: !0 })], s.prototype, "tileMatrixSet", null), s = p([V("esri.views.2d.layers.WMTSLayerView2D")], s);
const L = s;
export {
  L as default
};
//# sourceMappingURL=WMTSLayerView2D-D-kbFtbk.js.map
