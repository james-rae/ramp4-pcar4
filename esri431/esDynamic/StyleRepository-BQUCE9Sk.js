import { a as b, f as V, l as X } from "./StyleDefinition-D9H4pBJh.js";
import { aH as g, h6 as T, of as qt, og as Kt, oh as Wt, oi as me, g2 as we, ea as Ee } from "./main-DZxT3i7S.js";
import { L as W } from "./enums-qHpGJ28Q.js";
import { p as A, i as vt, b as Pt, I as be, N as re, P as Te } from "./GeometryUtils-C-fTcTXr.js";
...
let j = class Dt {
  ...
  setDataUniforms(t, e, r, a, i) {
    ...
    for (const s of o) {
      const { name: l, type: u, getValue: h } = s, p = h(r, e, a, i);
      if (p !== null) switch (u) {
        case "float":
          t.setUniform1f(l, p);
          break;
  ...
  encodeAttributes(t, e, r, a) {
    const i = this.attributesInfo(), o = this.getEncodingInfos(), s = [];
    let l = 0, u = 0;
    for (const h of Object.keys(o)) {
      ...
      let M = 0;
      for (const I of p) {
        const { offset: _, bufferElementsToAdd: U } = I;
        if (U > 0) {
          for (let H = 0; H < U; H++) s.push(0);
          l += u, u = I.bufferElementsToAdd;
        }
        const E = a ?? $.getValue(z ? z[M] : e, t);
        switch (f) {
          case c.R8_SIGNED:
          case c.R8_UNSIGNED:
            s[l] |= this._encodeByte(E * (d || 1), 8 * _);
            break;
          case c.R16_SIGNED:
          case c.R16_UNSIGNED:
            s[l] |= this._encodeShort(E * (d || 1), 8 * _);
            break;
          case c.R8G8_SIGNED:
          case c.R8G8_UNSIGNED:
            s[l] |= this._encodeByte(E * (d || 1), 8 * _), s[l] |= this._encodeByte(E * (d || 1), 8 * _ + 8);
            break;
          case c.R16G16_SIGNED:
          case c.R16G16_UNSIGNED:
            s[l] |= this._encodeShort(E * (d || 1), 8 * _), s[l] |= this._encodeShort(E * (d || 1), 8 * _ + 16);
            break;
          case c.R8G8B8A8_SIGNED:
          case c.R8G8B8A8_UNSIGNED:
            s[l] |= this._encodeByte(E * (d || 1), 8 * _), s[l] |= this._encodeByte(E * (d || 1), 8 * _ + 8), s[l] |= this._encodeByte(E * (d || 1), 8 * _ + 16), s[l] |= this._encodeByte(E * (d || 1), 8 * _ + 24);
            break;
          case c.R8G8B8A8_COLOR:
            s[l] = this._encodeColor(E);
            break;
          case c.R16G16B16A16_DASHARRAY:
          case c.R16G16B16A16_PATTERN:
            this._encodePattern(l, s, E);
            break;
          default:
  ...
  _buildAttributesInfo() {
    ...
    for (const p of o) {
      ...
      const $ = d.type;
      for (let z = 0; z < f; z++) {
        const { dataType: M, bytesPerElement: I, count: _, normalized: U } = Dt._encodingInfo[$], E = I * _, H = `${M}-${U === !0}`;
        let R = r[H], O = 0;
        if (!R || R.count + _ > 4) a++, R = { dataIndex: a, count: 0, offset: 0 }, _ !== 4 && (r[H] = R), t.push({ location: -1, name: "a_data_" + a, count: _, type: M, normalized: U }), O = Math.ceil(Math.max(E / 4, 1));
          ...
          B.count += _, O = Math.ceil(Math.max(B.count * I / 4, 1)) - Math.ceil(Math.max(R.offset / 4, 1));
        }
    ...
    let l = 0;
    const u = this._layout.get("geometry");
    for (const p of u) this._locations.set(p.name, l++);
    const h = this._layout.get("opacity");
  ...
  _buildVertexBufferLayout(t) {
    const e = /* @__PURE__ */ new Map(), r = this.geometryInfo();
    let a = r[0].stride;
    if (t.length === 0) e.set("geometry", r);
    else {
      const o = [];
      let s = a;
      for (const l of t) a += Zt(l.type) * l.count;
      for (const l of r) o.push(new v(l.name, l.count, l.type, l.offset, a, l.normalized));
      for (const l of t) o.push(new v(l.name, l.count, l.type, s, a, l.normalized)), s += Zt(l.type) * l.count;
      e.set("geometry", o);
    }
    const i = this.opacityInfo();
    i && e.set("opacity", i), this._layout = e, this._stride = a;
  }
...
`;
    ...
    for (const u of o) {
      l++;
      const { name: h, type: p, precisionFactor: f, isLayout: d } = s[u], L = f && f !== 1 ? " * " + 1 / f : "", { bytesPerElement: $, count: z } = Dt._encodingInfo[p], M = (I) => `a_data_${I.dataIndex}${ve(z, I.offset, $)}`;
      switch (this.getAtributeState(l)) {
        case q.UNIFORM:
          {
            const I = this._getType(z), _ = `u_${h}`;
            i.push({ name: _, type: I, getValue: (U, E, H, R) => {
...
`;
        ...
        case q.DATA_DRIVEN:
          {
            const I = M(e[u][0]);
            a += `${this._getType(z)} ${h} = ${I}${L};
`;
          }
          break;
        case q.INTERPOLATED_DATA_DRIVEN: {
          const I = `u_t_${h}`;
          i.push({ name: I, type: "float", getValue: (E, H, R, O) => (d ? E.getLayoutProperty(u) : E.getPaintProperty(u)).interpolator.interpolationUniformValue(R, H) }), r += `uniform float ${I};
`;
          const _ = M(e[u][0]), U = M(e[u][1]);
          a += `${this._getType(z)} ${h} = mix(${_}${L}, ${U}${L}, ${I});
`;
        }
      }
    }
  ...
  _encodeColor(t) {
    const e = 255 * t[3];
    return Ie.i8888to32(t[0] * e, t[1] * e, t[2] * e, e);
  }
  _encodePattern(t, e, r) {
    if (!r?.rect) return;
    const a = 2, i = r.rect, o = r.width, s = r.height;
    e[t] = this._encodeShort(i.x + a, 0), e[t] |= this._encodeShort(i.y + a + s, 16), e[t + 1] = this._encodeShort(i.x + a + o, 0), e[t + 1] |= this._encodeShort(i.y + a, 16);
  }
  _encodeByte(t, e) {
    return (255 & t) << e;
  }
...
const ne = { kind: "null" }, w = { kind: "number" }, S = { kind: "string" }, D = { kind: "boolean" }, Y = { kind: "color" }, Tt = { kind: "object" }, C = { kind: "value" };
function ht(n, t) {
  return { kind: "array", itemType: n, n: t };
}
const De = [ne, w, S, D, Y, Tt, ht(C)];
function rt(n) {
  if (n.kind === "array") {
...
function It(n) {
  ...
  if (typeof n == "boolean") return D;
  if (typeof n == "number") return w;
  if (n instanceof T) return Y;
  if (Array.isArray(n)) {
    let t;
    for (const e of n) {
      const r = It(e);
    ...
    return ht(t || C, n.length);
  }
  return typeof n == "object" ? Tt : C;
}
function xt(n, t) {
  if (t.kind === "array") return n.kind === "array" && (n.n === 0 && n.itemType.kind === "value" || xt(n.itemType, t.itemType)) && (typeof t.n != "number" || t.n === n.n);
  if (t.kind === "value") {
    for (const e of De) if (xt(n, e)) return !0;
  }
...
function Nt(n) {
  if (n === null) return "";
  const t = typeof n;
  return t === "string" ? n : t === "number" || t === "boolean" ? String(n) : n instanceof T ? n.toString() : JSON.stringify(n);
}
let Re = class {
  constructor(t) {
  ...
  static parse(t, e) {
    ...
    for (let l = 2; l < t.length - 1; l += 2) {
      ...
      for (const p of u) {
        ...
        if (s) {
          if (f !== s) throw new Error('"match" requires labels to have the same type');
        } else s = f;
        o[p] = i.length;
      }
      const h = y(t[l + 1], e);
...
class k {
  ...
  static parse(t, e, r) {
    ...
    const o = y(t[2], e, w);
    let s;
    a === "interpolate-hcl" || a === "interpolate-lab" ? s = Y : r && r.kind !== "value" && (s = r);
    const l = [];
    for (let u = 3; u < t.length; u += 2) {
      ...
      const p = y(t[u + 1], e, s);
      s || (s = p.type), l.push([h, p]);
    }
    if (s && s !== Y && s !== w && (s.kind !== "array" || s.itemType !== w)) throw new Error(`"${a}" cannot interpolate type ${rt(s)}`);
    return new k(a, s, i, o, l);
  }
  evaluate(t, e) {
    const r = this._stops;
    ...
    const o = r[i - 1][0], s = r[i][0], l = k.interpolationRatio(this.interpolation, a, o, s), u = r[i - 1][1].evaluate(t, e), h = r[i][1].evaluate(t, e);
    if (this._operator === "interpolate") {
      if (this.type.kind === "array" && Array.isArray(u) && Array.isArray(h)) return u.map((p, f) => A(p, h[f], l));
      if (this.type.kind === "color" && u instanceof T && h instanceof T) {
        const p = new T(u), f = new T(h);
        return new T([A(p.r, f.r, l), A(p.g, f.g, l), A(p.b, f.b, l), A(p.a, f.a, l)]);
      }
...
class Bt {
  ...
  static parse(t, e) {
    ...
    for (let o = 3; o < t.length; o += 2) {
      const s = t[o];
      if (typeof s != "number") throw new Error('"step" requires stop inputs as literal numbers');
      if (i.length && i[i.length - 1][0] >= s) throw new Error('"step" requires strictly ascending stop inputs');
      const l = y(t[o + 1], e);
      a || (a = l.type), i.push([s, l]);
    }
    return new Bt(a, r, i);
  }
  evaluate(t, e) {
...
class Gt {
  ...
  static parse(t, e, r) {
    ...
    for (let o = 1; o < t.length - 1; o += 2) {
      const s = t[o];
      if (typeof s != "string") throw new Error(`"let" requires a string to define variable names - found ${s}`);
      a.add(s, y(t[o + 1], e));
    }
...
at.ops = { downcase: (n) => n.toLowerCase(), upcase: (n) => n.toUpperCase() };
class Ft {
  constructor(t) {
...
class Mt {
  ...
  evaluate(t, e) {
    return new T(this._color.evaluate(t, e)).toRgba();
  }
}
class K {
  constructor(t, e) {
  ...
  static parse(t, e) {
    ...
    if (r === "array") {
      ...
      let s;
      if (t.length > 3) {
        if (s = t[2], s !== null && (typeof s != "number" || s < 0 || s !== Math.floor(s))) throw new Error('"array" length argument must be a positive integer literal');
        i++;
      }
      a = ht(a, s);
    } else switch (r) {
  ...
  evaluate(t, e) {
    ...
    for (const a of this._args) {
      ...
      if (r = It(i), xt(r, this.type)) return i;
    }
...
class G {
  ...
  static toColor(t) {
    ...
    if (typeof t == "string") {
      const e = T.fromString(t);
      if (e) return e;
      throw new Error(`"${t}" is not a color`);
    }
    if (Array.isArray(t)) return T.fromArray(t);
    throw new Error(`"${t}" is not a color`);
  }
}
...
function y(n, t, e) {
  ...
  if (!Array.isArray(n) || n.length === 0) throw new Error("Expression must be a non empty array");
  const a = n[0];
...
class bt {
  constructor(t, e) {
    ...
    if (e == null && (e = t.default), Array.isArray(e) && e.length > 0 && _e[e[0]]) {
      ...
      try {
        const o = t.type === "array" ? ht(i[t.value] || C, t.length) : i[t.type], s = y(e, null, o);
        this.getValue = this._buildExpression(s, t), this.isDataDriven = !0, s instanceof k && s.input instanceof le && (this.interpolator = s);
      } catch (o) {
  ...
  _validate(t, e) {
    ...
    } else e.type === "color" ? t = bt._parseColor(t) : e.type === "enum" ? typeof t == "string" && (t = e.values.indexOf(t)) : e.type === "array" && e.value === "enum" ? t = t.map((r) => typeof r == "string" ? e.values.indexOf(r) : r) : e.type === "string" && (t = Nt(t));
    return t;
  }
  _buildSimple(t) {
    return () => t;
  ...
  _interpolate(t, e, r) {
    ...
    if (a && i) {
      const s = i[0] - a[0], l = t - a[0], u = r === 1 ? l / s : (r ** l - 1) / (r ** s - 1);
  ...
  static _parseColor(t) {
    return Array.isArray(t) ? t : (typeof t == "string" && (t = new T(t)), t instanceof T && !this._isEmpty(t) ? T.toUnitRGBA(t) : void 0);
  }
}
const it = 8;
var ct;
...
class pt {
  ...
  isIntersectingFeature(t, e, r, a, i, o, s) {
    return !1;
  }
  ...
  computeAttributesKey(t, e, r, a) {
    ...
    for (const s of e) {
      let l = 3;
      if (!s || s !== a) {
        const u = r[s], { isLayout: h, isOptional: p } = u, f = h ? this.getLayoutProperty(s) : this.getPaintProperty(s);
...
class Qt extends pt {
  constructor(t, e, r, a) {
    super(t, e, r, a);
    const i = this.getPaintProperty("fill-color"), o = this.getPaintProperty("fill-opacity"), s = this.getPaintProperty("fill-pattern");
    this.hasDataDrivenColor = i?.isDataDriven, this.hasDataDrivenOpacity = o?.isDataDriven, this.hasDataDrivenFill = this.hasDataDrivenColor || this.hasDataDrivenOpacity || s?.isDataDriven;
  ...
  getFeatureInflatedBounds(t, e, r, a) {
    const i = Ht(t);
    if (!i) return null;
    const o = this.getPaintValue("fill-translate", e, t), s = a * Math.max(o[0], o[1]);
    return i[0] -= s, i[2] -= s, i[1] += s, i[3] += s, i;
  }
  isIntersectingFeature(t, e, r, a, i, o, s) {
    const l = a.getGeometry();
    if (!l) return !1;
    const u = it / s.normalizationRatio;
    t = t / s.normalizationRatio + s.normalizationOffsetX, e = e / s.normalizationRatio + s.normalizationOffsetY;
    const h = Pt(this.getPaintValue("fill-translate", i, a), this.getPaintValue("fill-translate-anchor", i, a), o, it);
    return t -= u * h.x, e -= u * h.y, !!be(t, e, l) || re(t, e, l, u * r);
  }
}
class Jt extends pt {
  constructor(t, e, r, a) {
    ...
    if (this.lineMaterial = new et(this.computeAttributesKey(W.LINE, et.ATTRIBUTES, et.ATTRIBUTES_INFO, i ? "line-dasharray" : "")), this.hasDataDrivenLine = this.getPaintProperty("line-blur")?.isDataDriven || this.getPaintProperty("line-color")?.isDataDriven || this.getPaintProperty("line-gap-width")?.isDataDriven || this.getPaintProperty("line-offset")?.isDataDriven || this.getPaintProperty("line-opacity")?.isDataDriven || this.getPaintProperty("line-pattern")?.isDataDriven || this.getPaintProperty("line-dasharray")?.isDataDriven || this.getLayoutProperty("line-cap")?.isDataDriven || this.getPaintProperty("line-width")?.isDataDriven, this.canUseThinTessellation = !1, !this.hasDataDrivenLine) {
      ...
      if (!o || typeof o == "number" && 0.5 * o < we) {
        const s = this.getPaintProperty("line-offset");
        (!s || typeof s == "number" && s === 0) && (this.canUseThinTessellation = !0);
      }
    }
  }
  getDashKey(t, e) {
  ...
  getFeatureInflatedBounds(t, e, r, a) {
    const i = Ht(t);
    if (!i) return null;
    const o = this.getPaintValue("line-translate", e, t), s = a * Math.max(o[0], o[1]);
    i[0] -= s, i[2] -= s, i[1] += s, i[3] += s;
...
class ee extends pt {
  ...
  getFeatureInflatedBounds(t, e, r, a) {
    const i = Ht(t);
    if (!i) return null;
    const o = this.getPaintValue("circle-translate", e, t), s = Math.max(o[0], o[1]);
    i[0] -= s, i[2] -= s, i[1] += s, i[3] += s;
  ...
  isIntersectingFeature(t, e, r, a, i, o, s) {
    ...
    if (!l) return !1;
    const u = it / s.normalizationRatio;
    t = t / s.normalizationRatio + s.normalizationOffsetX, e = e / s.normalizationRatio + s.normalizationOffsetY, r *= u;
...
function Ht(n) {
  ...
  for (const o of t) if (o) for (const s of o) e = Math.min(e, s.x), r = Math.min(r, s.y), a = Math.max(a, s.x), i = Math.max(i, s.y);
  return Ee(e, r, a, i);
}
class ot {
  constructor(t) {
    if (this._style = t, this.backgroundBucketIds = [], this._uidToLayer = /* @__PURE__ */ new Map(), this._layerByName = {}, this._runningId = 0, t.layers || (t.layers = []), this.version = parseFloat(t.version), this.layers = t.layers.map((e, r, a) => this._create(e, r, a)).filter((e) => !!e), this.layers) for (let e = 0; e < this.layers.length; e++) {
      ...
      this._layerByName[r.id] = r, this._uidToLayer.set(r.uid, r), r.type === b.BACKGROUND && this.backgroundBucketIds.push(r.id);
    }
  ...
  setStyleLayer(t, e) {
    ...
    if (o) {
      ...
      e || (e = s), e === s ? (i = !1, a = ot._recreateLayer(t, o), this.layers[e] = a, r.layers[e] = t) : (this.layers.splice(s, 1), r.layers.splice(s, 1), a = this._create(t, e, this.layers), this.layers.splice(e, 0, a), r.layers.splice(e, 0, t));
  ...
  setStyleLayerVisibility(t, e) {
    ...
    const a = r.layout || {};
    a.visibility = e;
    const i = { type: r.typeName, id: r.id, source: r.source, "source-layer": r.sourceLayer, minzoom: r.minzoom, maxzoom: r.maxzoom, filter: r.filter, layout: a, paint: r.paint }, o = ot._recreateLayer(i, r), s = this.layers.indexOf(r);
    this.layers[s] = o, this._style.layers[s].layout = a, this._layerByName[r.id] = o, this._uidToLayer.set(r.uid, o);
  }
  ...
  _identifyRefLayers() {
    ...
    for (const a of this.layers) {
      ...
      if (a.type === b.FILL) {
        ...
        s += "|" + (i?.visibility ?? ""), s += "|" + a.minzoom, s += "|" + a.maxzoom, s += "|" + JSON.stringify(a.filter), (o.hasDataDrivenFill || o.hasDataDrivenOutline) && (s += "|" + r), t.push({ key: s, layer: a });
      } else if (a.type === b.LINE) {
        const o = a, s = a.paint, l = s != null && (s["line-pattern"] != null || s["line-dasharray"] != null);
  ...
  _assignRefLayers(t) {
    ...
    for (let i = 0; i < a; i++) {
      ...
      if (o.key === e) o.layer.refLayerId = r;
        ...
        if (!o.layer.getPaintProperty("fill-outline-color")) for (let s = i + 1; s < a; s++) {
          const l = t[s];
          if (l.key !== e) break;
          if (l.layer.getPaintProperty("fill-outline-color")) {
            t[i] = l, t[s] = o, r = l.layer.id;
            break;
          }
        }
        ...
        for (let l = i + 1; l < a; l++) {
          ...
          if (u.key !== e) break;
          const h = u.layer;
          (s.canUseThinTessellation && !h.canUseThinTessellation || !s.canUseThinTessellation && (h.getPaintProperty("line-pattern") || h.getPaintProperty("line-dasharray"))) && (s = h, t[i] = u, t[l] = o, r = u.layer.id);
        }
      }
    }
  }
  _create(t, e, r) {
    ...
    switch (t.type) {
      case "background":
        return new Xt(b.BACKGROUND, t, a, i);
      case "fill":
        return new Qt(b.FILL, t, a, i);
      case "line":
        return new Jt(b.LINE, t, a, i);
      case "symbol":
        return new te(b.SYMBOL, t, a, i);
      case "raster":
        return console.warn(`Unsupported vector tile raster layer ${t.id}`), null;
      case "circle":
        return new ee(b.CIRCLE, t, a, i);
    }
    return null;
  }
  static _recreateLayer(t, e) {
    switch (t.type) {
      case "background":
        return new Xt(b.BACKGROUND, t, e.z, e.uid);
      case "fill":
        return new Qt(b.FILL, t, e.z, e.uid);
      case "line":
        return new Jt(b.LINE, t, e.z, e.uid);
      case "symbol":
        return new te(b.SYMBOL, t, e.z, e.uid);
      case "raster":
        return console.warn(`Unsupported vector tile raster layer ${t.id}`), null;
      case "circle":
        return new ee(b.CIRCLE, t, e.z, e.uid);
    }
    return null;
  }
}
...
//# sourceMappingURL=StyleRepository-BQUCE9Sk.js.map
