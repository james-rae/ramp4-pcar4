import { h9 as S, cF as F, b1 as v, b$ as O, ha as x } from "./main-DZxT3i7S.js";
import { O as C } from "./quat-BFf8_asR.js";
import { e as k } from "./quatf64-C16JxGFv.js";
import { t as A, n as w } from "./vec3f32-BS0cezmI.js";
import { a as R, b as U, d as B } from "./PointCloudUniqueValueRenderer-CIJLK593.js";
import { w as J, l as N, c as V, I as T } from "./I3SBinaryReader-Ig2phFmU.js";
import { I as q } from "./orientedBoundingBox-Dt2pL5Yb.js";
function z(s, t, a, n) {
  const { rendererJSON: c, isRGBRenderer: m } = s;
  let o = null, i = null;
  if (t && m) o = t;
  else if (t && c?.type === "pointCloudUniqueValueRenderer") {
    i = R.fromJSON(c);
    const e = i.colorUniqueValueInfos;
    o = new Uint8Array(3 * n);
    const f = g(i.fieldTransformType);
    for (let r = 0; r < n; r++) {
      const l = (f ? f(t[r]) : t[r]) + "";
    ...
    i = B.fromJSON(c);
    const e = i.colorClassBreakInfos;
    o = new Uint8Array(3 * n);
    const f = g(i.fieldTransformType);
    for (let r = 0; r < n; r++) {
      const l = f ? f(t[r]) : t[r];
  ...
  if (a && i?.colorModulation) {
    const e = i.colorModulation.minValue, f = i.colorModulation.maxValue, r = 0.3;
    for (let l = 0; l < n; l++) {
...
function E(s, t) {
  if (s.encoding == null || s.encoding === "") {
    const a = J(t, s);
    if (a.vertexAttributes.position == null) return;
    const n = N(t, a.vertexAttributes.position), c = a.header.fields, m = [c.offsetX, c.offsetY, c.offsetZ], o = [c.scaleX, c.scaleY, c.scaleZ], i = n.length / 3, e = new Float64Array(3 * i);
    for (let f = 0; f < i; f++) e[3 * f] = n[3 * f] * o[0] + m[0], e[3 * f + 1] = n[3 * f + 1] * o[1] + m[1], e[3 * f + 2] = n[3 * f + 2] * o[2] + m[2];
    return e;
  }
  if (s.encoding === "lepcc-xyz") return V(t).result;
}
function h(s, t, a) {
  return s?.attributeInfo.useElevation ? t ? X(t, a) : null : s?.attributeInfo.storageInfo ? T(s.attributeInfo.storageInfo, s.buffer, a) : null;
}
function X(s, t) {
  const a = new Float64Array(t);
  for (let n = 0; n < t; n++) a[n] = s[3 * n + 2];
  return a;
}
function Y(s, t, a, n, c) {
  const m = s.length / 3;
  let o = 0;
  for (let i = 0; i < m; i++) {
    let e = !0;
    for (let f = 0; f < n.length && e; f++) {
      const { filterJSON: r } = n[f], l = c[f].values[i];
      switch (r.type) {
    ...
    e && (a[o] = i, s[3 * o] = s[3 * i], s[3 * o + 1] = s[3 * i + 1], s[3 * o + 2] = s[3 * i + 2], t[3 * o] = t[3 * i], t[3 * o + 1] = t[3 * i + 1], t[3 * o + 2] = t[3 * i + 2], o++);
  }
  return o;
}
function g(s) {
...
class Z {
  transform(t) {
    ...
    for (const c of a.attributes) "buffer" in c.values && S(c.values.buffer) && c.values.buffer !== a.rgb.buffer && n.push(c.values.buffer);
  ...
  _transform(t) {
    ...
    const i = h(t.modulationAttributeData, a, n);
    t.modulationAttributeData != null && i && m.push({ attributeInfo: t.modulationAttributeData.attributeInfo, values: i });
    let e = z(t.rendererInfo, o, i, n);
    if (t.filterInfo && t.filterInfo.length > 0 && t.filterAttributesData != null) {
      const r = t.filterAttributesData.filter(F).map((l) => {
...
function _(s, t, a, n, c) {
  if (!O(s, n, 0, s, c, 0, t)) throw new Error("Can't reproject");
  const m = A(a.center), o = w(), i = w(), e = A(a.halfSize);
  C(M, a.quaternion);
  const f = new Float32Array(3 * t);
  for (let r = 0; r < t; r++) {
    let l = 3 * r;
    o[0] = s[l] - m[0], o[1] = s[l + 1] - m[1], o[2] = s[l + 2] - m[2], x(i, o, M), e[0] = Math.max(e[0], Math.abs(i[0])), e[1] = Math.max(e[1], Math.abs(i[1])), e[2] = Math.max(e[2], Math.abs(i[2])), f[l++] = o[0], f[l++] = o[1], f[l] = o[2];
  }
  return a.halfSize = e, f;
}
function j(s, t, a) {
  if (a !== 0) for (let n = 0; n < t; n++) s[3 * n + 2] += a;
}
const M = k();
function tt() {
  return new Z();
...
//# sourceMappingURL=PointCloudWorker-CJ9LI6iC.js.map
