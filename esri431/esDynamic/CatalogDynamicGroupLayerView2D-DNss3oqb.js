import { bQ as a, bR as s, bS as f, a8 as V, fv as C, gO as F, bm as U, gP as _, G as x, gQ as w } from "./main-DZxT3i7S.js";
import { j as H, y as I } from "./LayerView-BSi2ZKlw.js";
const u = Symbol(), O = (o) => {
  let e = class extends o {
    ...
    _createQuery() {
      const r = this.parent?.footprintLayerView, i = this.layer?.parent;
      if (!r || !i || i.destroyed) return null;
      const { layer: { maximumVisibleSublayers: n }, view: { scale: d } } = this;
      if (!n) return null;
      const { itemTypeField: l, itemSourceField: h, itemNameField: c, minScaleField: m, maxScaleField: L, objectIdField: v, orderBy: $ } = i, S = w(`${m} IS NULL OR ${d} <= ${m} OR ${m} = 0`, `${L} IS NULL OR ${d} >= ${L}`), p = $?.find((y) => y.field && !y.valueExpression), t = r.createQuery();
      if (t.returnGeometry = !1, t.num = n, t.outFields = [v, h, c], t.where = w(t.where, S), this.unsupportedItemTypes != null) {
        const y = `${l} NOT IN (${this.unsupportedItemTypes.map((b) => `'${b}'`)})`;
        t.where = w(t.where, y);
      }
      return p?.field && (t.orderByFields = [`${p.field} ${p.order === "descending" ? "DESC" : "ASC"}`], t.outFields.push(p.field)), t;
    }
  };
  return a([s({ readOnly: !0 })], e.prototype, "creatingLayerViews", null), a([s()], e.prototype, "layer", void 0), a([s()], e.prototype, "layerViews", void 0), a([s({ readOnly: !0 })], e.prototype, "unsupportedItemTypes", void 0), a([s()], e.prototype, "parent", void 0), a([s({ readOnly: !0 })], e.prototype, "isUpdating", null), e = a([f("esri.views.layers.CatalogDynamicGroupLayerView")], e), e;
};
let g = class extends O(H(I)) {
  constructor() {
...
g = a([f("esri.views.2d.layers.CatalogDynamicGroupLayerView2D")], g);
const E = g;
export {
  E as default
};
//# sourceMappingURL=CatalogDynamicGroupLayerView2D-DNss3oqb.js.map
