import { bQ as o, bR as n, aA as O, gN as Y, bS as k, bT as Q, eD as R, a8 as J, s as _, kU as Z, D as fe, hg as ge, kV as B, kW as we, cF as pe, ex as xe, ey as ve, eB as Se, ez as Me, eA as Ie, eC as Te, cz as re, f6 as j, cw as ie, B as se, at as Le, kX as Ee, kY as Ce, kZ as Pe, U as K, he as le, T as ae, I as G, gV as be, f9 as Re, eM as Ae } from "./main-DZxT3i7S.js";
import { o as oe } from "./xmlUtils-TLuV3CJ7.js";
var q;
let F = q = class extends Q {
  constructor(e) {
...
o([n({ type: O, json: { read: { source: "fullExtent" } } })], F.prototype, "fullExtent", void 0), o([n({ type: String, json: { read: { source: "id" } } })], F.prototype, "id", void 0), o([n({ type: Y, json: { read: { source: "tileInfo" } } })], F.prototype, "tileInfo", void 0), F = q = o([k("esri.layers.support.TileMatrixSet")], F);
const Oe = F;
var z;
let E = z = class extends Q {
  constructor(e) {
...
o([n({ json: { read: { source: "id" } } })], E.prototype, "id", void 0), o([n({ json: { read: { source: "title" } } })], E.prototype, "title", void 0), o([n({ json: { read: { source: "abstract" } } })], E.prototype, "description", void 0), o([n({ json: { read: { source: "legendUrl" } } })], E.prototype, "legendUrl", void 0), o([n({ json: { read: { source: "isDefault" } } })], E.prototype, "isDefault", void 0), o([n({ json: { read: { source: "keywords" } } })], E.prototype, "keywords", void 0), E = z = o([k("esri.layers.support.WMTSStyle")], E);
const Fe = E;
var H;
let w = H = class extends Q {
  constructor(e) {
  ...
  readFullExtent(e, t) {
    return (e = t.fullExtent) ? O.fromJSON(e) : null;
  }
  readFullExtents(e, t) {
    return t.fullExtents?.length ? t.fullExtents.map((r) => O.fromJSON(r)) : t.tileMatrixSets?.map((r) => O.fromJSON(r.fullExtent)).filter((r) => r) ?? [];
  }
  get imageFormat() {
  ...
  get tileMatrixSet() {
    return this.tileMatrixSets?.find(({ id: e }) => e === this.tileMatrixSetId) ?? null;
  }
  clone() {
    const e = new H();
...
function $e(e, t) {
  ...
  const C = x.indexOf("1.0.0/");
  C === -1 && v === "RESTful" ? x += "/" : C > -1 && (x = x.slice(0, C)), v === "KVP" && (x += C > -1 ? "" : "?"), p && (x = x.replace(/^http:/i, "https:"));
  const S = h("ServiceIdentification>ServiceTypeVersion", r), M = h("ServiceIdentification>AccessConstraints", r), P = M && /^none$/i.test(M) ? null : M, N = b("Layer", a), W = b("TileMatrixSet", a), D = N.map((L) => {
    const T = h("Identifier", L);
    return i.set(T, L), Ue(T, L, W, p, S);
  });
  return { copyright: P, dimensionMap: s, layerMap: i, layers: D, serviceMode: v, tileUrl: x };
}
function Ve(e) {
...
function Ue(e, t, r, i, s) {
  const a = h("Abstract", t), l = A("Format", t);
  return { id: e, fullExtent: Ne(t), fullExtents: We(t), description: a, formats: l, styles: De(t, i), title: h("Title", t), tileMatrixSets: Be(s, t, r) };
}
function me(e, t) {
  const r = [], i = e.layerMap?.get(t);
  if (!i) return null;
  const s = b("ResourceURL", i), a = b("Dimension", i);
  let l, u, c, m;
  return a.length && (l = h("Identifier", a[0]), u = A("Default", a[0]) || A("Value", a[0])), a.length > 1 && (c = h("Identifier", a[1]), m = A("Default", a[1]) || A("Value", a[1])), e.dimensionMap.set(t, { dimensions: u, dimensions2: m }), s.forEach((y) => {
...
function De(e, t) {
  return b("Style", e).map((r) => {
    const i = f("LegendURL", r), s = f("Keywords", r), a = s ? A("Keyword", s) : [];
...
function Be(e, t, r) {
  return b("TileMatrixSetLink", t).map((i) => Ke(e, i, r));
}
function Ke(e, t, r) {
  const i = f("TileMatrixSet", t).textContent, s = A("TileMatrix", t), a = r.find((S) => {
    const M = f("Identifier", S), P = M?.textContent;
    return !!(P === i || i.split(":") && i.split(":")[1] === P);
  }), l = f("TileMatrixSetLimits", t), u = l && b("TileMatrixLimits", l), c = /* @__PURE__ */ new Map();
  if (u?.length) for (const S of u) {
    const M = f("TileMatrix", S).textContent, P = +f("MinTileRow", S).textContent, N = +f("MaxTileRow", S).textContent, W = +f("MinTileCol", S).textContent, D = +f("MaxTileCol", S).textContent;
    c.set(M, { minCol: W, maxCol: D, minRow: P, maxRow: N });
  }
  ...
  s.length ? s.forEach((S, M) => {
    const P = $("TileMatrix", "Identifier", S, a);
    v.push(ue(P, m, M, i, c));
  }) : b("TileMatrix", a).forEach((S, M) => {
    v.push(ue(S, m, M, i, c));
  });
  const x = Je(e, a, y, I, v[0]).toJSON(), C = new Y({ dpi: 96, spatialReference: p, size: I, origin: y, lods: v }).toJSON();
  return { id: i, fullExtent: x, tileInfo: C };
}
function te(e) {
  e = e.toLowerCase();
...
function Je(e, t, r, i, s) {
  ...
  return qe(e, r.spatialReference, r) ? new O(y, c, p, m, r.spatialReference) : new O(c, y, m, p, r.spatialReference);
}
function qe(e, t, r) {
...
function ue(e, t, r, i, s) {
  ...
  u *= 96 / de;
  const m = +h("MatrixWidth", e), y = +h("MatrixHeight", e), { maxCol: p = m - 1, maxRow: g = y - 1, minCol: I = 0, minRow: v = 0 } = s.get(l) ?? {}, { x, y: C } = he(e, t);
  return new ge({ cols: [I, p], level: r, levelValue: l, origin: [x, C], scale: u, resolution: c, rows: [v, g] });
}
function ye(e, t, r) {
  let i;
...
var X;
const ce = { "image/png": ".png", "image/png8": ".png", "image/png24": ".png", "image/png32": ".png", "image/jpg": ".jpg", "image/jpeg": ".jpeg", "image/gif": ".gif", "image/bmp": ".bmp", "image/tiff": ".tif", "image/jpgpng": "", "image/jpegpng": "", "image/unknown": "" }, He = /* @__PURE__ */ new Set(["version", "service", "request", "layer", "style", "format", "tilematrixset", "tilematrix", "tilerow", "tilecol"]);
let d = X = class extends xe(ve(Se(Me(Ie(Te(Ae)))))) {
  constructor(...e) {
    ...
    }, j), ie(() => this.sublayers, "after-add", ({ item: t }) => {
      t.layer = this, t.parent = this;
    }, j), ie(() => this.sublayers, "after-remove", ({ item: t }) => {
      t.layer = null, t.parent = null;
    }, j), re(() => this.sublayers, (t, r) => {
  ...
  readActiveLayerFromItemOrWebDoc(e, t) {
    ...
    return new U({ id: a, imageFormat: c, styleId: m, tileMatrixSetId: l });
  }
  ...
  createWebTileLayer(e) {
    const t = this.getUrlTemplate(this.activeLayer.id, this.activeLayer.tileMatrixSetId, this.activeLayer.imageFormat, this.activeLayer.styleId), r = this._getTileMatrixSetById(e.tileMatrixSetId), i = r?.tileInfo, s = e.fullExtent, a = new Ce({ layerIdentifier: e.id, tileMatrixSet: e.tileMatrixSetId, url: this.url });
    return this.customLayerParameters && (a.customLayerParameters = this.customLayerParameters), this.customParameters && (a.customParameters = this.customParameters), new Pe({ fullExtent: s, urlTemplate: t, tileInfo: i, wmtsInfo: a });
  }
  ...
  getTileUrl(e, t, r) {
    ...
    let l = this.resourceInfo ? "" : je({ dimensionMap: this.dimensionMap, layerMap: this.layerMap }, this.activeLayer.id, this.activeLayer.tileMatrixSetId, this.activeLayer.imageFormat, this.activeLayer.styleId, a, t, r);
  ...
  getUrlTemplate(e, t, r, i) {
    if (!this.resourceInfo) {
      const s = ke({ dimensionMap: this.dimensionMap, layerMap: this.layerMap }, e, t, i);
      if (s) return s;
    }
  ...
  _appendParameters(e, t) {
    const r = G(e), i = { ...r.query, ...t }, s = be(i);
  ...
  _mergeParams(e, t) {
    ...
    if (r) {
      ...
      i.length && (e = e ? ae(e) : {}, i.forEach((s) => {
        e.hasOwnProperty(s) || He.has(s) || (e[s] = r[s]);
      }));
    }
    return e;
  }
