import { d as V } from "./WGLContainer-CjZoexai.js";
import { glslifyDefineMap as et, ProgramCache as st } from "./webglDeps-DXYheNKe.js";
import { aV as R, aY as N, aW as F, aX as Q, ig as x, ih as B, o6 as g, aF as m, aE as C, aD as a, hY as E, aH as S, Q as D, cQ as y, cU as k, cT as O, cX as Z, cS as G, aN as I, aq as $, cR as it, cV as rt, gR as v, lm as p, o7 as w, i0 as b, aG as M, o8 as at, lk as d, o9 as nt, oa as Y, fQ as lt, bo as ot, ll as A } from "./main-DZxT3i7S.js";
import { B as ht, a as ft, Q as ut } from "./renderState-vWaicc3E.js";
import { o as q } from "./interfaces-DN2-jsJC.js";
import { E as P } from "./BufferObject-Bb9Pw3sC.js";
import { x as L } from "./Program-7tJi4ey9.js";
import { t as X, f as _t, a as ct } from "./testSVGPremultipliedAlpha-CxmusDvb.js";
import { r as dt } from "./floatRGBA-R0V_PnPQ.js";
import { t as z } from "./doublePrecisionUtils-BJbYwoii.js";
import { o as J } from "./VertexArrayObject-DNKO7QGF.js";
...
let pt = class {
  ...
  get total() {
    return this.current.reduce((e, s, i) => e + (i < m.UNCOUNTED ? s : 0), 0);
  }
...
`);
  ...
  constructor(e, s, i) {
    const r = s.textureFilterAnisotropic, n = i.maxAnisotropy ?? 1 / 0;
...
class W {
  ...
  _createHelperProgram() {
    const e = W.getShaderSources();
    return this._rctx.programCache.acquire(e.vertex, e.fragment, /* @__PURE__ */ new Map([]));
  }
  _createIndexbuffer() {
    return P.createIndex(this._rctx, C.STATIC_DRAW, new Uint32Array([0]));
  }
  run() {
    this._program.compiled && this._indexBuffer && (this._rctx.bindVAO(null), this._rctx.useProgram(this._program), this._rctx.bindBuffer(this._indexBuffer, a.ELEMENT_ARRAY_BUFFER), this._rctx.drawElements(E.POINTS, 1, S.UNSIGNED_INT, 0));
  }
  dispose() {
...
class Ft extends X {
  constructor(e) {
    super(), this._rctx = e, this._helperProgram = null, D("mac") && D("chrome") && (this._program = this._prepareProgram(), this._helperProgram = this._prepareHelperProgram());
  }
  dispose() {
    super.dispose(), this._helperProgram?.dispose(), this._helperProgram = null;
  }
  _test(e) {
    const s = this._rctx, i = s.getBoundFramebufferObject(), { x: r, y: n, width: l, height: _ } = s.getViewport();
    s.resetState();
    const f = new y(1);
    f.wrapMode = k.CLAMP_TO_EDGE, f.samplingMode = O.NEAREST;
    const o = new L(s, f), h = P.createIndex(this._rctx, C.STATIC_DRAW, new Uint8Array([0]));
    s.bindFramebuffer(o), s.setViewport(0, 0, 1, 1), s.useProgram(this._helperProgram), s.bindBuffer(h, a.ELEMENT_ARRAY_BUFFER), s.drawElements(E.POINTS, 1, S.UNSIGNED_BYTE, 0), s.useProgram(e), s.bindVAO(null), s.drawArrays(E.TRIANGLES, 0, 258);
    const c = new Uint8Array(4);
    return o.readPixels(0, 0, 1, 1, Z.RGBA, G.UNSIGNED_BYTE, c), s.setViewport(r, n, l, _), s.bindFramebuffer(i), o.dispose(), h.dispose(), c[0] === 255;
  }
  _prepareProgram() {
...
let Rt = class extends X {
  ...
  _test(e) {
    ...
    const s = this._rctx, i = s.getBoundFramebufferObject(), { x: r, y: n, width: l, height: _ } = s.getViewport(), f = this._runProgram(e), o = this._runProgram(this._obfuscated);
    return s.setViewport(r, n, l, _), s.bindFramebuffer(i), f !== 0 && (o === 0 || f / o > 5);
  }
  _runProgram(e) {
    const s = this._rctx;
    s.resetState();
    const i = new y(1);
    i.wrapMode = k.CLAMP_TO_EDGE, i.samplingMode = O.NEAREST;
    const r = new L(s, i), n = P.createVertex(s, C.STATIC_DRAW, new Uint16Array([0, 0, 1, 0, 0, 1, 1, 1])), l = new J(s, /* @__PURE__ */ new Map([["position", 0]]), /* @__PURE__ */ new Map([["geometry", [new tt("position", 2, S.UNSIGNED_SHORT, 0, 4)]]]), /* @__PURE__ */ new Map([["geometry", n]])), _ = $(5633261287538229e-9, 2626832878767164e-9, 1.4349880495278358e6), f = $(563327146742708e-8, 2.6268736381334523e6, 1434963231608387e-9), o = new Float32Array(6);
    z(_, o, 3);
    const h = new Float32Array(6);
    z(f, h, 3), s.useProgram(e), e.setUniform3f("u_highA", o[0], o[2], o[4]), e.setUniform3f("u_lowA", o[1], o[3], o[5]), e.setUniform3f("u_highB", h[0], h[2], h[4]), e.setUniform3f("u_lowB", h[1], h[3], h[5]), s.bindFramebuffer(r), s.setViewport(0, 0, 1, 1), s.bindVAO(l), s.drawArrays(E.TRIANGLE_STRIP, 0, 4);
    const c = new Uint8Array(4);
    r.readPixels(0, 0, 1, 1, Z.RGBA, G.UNSIGNED_BYTE, c), l.dispose(), r.dispose();
...
class mt extends X {
  ...
  _test(e) {
    const s = this._rctx, i = new y(1);
    i.wrapMode = k.CLAMP_TO_EDGE, i.dataType = G.FLOAT, i.internalFormat = it.RGBA32F, i.samplingMode = O.NEAREST;
    const r = new L(s, i), n = P.createVertex(s, C.STATIC_DRAW, new Uint16Array([0, 0, 1, 0, 0, 1, 1, 1])), l = new J(s, /* @__PURE__ */ new Map([["a_pos", 0]]), /* @__PURE__ */ new Map([["geometry", [new tt("a_pos", 2, S.UNSIGNED_SHORT, 0, 4)]]]), /* @__PURE__ */ new Map([["geometry", n]]));
...
class Bt {
  constructor(e) {
    this.rctx = e, this.floatBufferBlend = new mt(e), this.svgPremultipliesAlpha = new _t(e), this.doublePrecisionRequiresObfuscation = new Rt(e), this.drawArraysRequiresIndicesTypeReset = new Ft(e);
  }
  dispose() {
...
function St(t, e, s, i, r) {
  if (e[s]) return !1;
  for (const n of r) if (t.getExtension(n)) return !0;
  return !1;
}
function Ot(t, e) {
...
class Ut {
  ...
  get colorBufferFloat() {
    return this._colorBufferFloat ??= Ct(this._gl, this._disabledExtensions), this._colorBufferFloat;
  }
  get loseContext() {
  ...
  get textureFloatLinear() {
    return this._textureFloatLinear ??= St(this._gl, this._disabledExtensions, "textureFloatLinear", !1, ["OES_texture_float_linear"]), this._textureFloatLinear;
  }
  enable(e) {
    return this[e];
  }
...
let zt = class {
  ...
  configure(t) {
    this._capabilities = new Ut(this.gl, t), this._parameters = new bt(this.gl, this._capabilities, t), rt.TEXTURE_UNIT_FOR_UPDATES = this._parameters.maxTextureImageUnits - 1;
  ...
  setActiveTexture(t, e = !1) {
    ...
    return t >= 0 && (e || t !== this._state.activeTexture) && (this.gl.activeTexture(w + t), this._state.activeTexture = t), s;
  }
  setDrawBuffers(t) {
    ...
    if (i?.length !== t.length || !i.every((r, n) => r === t[n])) if (t.length > this.parameters.maxDrawBuffers) console.error("Setting more active draw buffers than GL.MAX_DRAW_BUFFERS allows.");
    else {
      if (s) {
  ...
  clear(t, e = 255) {
    if (t) {
      ...
      t & b.DEPTH && this.setDepthWriteEnabled(!0), t & b.STENCIL && this.setStencilWriteMask(e), this.gl.clear(t);
    }
  }
  clearFramebuffer(t, e = !1, s = !1) {
    let i = 0;
    if (t) {
      const n = Math.max(1e-13, t[3]);
      this.setClearColor(t[0], t[1], t[2], n), i |= b.COLOR;
    }
    e && (i |= b.DEPTH), s === !1 ? s = 0 : (s === !0 && (s = 255), i |= b.STENCIL), i && this.clear(i, s);
  }
  drawArrays(t, e, s) {
    ...
    if (p()) {
      ...
      for (let r = 0; r < i.length; r++) {
        const n = i[r];
        if (n != null && n === this._state.drawFramebuffer?.colorTexture) throw new Error(`Detected readWrite. Texture already bound at index ${r}`);
      }
    }
    this.gl.drawArrays(t, e, s), M(this.gl);
  }
  drawArraysInstanced(t, e, s, i) {
    this.gl.drawArraysInstanced(t, e, s, i), M(this.gl);
  }
  drawElements(t, e, s, i) {
    ...
    if (p() && (this._numOfDrawCalls++, this._numOfTriangles += j(t, e)), this.gl.drawElements(t, e, s, i), p()) {
      const r = at(this);
      if (r) {
        const n = this.getBoundVAO(), l = n?.indexBuffer, _ = n?.vertexBuffers, f = { indexBuffer: l, vertexBuffers: _ }, o = { mode: t, count: e, type: s, offset: i }, h = l?.size ?? 0, c = i + e, T = h < c ? `. Buffer is too small. Attempted to draw index ${c} of ${h}` : "";
        console.error(`drawElements: ${r}${T}`, { args: o, vao: f });
      }
    }
  }
  drawElementsInstanced(t, e, s, i, r) {
    this.gl.drawElementsInstanced(t, e, s, i, r), M(this.gl);
  }
  logInfo() {
    p() && console.log(`DrawCalls: ${this._numOfDrawCalls}, Triangles: ${this._numOfTriangles}`);
  }
  resetInfo() {
    p() && (this._numOfDrawCalls = 0, this._numOfTriangles = 0);
  }
  get capabilities() {
  ...
  blitFramebuffer(t, e, s = 0, i = 0, r = t.width, n = t.height, l = 0, _ = 0, f = e.width, o = e.height, h = b.COLOR, c = O.NEAREST) {
    this.bindFramebufferSeparate(t, d.READ_FRAMEBUFFER), this.bindFramebufferSeparate(e, d.DRAW_FRAMEBUFFER), this.gl.blitFramebuffer(s, i, r, n, l, _, f, o, h, c);
  }
  bindBuffer(t, e) {
  ...
  bindBufferRange(t, e, s, i, r) {
    ...
    n != null && (n.buffer === s && n.offset === i && n.size === r || (i % this._parameters.uniformBufferOffsetAlignment == 0 ? (this.gl.bindBufferRange(t, e, s.glName, i, r), n.buffer = s, n.offset = i, n.size = r) : console.error("Uniform buffer binding offset is not a multiple of the context offset alignment")));
  }
  bindUBO(t, e, s, i) {
    e != null ? (p() && (i ?? e.byteLength) > this._parameters.maxUniformBlockSize && console.error("Attempting to bind more data than the maximum uniform block size"), e.initialize(), s !== void 0 && i !== void 0 ? this.bindBufferRange(a.UNIFORM_BUFFER, t, e.buffer, s, i) : this.bindBufferBase(a.UNIFORM_BUFFER, t, e.buffer)) : this.bindBufferBase(a.UNIFORM_BUFFER, t, null);
  }
  unbindUBO(t) {
  ...
  async clientWaitAsync(t = ot(10)) {
    const { gl: e } = this, s = e.fenceSync(nt.SYNC_GPU_COMMANDS_COMPLETE, 0);
    if (!s) throw new Error("Client wait failed, could not create sync object");
    let i;
    this.instanceCounter.increment(m.Sync, s), e.flush();
    do
      await lt(t), i = e.clientWaitSync(s, 0, 0);
    while (i === Y.TIMEOUT_EXPIRED);
    if (this.instanceCounter.decrement(m.Sync, s), e.deleteSync(s), i === Y.WAIT_FAILED) throw new Error("Client wait failed");
  }
  ...
  enforceState() {
    ...
    for (let s = 0; s < this._parameters.maxUniformBufferBindings; s++) {
      ...
      if (i != null) {
        const { buffer: r, offset: n, size: l } = i;
        r !== null ? n === 0 && l === 0 ? t.bindBufferBase(a.UNIFORM_BUFFER, s, r.glName) : t.bindBufferRange(a.UNIFORM_BUFFER, s, r.glName, n, l) : t.bindBufferBase(a.UNIFORM_BUFFER, s, null);
      }
    }
    ...
    for (let s = 0; s < this.parameters.maxTextureImageUnits; s++) {
      t.activeTexture(w + s), t.bindTexture(A.TEXTURE_2D, null), t.bindTexture(A.TEXTURE_CUBE_MAP, null), t.bindTexture(A.TEXTURE_3D, null), t.bindTexture(A.TEXTURE_2D_ARRAY, null);
      const i = this._state.textureUnitMap[s];
      i != null && t.bindTexture(i.descriptor.target, i.glName);
    }
    t.activeTexture(w + this._state.activeTexture);
...
//# sourceMappingURL=RenderingContext-B2w7Py6c.js.map
