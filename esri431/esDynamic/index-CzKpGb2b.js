import { a as i } from "./fabric-B6qRtwzH.js";
import { F as b, bp as A, A as S } from "./main-DZxT3i7S.js";
class y extends b {
  get config() {
  ...
  async make(s) {
    ...
    if (p.isWebMercator())
      ...
      const o = new A("pole", { x: -96, y: 90 }), w = await this.$iApi.geo.proj.projectGeometry(p, o), a = this.$iApi.geo.map.mapPointToScreenPoint(w), c = {
      ...
      h = Math.min(Math.max(x, 0), 100), l = (n) => {
        ...
        const P = t.clientWidth * n / 100;
        return Math.atan((a.screenX - P) / (50 / 2 - a.screenY)) * (180 / Math.PI);
      };
    }
    const g = {
      top: 0,
    ...
    ), e.set(S(g, s || {})), Promise.resolve(e);
  }
}
export {
  y as default
};
//# sourceMappingURL=index-CzKpGb2b.js.map
