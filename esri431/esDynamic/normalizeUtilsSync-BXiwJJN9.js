import { b7 as A, b8 as X, b9 as j, a7 as J, ba as b, bb as N, bc as R, aS as q, bd as z, be as M, bf as B, bg as D, bh as E, bi as p } from "./main-DZxT3i7S.js";
function U(i) {
  return k(i, !0);
}
function V(i) {
...
function k(i, s) {
  ...
  const t = i.spatialReference, n = X(t), e = j(i) ? i.toJSON() : i;
  if (!n) return e;
  const h = J(t) ? 102100 : 4326, u = b[h].maxX, _ = b[h].minX;
  if (N(e)) return S(e, u, _);
  if (R(e)) return e.points = e.points.map((o) => S(o, u, _)), e;
  if (q(e)) return F(e, n);
  if (z(e) || M(e)) {
    const o = B(H, e), r = { xmin: o[0], ymin: o[1], xmax: o[2], ymax: o[3] }, x = p(r.xmin, _) * (2 * u), m = x === 0 ? e : D(e, x);
    return r.xmin += x, r.xmax += x, r.xmax > u ? w(m, u, s) : r.xmin < _ ? w(m, _, s) : m;
  }
  return e;
}
function F(i, s) {
  if (!s) return i;
  const t = G(i, s).map((n) => n.extent);
...
function G(i, s) {
  const t = [], { ymin: n, ymax: e, xmin: h, xmax: u } = i, _ = i.xmax - i.xmin, [o, r] = s.valid, { x, frameId: m } = T(i.xmin, s), { x: a, frameId: c } = T(i.xmax, s), $ = x === a && _ > 0;
  if (_ > 2 * r) {
    const v = { xmin: h < u ? x : a, ymin: n, xmax: r, ymax: e }, O = { xmin: o, ymin: n, xmax: h < u ? a : x, ymax: e }, C = { xmin: 0, ymin: n, xmax: r, ymax: e }, P = { xmin: o, ymin: n, xmax: 0, ymax: e }, f = [], y = [];
    g(v, C) && f.push(m), g(v, P) && y.push(m), g(O, C) && f.push(c), g(O, P) && y.push(c);
    for (let d = m + 1; d < c; d++) f.push(d), y.push(d);
    t.push(new l(v, [m]), new l(O, [c]), new l(C, f), new l(P, y));
  } else x > a || $ ? t.push(new l({ xmin: x, ymin: n, xmax: r, ymax: e }, [m]), new l({ xmin: o, ymin: n, xmax: a, ymax: e }, [c])) : t.push(new l({ xmin: x, ymin: n, xmax: a, ymax: e }, [m]));
  return t;
}
function T(i, s) {
...
function g(i, s) {
  ...
  return I(i, t, n) && I(i, t, h) && I(i, e, h) && I(i, e, n);
}
function I(i, s, t) {
...
function w(i, s, t = !0) {
  const n = !M(i);
  if (n && E(i), t)
    return new K().cut(i, s);
  const e = n ? i.rings : i.paths, h = n ? 4 : 2, u = e.length, _ = -2 * s;
  for (let o = 0; o < u; o++) {
    const r = e[o];
    if (r && r.length >= h) {
      const x = [];
      for (const m of r) x.push([m[0] + _, m[1]]);
      e.push(x);
    }
  }
...
//# sourceMappingURL=normalizeUtilsSync-BXiwJJN9.js.map
