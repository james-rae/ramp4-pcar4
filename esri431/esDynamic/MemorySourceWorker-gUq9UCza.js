import { dz as O, s as I, dA as Q, dC as Z, dB as A, cn as C, dD as E, cd as b, dG as _, dI as w, dJ as $, dK as k, dy as S, bb as v } from "./main-DZxT3i7S.js";
import { t as P, n as G } from "./objectIdUtils-EszJVKSV.js";
import { m as z } from "./FeatureStore-zvkXSx1l.js";
import { V as L } from "./QueryEngine-DVmSGTfj.js";
import { a as W, u as B, l as H } from "./clientSideDefaults-ppWQ35EG.js";
import { j as U, f as g, p as T, d as j, y as D } from "./sourceUtils-CDUC47ds.js";
const J = S, K = { xmin: -180, ymin: -90, xmax: 180, ymax: 90, spatialReference: S }, N = { hasAttachments: !1, capabilities: "query, editing, create, delete, update", useStandardizedQueries: !0, supportsCoordinatesQuantization: !0, supportsReturningQueryGeometry: !0, advancedQueryCapabilities: { supportsQueryAttachments: !1, supportsStatistics: !0, supportsPercentileStatistics: !0, supportsReturningGeometryCentroid: !0, supportsQueryWithDistance: !0, supportsDistinct: !0, supportsReturningQueryExtent: !0, supportsReturningGeometryProperties: !1, supportsHavingClause: !0, supportsOrderBy: !0, supportsPagination: !0, supportsQueryWithResultType: !0, supportsSqlExpression: !0, supportsDisjointSpatialRel: !0, supportsQueryWithCacheHint: !0 } };
function V(h) {
...
class re {
  ...
  async load(e) {
    const s = [], { features: a } = e, r = this._inferLayerProperties(a, e.fields), d = e.fields || [], u = e.hasM != null ? e.hasM : !!r.hasM, f = e.hasZ != null ? e.hasZ : !!r.hasZ, p = !e.spatialReference && !r.spatialReference, o = p ? J : e.spatialReference || r.spatialReference, m = p ? K : null, y = e.geometryType || r.geometryType, l = !y;
    ...
    if (r.objectIdField && t !== r.objectIdField && (s.push({ name: "feature-layer:duplicated-oid-field", message: `Provided objectIdField "${t}" doesn't match the field name "${r.objectIdField}", found in the provided fields` }), t = r.objectIdField), t && !r.objectIdField) {
      const i = c.get(t);
      i ? (t = i.name, i.type = "esriFieldTypeOID", i.editable = !1, i.nullable = !1) : d.unshift({ alias: t, name: t, type: "esriFieldTypeOID", editable: !1, nullable: !1 });
    }
    for (const i of d) {
    ...
    const F = {};
    for (const i of d) if (i.type !== "esriFieldTypeOID" && i.type !== "esriFieldTypeGlobalID") {
      const q = A(i);
      q !== void 0 && (F[i.name] = q);
    }
    if (n) {
    ...
    const R = { warnings: s, featureErrors: [], layerDefinition: { ...N, drawingInfo: B(y), templates: H(F), extent: m, geometryType: y, objectIdField: t, fields: d, hasZ: f, hasM: u, timeInfo: n, dateFieldsTimeReference: x }, assignedObjectIds: {} };
    if (this._queryEngine = new L({ fieldsIndex: O.fromLayerJSON({ fields: d, timeInfo: n, dateFieldsTimeReference: x }), geometryType: y, hasM: u, hasZ: f, objectIdField: t, spatialReference: o, featureStore: new z({ geometryType: y, hasM: u, hasZ: f }), timeInfo: n }), !a?.length) return this._nextObjectId = P, R;
    const M = G(t, a);
    return this._nextObjectId = M + 1, await E(a, o), this._loadInitialFeatures(R, a);
  }
  async applyEdits(e) {
    const { spatialReference: s, geometryType: a } = this._queryEngine;
    return await Promise.all([U(s, a), E(e.adds, s), E(e.updates, s)]), this._applyEdits(e);
  }
  queryFeatures(e, s = {}) {
  ...
  _inferLayerProperties(e, s) {
    let a, r, d = null, u = null, f = null;
    for (const p of e) {
      const o = p.geometry;
      if (o != null && (d || (d = b(o)), u || (u = o.spatialReference), a == null && (a = V(o)), r == null && (r = X(o)), d && u && a != null && r != null)) break;
    }
    if (s && s.length) {
      let p = null;
      s.some((o) => {
        const m = o.type === "esriFieldTypeOID", y = !o.type && o.name && o.name.toLowerCase() === "objectid";
        return p = o, m || y;
      }) && (f = p.name);
    }
    return { geometryType: d, spatialReference: u, objectIdField: f, hasM: r, hasZ: a };
  }
  async _loadInitialFeatures(e, s) {
    const { geometryType: a, hasM: r, hasZ: d, objectIdField: u, spatialReference: f, featureStore: p, fieldsIndex: o } = this._queryEngine, m = [];
    for (const t of s) {
      if (t.uid != null && (e.assignedObjectIds[t.uid] = -1), t.geometry && a !== b(t.geometry)) {
        e.featureErrors.push(g("Incorrect geometry type."));
        continue;
      }
      const n = this._createDefaultAttributes(), c = T(o, n, t.attributes, !0);
  ...
  async _applyEdits(e) {
    const { adds: s, updates: a, deletes: r } = e, d = { addResults: [], deleteResults: [], updateResults: [], uidToObjectId: {} };
    if (s?.length && this._applyAddEdits(d, s), a?.length && this._applyUpdateEdits(d, a), r?.length) {
      for (const p of r) d.deleteResults.push(j(p));
    ...
    return { extent: u, timeExtent: f, featureEditResults: d };
  }
  _applyAddEdits(e, s) {
    const { addResults: a } = e, { geometryType: r, hasM: d, hasZ: u, objectIdField: f, spatialReference: p, featureStore: o, fieldsIndex: m } = this._queryEngine, y = [];
    for (const l of s) {
    ...
    o.addMany(w([], y, r, u, d, f));
  }
  _applyUpdateEdits({ updateResults: e }, s) {
    const { geometryType: a, hasM: r, hasZ: d, objectIdField: u, spatialReference: f, featureStore: p, fieldsIndex: o } = this._queryEngine;
    for (const m of s) {
      ...
      if (y) {
        const c = T(o, n.attributes, y);
        if (c) {
          e.push(c);
          continue;
        }
      }
      p.add(k(n, a, d, r, u)), e.push(j(t));
    }
  }
...
//# sourceMappingURL=MemorySourceWorker-gUq9UCza.js.map
