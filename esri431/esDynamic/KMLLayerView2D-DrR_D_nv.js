import { cI as T, D as k, cQ as U, cU as A, cX as G, cS as B, gR as E, cV as F, U as N, cp as D, a8 as O, cz as W, gS as H, cE as S, aA as $, cH as K, b1 as j, gT as Q, gU as L, bN as J, gV as X, bQ as v, bR as C, bS as Y } from "./main-DZxT3i7S.js";
import { b as P, g as Z, d as tt } from "./kmlUtils-BLLx81LN.js";
import { g as et, f as it, S as at } from "./Bitmap-6elP6Pc-.js";
import { a as st } from "./BitmapContainer-BUbnXPTn.js";
import { j as rt, y as nt } from "./LayerView-BSi2ZKlw.js";
import { t as M } from "./GraphicContainer-DBCyi5jQ.js";
import { F as R } from "./GraphicsView2D-DQwEO3YK.js";
import { C as ot, i as lt } from "./rasterProjectionHelper-CYzanw_V.js";
import { a as ht } from "./WGLContainer-CjZoexai.js";
import { y as pt, o as ct } from "./RenderingContext-B2w7Py6c.js";
import { x as mt } from "./Program-7tJi4ey9.js";
import { c as dt } from "./rasterUtils-CIOEzdot.js";
class p {
  constructor(t) {
    if (this._ownsRctx = !1, t) this._ownsRctx = !1, this._rctx = t;
      ...
      const i = document.createElement("canvas").getContext("webgl2");
      i.getExtension("OES_texture_float"), this._rctx = new pt(i, {});
    }
    const a = { applyProjection: !0, bilinear: !1, bicubic: !1 }, r = ct("raster/reproject", "raster/reproject", /* @__PURE__ */ new Map([["a_position", 0]]), a);
    this._program = this._rctx.programCache.acquire(r.shaders.vertexShader, r.shaders.fragmentShader, r.attributes), this._rctx.useProgram(this._program), this._program.setUniform1f("u_opacity", 1), this._program.setUniform1i("u_image", 0), this._program.setUniform1i("u_flipY", 0), this._program.setUniform1i("u_transformGrid", 1), this._quad = new ht(this._rctx, [0, 0, 1, 0, 0, 1, 1, 1]);
  }
  reprojectTexture(t, a, r = !1) {
    const i = T(t.extent, a), s = new k({ x: (t.extent.xmax - t.extent.xmin) / t.texture.descriptor.width, y: (t.extent.ymax - t.extent.ymin) / t.texture.descriptor.height, spatialReference: t.extent.spatialReference }), { x: l, y: h } = ot(s, a, t.extent);
    let n = (l + h) / 2;
    const o = Math.round((i.xmax - i.xmin) / n), g = Math.round((i.ymax - i.ymin) / n);
    n = (i.width / o + i.height / g) / 2;
    const I = new k({ x: n, y: n, spatialReference: i.spatialReference }), _ = lt({ projectedExtent: i, srcBufferExtent: t.extent, pixelSize: I, hasWrapAround: !0, spacing: [16, 16] }), w = dt(this._rctx, _), y = new U(o, g);
    y.wrapMode = A.CLAMP_TO_EDGE;
    const c = new mt(this._rctx, y);
    this._rctx.bindFramebuffer(c), this._rctx.setViewport(0, 0, o, g), this._rctx.useProgram(this._program), this._rctx.bindTexture(t.texture, 0), this._rctx.bindTexture(w, 1), this._quad.bind();
    const { width: x = 0, height: b = 0 } = t.texture.descriptor;
    if (this._program.setUniform2f("u_srcImageSize", x, b), this._program.setUniform2fv("u_transformSpacing", _.spacing), this._program.setUniform2fv("u_transformGridSize", _.size), this._program.setUniform2f("u_targetImageSize", o, g), this._quad.draw(), this._quad.unbind(), this._rctx.useProgram(null), this._rctx.bindFramebuffer(null), w.dispose(), r) {
      const { width: m, height: V } = c, u = new ImageData(m ?? 0, V ?? 0);
      c.readPixels(0, 0, m ?? 0, V ?? 0, G.RGBA, B.UNSIGNED_BYTE, u.data);
      const z = c.detachColorTexture(E.COLOR_ATTACHMENT0);
      return c.dispose(), { texture: z, extent: i, imageData: u };
    }
    const f = c.detachColorTexture(E.COLOR_ATTACHMENT0);
    return c.dispose(), { texture: f, extent: i };
  }
  reprojectBitmapData(t, a) {
    const r = et(t.bitmapData) ? it(t.bitmapData) : t.bitmapData, i = new U();
    i.wrapMode = A.CLAMP_TO_EDGE, i.width = t.bitmapData.width, i.height = t.bitmapData.height;
    const s = new F(this._rctx, i, r), l = this.reprojectTexture({ texture: s, extent: t.extent }, a, !0);
    l.texture.dispose();
  ...
  async loadAndReprojectBitmapData(t, a, r) {
    const i = (await N(t, { responseType: "image" })).data, s = document.createElement("canvas");
...
let d = class extends rt(nt) {
  constructor() {
    super(...arguments), this._bitmapIndex = /* @__PURE__ */ new Map(), this._mapImageContainer = new st(), this._kmlVisualData = new q(), this._fetchController = null, this.allVisiblePoints = new D(), this.allVisiblePolylines = new D(), this.allVisiblePolygons = new D(), this.allVisibleMapImages = new O();
  }
  async hitTest(e, t) {
    const a = this.layer;
  ...
  attach() {
    ...
    }), W(() => this.layer.visibleSublayers, (e) => {
  ...
  _addMapImage(e) {
    (this.view.spatialReference?.isWGS84 || this.view.spatialReference?.isWebMercator) && this._imageReprojector.loadAndReprojectBitmapData(e.href, $.fromJSON(e.extent), this.view.spatialReference).then((t) => {
      const a = new at(t.bitmapData);
  ...
  async _getViewDependentUrl(e, t) {
    ...
    if (a != null) {
      ...
      let s;
      if (await K(), r != null && r !== 1) {
        const m = new $(t.extent);
        m.expand(r), s = m;
      } else s = t.extent;
      s = T(s, j.WGS84);
      const l = T(s, j.WebMercator), h = s.xmin, n = s.xmax, o = s.ymin, g = s.ymax, I = t.size[0] * t.pixelRatio, _ = t.size[1] * t.pixelRatio, w = Math.max(l.width, l.height), y = { "[bboxWest]": h.toString(), "[bboxEast]": n.toString(), "[bboxSouth]": o.toString(), "[bboxNorth]": g.toString(), "[lookatLon]": s.center.x.toString(), "[lookatLat]": s.center.y.toString(), "[lookatRange]": w.toString(), "[lookatTilt]": "0", "[lookatHeading]": t.rotation.toString(), "[lookatTerrainLon]": s.center.x.toString(), "[lookatTerrainLat]": s.center.y.toString(), "[lookatTerrainAlt]": "0", "[cameraLon]": s.center.x.toString(), "[cameraLat]": s.center.y.toString(), "[cameraAlt]": w.toString(), "[horizFov]": "60", "[vertFov]": "60", "[horizPixels]": I.toString(), "[vertPixels]": _.toString(), "[terrainEnabled]": "0", "[clientVersion]": Q, "[kmlVersion]": "2.2", "[clientName]": "ArcGIS API for JavaScript", "[language]": "en-US" }, c = (m) => {
        for (const V in m) {
          let u;
          for (u in y) m[V] = m[V].replace(u, y[u]);
        }
      }, x = L(a);
      c(x);
      let b = {};
      i != null && (b = L(i), c(b));
      const f = J(e.href);
      return f.query = { ...f.query, ...x, ...b }, `${f.path}?${X(x)}`;
    }
    return e.href;
  }
  ...
  _fetchParsedKML(e, t) {
    return Z(e, this.layer.spatialReference, this.layer.refreshInterval, t).then((a) => tt(a.data));
  }
  _removeMapImage(e) {
...
v([C()], d.prototype, "_pointsView", void 0), v([C()], d.prototype, "_polylinesView", void 0), v([C()], d.prototype, "_polygonsView", void 0), v([C()], d.prototype, "updating", void 0), d = v([Y("esri.views.2d.layers.KMLLayerView2D")], d);
const It = d;
export {
  It as default
};
//# sourceMappingURL=KMLLayerView2D-DrR_D_nv.js.map
