import { g1 as R } from "./main-DZxT3i7S.js";
var p, g, N, f, m, d, S, b, U, I, L, D, h, M, a;
(function(t) {
  ...
  t.Rgb8 = "Rgb8", t.Rgba8 = "Rgba8", t.R8 = "R8", t.Bgr8 = "Bgr8", t.Bgra8 = "Bgra8", t.Rg8 = "Rg8", t.Dxt1 = "Dxt1", t.Dxt5 = "Dxt5", t.NotSet = "NotSet";
}(N || (N = {})), function(t) {
...
function T() {
  return new Promise((t) => import("./lyr3DWorker-B2wruIo5.js").then((e) => e.l).then(({ default: e }) => {
    const n = e({ locateFile: x, onRuntimeInitialized: () => t(n) });
  })).catch((t) => {
    throw t;
  });
}
function x(t) {
  return R(`esri/libs/lyr3d/${t}`);
}
let o, r;
async function F(t) {
  if (await y(), t.inputs.length < 1) return { result: { status: a.Failed, error: "", jobDescJson: "", data: new Uint8Array(0), missingInputUrls: [] }, transferList: [] };
  const e = { ptrs: [], sizes: [] };
  for (const i of t.inputs) {
    const u = r._malloc(i.byteLength);
    new Uint8Array(r.HEAPU8.buffer, u, i.byteLength).set(new Uint8Array(i)), e.ptrs.push(u), e.sizes.push(i.byteLength);
  }
  const n = r.process(t.jobDescJson, e, t.isMissingResourceCase), l = n.status === a.Succeeded && n.data, c = n.status === a.MissingInputs && n.missingInputUrls.length > 0;
  if (l) {
    const i = n.data.slice();
    n.data = i;
  } else c && (n.jobDescJson = n.jobDescJson.slice(), n.originalInputs = t.inputs);
  for (let i = 0; i < e.ptrs.length; ++i) r._free(e.ptrs[i]);
  const s = [];
...
function y() {
  return r ? Promise.resolve() : (o || (o = T().then((t) => {
    r = t, r.initialize_lyr3d_worker_wasm(), o = null;
  })), o);
}
export {
  w as destroyWasm,
...
//# sourceMappingURL=Lyr3DWorker-DNraFJTu.js.map
