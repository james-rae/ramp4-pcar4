import { oj as Le, b6 as Ae, jS as je, ok as qe, l9 as Ye, cu as Ge } from "./main-DZxT3i7S.js";
import { E as _e, I as ae } from "./enums-qHpGJ28Q.js";
import { h as L, s as be, o as oe, r as se, f as $e, a as Ve, e as ke, i as Ze, c as Y, p as ye, d as Je, g as Qe, _ as Xe } from "./GeometryUtils-C-fTcTXr.js";
import { t as Ce } from "./Rect-DD6XS68q.js";
import { o as Se } from "./config-nuMERBvb.js";
import { l as ie, m as k, o as Ee, n as Pe, p as et, u as we, a as ee } from "./StyleDefinition-D9H4pBJh.js";
import { a as tt, c as st, i as it } from "./TurboLine-BfHYB3zi.js";
import { s as rt, e as nt } from "./SourceLayerData-DEo9tEFZ.js";
import { t as v, U as at, O as ot, l as lt } from "./StyleRepository-BQUCE9Sk.js";
import { e as ve } from "./earcut-XDcq3zAf.js";
import { C as ht } from "./BidiEngine-Bdqv5H5j.js";
function ct(u) {
...
let Fe = class {
  ...
  getIconPlacement(t, e, i) {
    ...
    n && (r += t.angle);
    const o = new ze([]);
    return i.allowOverlap && i.ignorePlacement || !Se || (o.iconColliders = []), this._addIconPlacement(o, s, e, i, r), n && a && this._addIconPlacement(o, s, e, i, r + oe), o;
  }
  _addIconPlacement(t, e, i, s, n) {
    ...
    if (t.shapes.push(_), (!s.allowOverlap || !s.ignorePlacement) && Se) {
  ...
  getTextPlacement(t, e, i, s) {
    ...
    for (const b of e) {
      const S = b.glyphMosaicItem;
      if (!S || S.rect.isEmpty) continue;
      const V = S.rect, C = S.metrics, O = S.page;
      if (h && M) {
        if (D !== void 0 && D !== b.y) {
          let E, F, G, $;
          P ? (E = -I + T, F = x + B, G = I - g, $ = m - x) : (E = x + T, F = g + B, G = m - x, $ = I - g);
          const j = { xTile: t.x, yTile: t.y, dxPixels: E * _ - l, dyPixels: F * _ - l, hard: !s.optional, partIndex: 1, width: G * _ + 2 * l, height: $ * _ + 2 * l, angle: a, minLod: X, maxLod: se };
      ...
      const Z = [];
      if (d) {
        const E = 0.5 * S.metrics.width, F = (b.x + C.left - p + E) * _ * le;
      ...
      if (!d && P) if (b.vertical) {
        const E = (N + J) / 2 - C.height / 2, F = (W + Ie) / 2 + C.width / 2;
        R = new L(-F - p + T, E - p + B), U = new L(R.x + V.width, R.y + V.height), he = new L(R.x, U.y), ce = new L(U.x, R.y);
      ...
      for (const E of Z) {
        ...
        const fe = W, Me = Ie, de = E.glyphAngle + a;
        if (de !== 0) {
        ...
        let re = 0, ne = 256;
        if (d && P ? b.vertical ? E.alternateVerticalGlyph ? (re = 32, ne = 96) : (re = 224, ne = 32) : (re = 224, ne = 96) : (re = 192, ne = 64), y.push(new Fe(F, $, G, j, V, E.labelAngle, re, ne, E.anchor, E.minzoom, E.maxzoom, E.page)), h && (!w || this._legible(E.labelAngle))) {
          if (M) N < x && (x = N), fe < g && (g = fe), J > m && (m = J), Me > I && (I = Me);
          else if (E.minzoom < Te) {
            const q = { xTile: t.x, yTile: t.y, dxPixels: (N + T) * _ - l, dyPixels: (fe + T) * _ - l, hard: !s.optional, partIndex: 1, width: (J - N) * _ + 2 * l, height: (Me - fe) * _ + 2 * l, angle: de, minLod: E.minzoom, maxLod: E.maxzoom };
            h.push(q);
          }
        }
      }
    ...
    if (h && M) {
      let b, S, V, C;
      P ? (b = -I + T, S = x + B, V = I - g, C = m - x) : (b = x + T, S = g + B, V = m - x, C = I - g);
      const O = { xTile: t.x, yTile: t.y, dxPixels: b * _ - l, dyPixels: S * _ - l, hard: !s.optional, partIndex: 1, width: V * _ + 2 * l, height: C * _ + 2 * l, angle: a, minLod: X, maxLod: se };
      h.push(O);
    }
    const A = new ze(y);
...
class Mt extends me {
  ...
  processFeatures(t) {
    ...
    if (c || f) {
      const A = [];
      for (const b of s) {
        const S = c ? o.getValue(i, b) : this._getDashArrayKey(b, i, e, l, M, h, p), V = this._spriteInfo[S];
        if (!V?.rect) continue;
      ...
      A.sort((b, S) => b.page - S.page), n.textured = !0;
      for (const { ddAttributes: b, page: S, cap: V, join: C, miterLimit: O, roundLimit: Z, halfWidth: N, offset: W, geometry: J } of A) n.capType = V, n.joinType = C, n.miterLimit = O, n.roundLimit = Z, n.halfWidth = N, n.offset = W, this._processFeature(J, b, S);
    } else {
      if (o) {
      ...
      for (const A of s) {
        ...
        const S = A.getGeometry(t);
        this._processFeature(S, b);
      }
    }
  }
  serialize() {
  ...
  processFeatures(t) {
    ...
    for (const x of this._symbolFeatures) {
      ...
      if (w) {
        ...
        const B = c.letterSpacing * H, A = n ? 0 : c.maxWidth * H, b = c.lineHeight * H, S = x.textFont.map((V) => f.getGlyphItems(V));
        if (g = new Ke(S, A, b, B, P, D, T).getShaping(w, x.rtl, _), g && g.length > 0) {
          let V = 1e30, C = -1e30;
      ...
      if (y && p || (h || f ? h || p ? f || y || (p = null) : y = null : (y = null, p = null)), p) {
        ...
        if (this._storePlacedGlyphs(t, p.shapes, o, i.rotationAlignment, M), p.textColliders) {
          ...
          for (const x of p.textColliders) {
            ...
            const m = x.angle;
            if (m) {
              const g = Math.cos(m), I = Math.sin(m), w = x.dxPixels * g - x.dyPixels * I, _ = x.dxPixels * I + x.dyPixels * g, P = (x.dxPixels + x.width) * g - x.dyPixels * I, D = (x.dxPixels + x.width) * I + x.dyPixels * g, T = x.dxPixels * g - (x.dyPixels + x.height) * I, B = x.dxPixels * I + (x.dyPixels + x.height) * g, A = (x.dxPixels + x.width) * g - (x.dyPixels + x.height) * I, b = (x.dxPixels + x.width) * I + (x.dyPixels + x.height) * g, S = Math.min(w, P, T, A), V = Math.max(w, P, T, A), C = Math.min(_, D, B, b), O = Math.max(_, D, B, b);
              x.dxPixels = S, x.dyPixels = C, x.width = V - S, x.height = O - C;
            }
          }
        }
      }
...
//# sourceMappingURL=WorkerTileHandler-BfqqUsWr.js.map
