import { V as I, W as $, X as v, T as E, s as n, Y as S, Z as w, G as R, _ as O, $ as k, a0 as L, a1 as U, a2 as T, a3 as j, a4 as G, a5 as B, a6 as y, a7 as V, a8 as W } from "./main-DZxT3i7S.js";
function F(e) {
...
async function M(e, t, r, a) {
  ...
  const { edits: s, options: o } = await x(e, r, a);
...
async function x(e, t, r) {
  ...
  if (Y(t, a, r, !!s, !!o, `${e.type}-layer`), !a.data.isVersioned && r?.gdbVersion) throw new n(`${e.type}-layer:invalid-parameter`, "'gdbVersion' is applicable only if the layer supports versioned data. See: 'capabilities.data.isVersioned'");
  ...
  const d = P(t, a, `${e.type}-layer`), u = r?.globalIdUsed || p, c = e.fields.filter((l) => l.type === "big-integer" || l.type === "oid" && (l.length || 0) >= 8);
  if (u) {
  ...
  return d.addFeatures.forEach((l) => C(l, e, u, c)), d.updateFeatures.forEach((l) => Z(l, e, u, c)), d.deleteFeatures.forEach((l) => q(l, e, u, c)), d.addAttachments.forEach((l) => m(l, e)), d.updateAttachments.forEach((l) => m(l, e)), p && await H(d, e), { edits: await N(d), options: i };
}
function f(e, t, r, a) {
  if (r) {
  ...
  if (a.length && "attributes" in e) for (const s of a) {
    ...
    if (o !== void 0 && !j(s, o)) throw new n(`${t.type}-layer:invalid-parameter`, `Big-integer field '${s.name}' of the feature must be less than ${Number.MAX_SAFE_INTEGER}`, { feature: e });
  }
...
function A(e, t) {
  if ("geometry" in e && e.geometry?.type === "mesh" && t.infoFor3D != null && t.spatialReference != null) {
    const { geometry: r } = e, { spatialReference: a, vertexSpace: s } = r, o = t.spatialReference, p = s.type === "local", i = G(o), d = B(o, a), u = d || y(o) && (y(a) || V(a));
...
async function H(e, t) {
  const { infoFor3D: r } = t;
  if (r == null) return;
  if (!T(r)) throw new n(`${t.type}-layer:binary-gltf-asset-not-supported`, "3DObjectFeatureLayer requires binary glTF (.glb) support for updating mesh geometry.");
  e.addAssetFeatures ??= [];
  const { addAssetFeatures: a } = e;
  for (const s of e.addFeatures ?? []) b(s) && a.push(s);
  for (const s of e.updateFeatures ?? []) b(s) && a.push(s);
}
function b(e) {
...
export {
  ee as applyEdits,
  Y as checkEditingCapabilities,
  Q as isFeatureIdentifierArrayWithGlobalId,
  K as isFeatureIdentifierArrayWithObjectId,
  D as isFeatureIdentifierWithGlobalId,
  z as isFeatureIdentifierWithObjectId,
  X as normalizeCollections,
  P as normalizeEdits,
  N as normalizeGeometries,
  g as shallowCloneFeature,
  te as uploadAssets
};
//# sourceMappingURL=editingSupport-CSoNafh9.js.map
