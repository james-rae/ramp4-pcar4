import { D as W } from "./main-DZxT3i7S.js";
import { K, C as L, l as q } from "./rasterProjectionHelper-CYzanw_V.js";
class F {
  ...
  decreaseRefCount(n, e) {
    const t = n + "/" + e, r = this._cachedBlocks;
    if (r.has(t)) {
      const l = r.get(t);
      return l.refCount--, l.refCount <= 0 && (r.delete(t), l.controller && l.controller.abort()), l.refCount;
    }
    return 0;
  }
  getBlock(n, e) {
    ...
    if (r.has(t)) {
      const l = r.get(t);
      return l.ts = Date.now(), l.refCount++, r.delete(t), r.set(t, l), l.block;
    }
    return null;
  }
  putBlock(n, e, t, r) {
    const l = this._cachedBlocks, s = n + "/" + e;
    if (l.has(s)) {
      const i = l.get(s);
      i.ts = Date.now(), i.refCount++;
    } else l.set(s, { block: t, ts: Date.now(), refCount: 1, controller: r });
    this._trim(), this._updateTimer();
  }
  deleteBlock(n, e) {
    const t = this._cachedBlocks, r = n + "/" + e;
    t.has(r) && t.delete(r);
  }
  updateMaxSize(n) {
  ...
  _updateTimer() {
    ...
    this._timer = setInterval(() => {
      ...
      for (let r = 0; r < e.length && e[r][1].ts <= t - this._duration; r++) n.delete(e[r][0]);
...
function V(o, n, e) {
  ...
  if (n == null || t[n] == null) {
    for (let l = 0; l < t.length; l++) {
      const s = t[l]?.cache.get(e);
      if (s) return s.refCount++, s.block;
    }
    return h.getBlock(o, e);
  }
  const r = t[n]?.cache.get(e);
  if (r) return r.refCount++, r.block;
  for (let l = 0; l < t.length; l++) {
    if (l === n || !t[l]) continue;
    const s = t[l]?.cache, i = s?.get(e);
    if (s && i) return i.refCount++, s.set(e, i), i.block;
  }
  return null;
}
function X(o, n, e, t, r = null) {
  const l = u.get(o);
  if (!l) return void (n == null && h.putBlock(o, e, t, r));
  if (n == null || l[n] == null) return void h.putBlock(o, e, t, r);
  const s = { refCount: 1, block: t, isResolved: !1, isRejected: !1, controller: r };
  t.then(() => s.isResolved = !0).catch(() => s.isRejected = !0), l[n]?.cache.set(e, s);
}
function Y(o, n, e) {
  const t = u.get(o);
...
function Z(o, n, e, t, r, l, s = null) {
  ...
  for (let d = 0; d < M.length; d++) {
    ...
    if (r == null && !(r = L(I, R, a, s))) return;
    const { pyramidLevel: p, pyramidResolution: x, excessiveReading: T } = q(r, y, l || "closest");
    if (T) return;
...
//# sourceMappingURL=RawBlockCache-CCRacHkC.js.map
