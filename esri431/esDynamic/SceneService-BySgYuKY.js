import { U as y, s as c, bQ as n, bR as p, eE as T, b1 as _, eD as h, aA as S, iZ as P, eG as L, f9 as O, bS as U, fv as K, i_ as V, d as C, i$ as M, j0 as q, G as w, j1 as k, I as z, S as F, e as J } from "./main-DZxT3i7S.js";
import { i as $ } from "./originUtils-BLsWtgV9.js";
import { o as A } from "./jsonContext-iveSquhc.js";
import { p as D, n as B } from "./resourceUtils-BITspl6q.js";
import { t as Z } from "./saveUtils-GXJ71Xww.js";
async function G(a, o, t, e, i, r, s) {
  let l = null;
  if (t != null) {
    const u = `${a}/nodepages/`, E = u + Math.floor(t.rootIndex / t.nodesPerPage);
    try {
      return { type: "page", rootPage: (await y(E, { query: { f: "json", ...e, token: i }, responseType: "json", signal: s })).data, rootIndex: t.rootIndex, pageSize: t.nodesPerPage, lodMetric: t.lodSelectionMetricType, urlPrefix: u };
    } catch (N) {
      r?.warn("#fetchIndexInfo()", "Failed to load root node page. Falling back to node documents.", E, N), l = N;
    }
  }
  if (!o) return null;
  const d = o?.split("/").pop(), f = `${a}/nodes/`, v = f + d;
  try {
    return { type: "node", rootNode: (await y(v, { query: { f: "json", ...e, token: i }, responseType: "json", signal: s })).data, urlPrefix: f };
  } catch (u) {
...
const re = (a) => {
  let o = class extends a {
    ...
    readFullExtent(t, e, i) {
      ...
      const r = e.store, s = I(e);
      return s == null || r?.extent == null || !Array.isArray(r.extent) || r.extent.some((l) => l < x) ? null : new S({ xmin: r.extent[0], ymin: r.extent[1], xmax: r.extent[2], ymax: r.extent[3], spatialReference: s });
    }
    ...
    readTitleService(t, e) {
      ...
      if (this.sublayerTitleMode === "item-title") return this.url ? V(this.url, e.name) : e.name;
      let r = e.name;
      if (!r && this.url) {
        const s = C(this.url);
        s != null && (r = s.title);
    ...
    get parsedUrl() {
      const t = this._get("url"), e = z(t);
      return e && this.layerId != null && (e.path = `${e.path}/layers/${this.layerId}`), e;
    }
    async _fetchIndexAndUpdateExtent(t, e) {
      this.indexInfo = G(this.parsedUrl?.path ?? "", this.rootNode, t, this.customParameters, this.apiKey, w.getLogger(this), e), this.fullExtent == null || this.fullExtent.hasZ || this._updateExtent(await this.indexInfo);
    }
    _updateExtent(t) {
      if (t?.type === "page") {
        ...
        const r = i.obb.halfSize, s = i.obb.center[2], l = Math.sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);
        this.fullExtent.zmin = s - l, this.fullExtent.zmax = s + l;
      } else if (t?.type === "node") {
        const e = t.rootNode?.mbs;
        if (!Array.isArray(e) || e.length !== 4 || e[0] < x) return;
        const i = e[2], r = e[3], { fullExtent: s } = this;
        s && (s.zmin = i - r, s.zmax = i + r);
      }
    }
    ...
    async _saveAs(t, e) {
      ...
      const s = r.portal || J.getDefault();
      await this._ensureLoadBeforeSave(), r.type = b, r.portal = s;
      const l = A(r, "portal-item", !0), d = { layers: [this.write({}, l)] };
      return await Promise.all(l.resources.pendingOperations ?? []), await this._validateAgainstJSONSchema(d, l, i), this.url && (r.url = this.url), r.title || (r.title = this.title), R(r, i, m.newItem), await s.signIn(), await s.user.addItem({ item: r, folder: i?.folder, data: d }), await D(this.resourceReferences, l), this.portalItem = r, $(l), l.portalItem = r, r;
    }
    async _save(t) {
      const e = { ...j, ...t };
      ...
      if (this.portalItem.type !== b) throw new c("sceneservice:portal-item-wrong-type", `Portal item needs to have type "${b}"`);
      await this._ensureLoadBeforeSave();
      const i = A(this.portalItem, "portal-item", !0), r = { layers: [this.write({}, i)] };
      return await Promise.all(i.resources.pendingOperations ?? []), await this._validateAgainstJSONSchema(r, i, e), this.url && (this.portalItem.url = this.url), this.portalItem.title || (this.portalItem.title = this.title), R(this.portalItem, e, m.existingItem), await B(this.portalItem, r, this.resourceReferences, i), $(i), this.portalItem;
    }
    async _validateAgainstJSONSchema(t, e, i) {
      const r = i?.validationOptions;
      Z(e, { errorName: "sceneservice:save" }, { ignoreUnsupported: r?.ignoreUnsupported, supplementalUnsupportedErrors: ["scenemodification:unsupported"] });
      const s = r?.enabled, l = Q();
      if (s && l) {
...
//# sourceMappingURL=SceneService-BySgYuKY.js.map
