import { dD as T, s as g, J as p, dz as E, cn as P, dE as q, G as y, e1 as _, cu as R, a5 as I, dF as b, dG as $, dH as j } from "./main-DZxT3i7S.js";
import { m as N } from "./FeatureStore-zvkXSx1l.js";
import { V as O } from "./QueryEngine-DVmSGTfj.js";
import { E as Q, N as M } from "./geojson-B1MnUu7p.js";
import { p as k } from "./sourceUtils-CDUC47ds.js";
import { o as L, e as G, K as f } from "./wfsUtils-BRgl2uBl.js";
...
class H {
  ...
  async load(t, e = {}) {
    const { getFeatureUrl: s, getFeatureOutputFormat: o, fields: i, geometryType: u, featureType: r, maxRecordCount: n, maxTotalRecordCount: c, maxPageCount: C, objectIdField: d, customParameters: F } = t, { spatialReference: l, getFeatureSpatialReference: m } = L(s, r, t.spatialReference);
    try {
      await T(m, l);
    } catch {
      throw new g("unsupported-projection", "Projection not supported", { inSpatialReference: m, outSpatialReference: l });
    }
    p(e), this._customParameters = F, this._featureType = r, this._fieldsIndex = E.fromLayerJSON({ fields: i, dateFieldsTimeReference: i.some((S) => S.type === "esriFieldTypeDate") ? { timeZoneIANA: P } : null }), this._geometryType = u, this._getFeatureUrl = s, this._getFeatureOutputFormat = o, this._getFeatureSpatialReference = m, this._maxRecordCount = n, this._maxTotalRecordCount = c, this._maxPageCount = C, this._objectIdField = d, this._spatialReference = l;
    ...
    return this._queryEngine = new O({ fieldsIndex: this._fieldsIndex, geometryType: u, hasM: !1, hasZ: !1, objectIdField: d, spatialReference: l, timeInfo: null, featureStore: new N({ geometryType: u, hasM: !1, hasZ: !1 }) }), this._queryEngine.featureStore.addMany(h.features), { warnings: w(h), extent: (await this._queryEngine.fetchRecomputedExtents()).fullExtent };
  }
  async applyEdits() {
  ...
  async refresh(t) {
    return this._customParameters = t.customParameters, this._maxRecordCount = t.maxRecordCount, this._maxTotalRecordCount = t.maxTotalRecordCount, this._maxPageCount = t.maxPageCount, this._snapshotTask?.abort(), this._snapshotTask = q((e) => this._snapshotFeatures({ signal: e })), this._snapshotTask.promise.then((e) => {
  ...
  async _snapshotFeatures(t) {
    const e = t?.signal, s = this._maxTotalRecordCount, o = this._maxPageCount, i = this._supportsPagination ? await G(this._getFeatureUrl, this._featureType.typeName, { customParameters: this._customParameters, signal: e }) : void 0;
    let u = [];
    const r = [];
    if (i == null) try {
      ...
      const n = Math.min(i, s), c = J(this, Math.max(1, Math.min(Math.ceil(n / this._maxRecordCount), o)), e);
      await Promise.allSettled(Array.from({ length: 10 }).map(() => U(c, u, r)));
    }
  ...
  _processGeoJSON(t, e) {
    ...
    if (!I(this._spatialReference, this._getFeatureSpatialReference)) for (const r of i) r.geometry != null && (r.geometry = b($(j(r.geometry, this._geometryType, !1, !1), this._getFeatureSpatialReference, this._spatialReference)));
    let u = s ?? 1;
    for (const r of i) {
      const n = {};
...
//# sourceMappingURL=WFSSourceWorker-xGFCqp-n.js.map
