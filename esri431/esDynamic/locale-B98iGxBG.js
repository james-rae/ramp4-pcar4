import { a as w, c as S } from "./dom-N58V0tYi.js";
import { n as v } from "./key-CZySMiwx.js";
import { c as L } from "./observers-lnyvGi5X.js";
...
function q(t) {
  return !t || !M(t) ? "" : h(t, (e) => {
    let n = !1;
    const s = e.split("").filter((r, a) => r.match(/\./g) && !n ? (n = !0, !0) : r.match(/-/g) && a === 0 ? !0 : v.includes(r)).join("");
    return f(s) ? new i(s).toString() : "";
  });
}
const I = /^([-0])0+(?=\d)/, D = /(?!^\.)\.$/, _ = /(?!^-)-/g, x = /^-\b0\b\.?0*$/, R = /0*$/, Q = (t) => h(t, (e) => {
  const n = e.replace(_, "").replace(D, "").replace(I, "$1");
...
function z(t) {
  ...
  return `${s ? "-" : ""}${$.charAt(0) === "." ? "0" : ""}${$.replace(A, "").replace(I, "")}`;
}
function M(t) {
  return v.some((e) => t.includes(e));
}
function Y(t, e, n) {
  const s = e.split(".")[1];
  if (s) {
...
const u = "en", j = [
  ...
  "zh-HK",
  "zh-TW"
], H = ["arab", "arabext", "latn"], E = (t) => H.includes(t), p = new Intl.NumberFormat().resolvedOptions().numberingSystem, N = p === "arab" || !E(p) ? "latn" : p, P = (t) => E(t) ? t : N;
function Z(t, e = "cldr") {
  const n = e === "cldr" ? k : j;
...
function U(t) {
  return t.el.lang || S(t.el, "[lang]")?.lang || document.documentElement.lang || u;
}
class B {
  constructor() {
    this.delocalize = (e) => (
  ...
  set numberFormatOptions(e) {
    if (e.locale = Z(e?.locale), e.numberingSystem = P(e?.numberingSystem), // No need to create the formatter if `locale` and `numberingSystem`
...
//# sourceMappingURL=locale-B98iGxBG.js.map
