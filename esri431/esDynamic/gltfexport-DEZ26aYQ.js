import { s as k, J as v, m5 as ie, _ as re, aH as m, eb as L, dY as Y, am as ne, ao as ae, ap as M, H as j, it as oe, dX as $, o5 as Q, fX as ce, bW as he, cU as b, G as fe, fP as ue } from "./main-DZxT3i7S.js";
import { N as le, T as de } from "./quat-BFf8_asR.js";
import { r as _e, t as me, e as pe } from "./quatf64-C16JxGFv.js";
import { m as ge } from "./Mesh-C64CSjtZ.js";
import { B as we } from "./vertexSpaceConversion-Xm0UohEX.js";
import { r as U } from "./resourceUtils-CeYHTFuO.js";
function Ae(i) {
  const e = q(i);
...
async function J(i, e) {
  const t = q(i);
  if (t == null) throw new k("imageToArrayBuffer", "Unsupported image type");
  const s = Te(i), r = await new Promise((a) => t.toBlob(a, s));
  if (v(e), !r) throw new k("imageToArrayBuffer", "Failed to encode image");
...
let I = class N {
  constructor(e, t) {
    ...
    const s = Re(e);
    if (this._length += W(s.byteLength, 4), t && (this._length += N.CHUNK_HEADER_SIZE, this._length += t.byteLength, t.byteLength % 4)) throw new Error("Expected BIN chunk length to be divisible by 4 at this point");
    this.buffer = new ArrayBuffer(this._length), this._outView = new DataView(this.buffer), this._writeHeader();
    const r = this._writeChunk(s, 12, O.JSON, 32);
    t && this._writeChunk(t, r, O.BIN);
  }
  _writeHeader() {
...
I.HEADER_SIZE = 12, I.CHUNK_HEADER_SIZE = 8, I.MAGIC = 1179937895, I.VERSION = 2;
var R, E, y, g, K, B, X;
(function(i) {
...
function Ne(i, e) {
  if (i.components) for (const t of i.components) t.faces && t.shading === "smooth" && Ie(t, e);
}
function Ie(i, e) {
  e.normal == null && (e.normal = new Float32Array(e.position.length));
  const t = i.faces, { position: s, normal: r } = e, n = t.length / 3;
  for (let a = 0; a < n; ++a) {
    const o = 3 * t[3 * a], c = 3 * t[3 * a + 1], h = 3 * t[3 * a + 2], u = L(Me, s[o], s[o + 1], s[o + 2]), p = L(Se, s[c], s[c + 1], s[c + 2]), l = L(Le, s[h], s[h + 1], s[h + 2]), w = Y(p, p, u), d = Y(l, l, u), _ = ne(w, w, d);
    r[o] += _[0], r[o + 1] += _[1], r[o + 2] += _[2], r[c] += _[0], r[c + 1] += _[1], r[c + 2] += _[2], r[h] += _[0], r[h + 1] += _[1], r[h + 2] += _[2];
  }
  for (let a = 0; a < r.length; a += 3) L(x, r[a], r[a + 1], r[a + 2]), ae(x, x), r[a] = x[0], r[a + 1] = x[1], r[a + 2] = x[2];
}
const Me = M(), Se = M(), Le = M(), x = M(), P = () => fe.getLogger("esri.geometry.support.meshUtils.exporters.gltf.gltf");
class Oe {
  constructor(e, t) {
  ...
  _addNodes(e) {
    ...
    j(s, oe) || (t.translation = $(s));
    const r = e.rotation;
    le(r, _e) || (t.rotation = me(r));
    const n = e.scale;
    j(n, Q) || (t.scale = $(n));
  ...
  _createMeshes(e) {
    ...
    const o = we({ vertexAttributes: e.vertexAttributes, vertexSpace: e.vertexSpace, transform: this.options?.ignoreLocalTransform ? null : e.transform, spatialReference: e.spatialReference }, a, { targetUnit: this.options.unitConversionDisabled ? void 0 : "meters" });
    if (!o) return [];
    Ne(e, o), Be(o);
    const { position: c, normal: h, tangent: u } = o, { uv: p, color: l } = e.vertexAttributes, w = r.addBufferView(m.FLOAT, g.VEC3, y.ARRAY_BUFFER);
    let d, _, A, T;
    h && (d = r.addBufferView(m.FLOAT, g.VEC3, y.ARRAY_BUFFER)), p && (_ = r.addBufferView(m.FLOAT, g.VEC2, y.ARRAY_BUFFER)), u && (A = r.addBufferView(m.FLOAT, g.VEC4, y.ARRAY_BUFFER)), l && (T = r.addBufferView(m.UNSIGNED_BYTE, g.VEC4, y.ARRAY_BUFFER)), w.startAccessor("POSITION"), d && d.startAccessor("NORMAL"), _ && _.startAccessor("TEXCOORD_0"), A && A.startAccessor("TANGENT"), T && T.startAccessor("COLOR_0");
    const te = o.position.length / 3;
    for (let f = 0; f < te; ++f) w.push(c[3 * f]), w.push(c[3 * f + 1]), w.push(c[3 * f + 2]), d && h != null && (d.push(h[3 * f]), d.push(h[3 * f + 1]), d.push(h[3 * f + 2])), _ && p != null && (_.push(p[2 * f]), _.push(p[2 * f + 1])), A && u != null && (A.push(u[4 * f]), A.push(u[4 * f + 1]), A.push(u[4 * f + 2]), A.push(u[4 * f + 3])), T && l != null && (T.push(l[4 * f]), T.push(l[4 * f + 1]), T.push(l[4 * f + 2]), T.push(l[4 * f + 3]));
    const se = w.endAccessor(), H = this._addAccessor(w.index, se);
    let C, V, z, F, S;
    if (d) {
      const f = d.endAccessor();
      C = this._addAccessor(d.index, f);
    }
    if (_) {
      const f = _.endAccessor();
      V = this._addAccessor(_.index, f);
    }
    if (A) {
    ...
    const D = [];
    return e.components && e.components.length > 0 && e.components[0].faces ? (S = r.addBufferView(m.UNSIGNED_INT, g.SCALAR, y.ELEMENT_ARRAY_BUFFER), this._addMeshVertexIndexed(S, e.components, D, H, C, V, z, F)) : this._addMeshVertexNonIndexed(e.components, D, H, C, V, z, F), w.finalize(), d && d.finalize(), _ && _.finalize(), A && A.finalize(), S && S.finalize(), T && T.finalize(), s || r.finalize(), D;
  }
  _addMaterial(e) {
    if (e === null) return;
  ...
  _addImage(e) {
    ...
    if (e.url) s.uri = e.url;
      ...
      switch (a.output.image) {
        case E.GLB: {
          const o = a.binChunkBuffer.addBufferView(m.UNSIGNED_BYTE, g.SCALAR);
          if (U(n)) n.data != null && o.writeOutToBuffer(n.data, 0);
          else {
            const c = J(n, this.options.signal).then(({ data: h, type: u }) => (s.mimeType = u, h));
        ...
        default:
          ...
          a.promises.push(J(n, this.options.signal).then(({ data: o, type: c }) => {
...
function Be({ position: i, normal: e, tangent: t }) {
  G(i, 3), G(e, 3), G(t, 4);
}
function G(i, e) {
...
class ze {
  constructor(e) {
    this.mesh = e, this.name = "", this.translation = M(), this.rotation = pe(), this.scale = $(Q), this._nodes = [];
  }
  addNode(e) {
    if (this._nodes.includes(e)) throw new Error("Node already added");
    this._nodes.push(e);
...
async function De(i, e) {
  const t = new Oe(i, e).gltf, s = t.extras.promises;
  let r = 1, n = 1, a = null;
  ...
  return e.output?.buffer === R.GLB || e.output?.image === E.GLB ? c.set(ee, new I(h, a).buffer) : c.set(Fe, h), c;
}
