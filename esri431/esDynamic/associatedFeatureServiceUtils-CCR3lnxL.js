import { d as w, s as c, S as p, e as j, B as u, U as y, C as D } from "./main-DZxT3i7S.js";
async function q(e, n) {
  ...
  if (r.sceneLayerItem ??= await $(r), r.sceneLayerItem == null) return v(r.sceneServerUrl.replace("/SceneServer", "/FeatureServer"), r);
  const a = await A(r);
...
async function $(e) {
  ...
  if (!n) return null;
  const t = new p({ id: n, apiKey: e.apiKey }), r = await b(e);
...
async function A({ sceneLayerItem: e, signal: n }) {
  if (!e) return null;
  try {
    ...
    if (!t) return null;
    const r = new p({ portal: t.portal, id: t.id });
...
//# sourceMappingURL=associatedFeatureServiceUtils-CCR3lnxL.js.map
