import { bQ as t, bR as r, bS as $, fF as L, bU as N, aC as b, b1 as g, hL as U, s as w, eP as z, eO as H, hj as J, hk as V, ex as Z, hi as k, hl as W, eB as K, ez as X, eA as Y, ey as ee, eC as te, hm as R, ho as se, fS as x, hn as re, eU as oe, ay as ie, aA as ne, e8 as F, hs as pe, ht as ae, hr as le, eF as ue, hv as de, cB as ce, hw as ye, hx as he, hy as fe, hM as me, eG as ge, hA as ve, eM as Se } from "./main-DZxT3i7S.js";
import { R as Ce, x as I, C as O, P as be, O as we, N as Re, q as xe, v as Fe, k as Ie } from "./ogcFeatureUtils-BFNgArIn.js";
import { s as Oe } from "./capabilities-Cjn77swe.js";
let d = class extends L {
  ...
  _getStorageSpatialReference(e) {
    const o = e.storageCrs ?? I, i = O(o);
    return i == null ? g.WGS84 : new g({ wkid: i });
  }
  _getSupportedSpatialReferences(e, o) {
    const i = "#/crs", n = e.crs ?? [I], l = n.includes(i) ? n.filter((p) => p !== i).concat(o.crs ?? []) : n, a = /^http:\/\/www\.opengis.net\/def\/crs\/epsg\/.*\/3785$/i;
    return l.filter((p) => !a.test(p));
  }
  async _loadOGCServices(e, o) {
    const i = o != null ? o.signal : null, { apiKey: n, collectionId: l, customParameters: a, fields: p, geometryType: D, hasZ: T, objectIdField: j, timeInfo: P, url: q } = e, _ = { fields: p?.map((u) => u.toJSON()), geometryType: U.toJSON(D), hasZ: T ?? !1, objectIdField: j, timeInfo: P?.toJSON() }, c = { apiKey: n, customParameters: a, signal: i }, h = await be(q, c), [v, S] = await Promise.all([we(h, c), Re(h, c)]);
    ...
    const E = this._conformsToType(v, "http://www.opengis.net/spec/ogcapi-features-1/1.0/conf/oas30") ? await xe(h, c) : null, C = await Fe(y, _, c), A = this._getMaxRecordCount(E), B = this._getCapabilities(C.hasZ, A), Q = this._getStorageSpatialReference(y).toJSON(), G = this._getSupportedSpatialReferences(y, S), M = new RegExp(`^${b(Ie)}`, "i"), f = {};
    for (const u of G) {
      const m = O(u);
      m != null && (f[m] || (f[m] = u.replace(M, "")));
    }
    this.featureDefinition = { capabilities: B, collection: y, layerDefinition: C, spatialReference: Q, supportedCrs: f };
  }
};
...
const $e = ve();
let s = class extends z(H(J(V(Z(k(W(K(X(Y(ee(te(Se)))))))))))) {
  constructor(e) {
  ...
  queryFeatures(e, o) {
    return this.load().then(() => this.source.queryFeatures(x.from(e) || this.createQuery(), o)).then((i) => (i?.features?.forEach((n) => {
  ...
  async _fetchService(e) {
    await this.source.load(e), this.read(this.source.featureDefinition, { origin: "service" }), R(this.renderer, this.fieldsIndex), re(this.timeInfo, this.fieldsIndex);
  }
  _getLayerDomain(e) {
