import "tiny-emitter";
import { F as C, w as y, m as z, G, P as T, x as W, L as B, y as H, z as R } from "./main-DFWvmD-k.js";
...
import "proj4";
import { debounce as Y } from "es-toolkit/function";
...
const Q = "path", Z = 12, ee = "M14.4 6L14 4H5v17h2v-7h5.6l.4 2h7V6z", te = "#ff0000ff", oe = 5, re = 6, ie = {
  ...
  __name: "northarrow",
  setup(S) {
    const t = z(), o = y(), e = j("iApi"), f = r(), n = E(() => o.arrowIcon), M = E(() => o.poleIcon), p = r(0), s = r(0), c = r(!1), P = r(`<svg xmlns="http://www.w3.org/2000/svg" fit=""  width="25" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" focusable="false">
    ...
    X(() => {
      ...
      $.value = i.tileSchemas, n?.value && (P.value = `<img width='25' src='${n.value}'>`), e.geo.map.esriView?.ready && x(e.geo.map.getExtent()), A.push(e.event.on(G.MAP_EXTENTCHANGE, Y(x, 300)));
    }), F(() => {
    ...
    const x = async (i) => {
      ...
      for (const d of $.value)
        if (_?.tileSchemaId === d.id) {
          h = d?.hasNorthPole;
          break;
        }
      const l = e.$vApp.$el.querySelector(".inner-shell"), m = f.value.querySelector(".northarrow").getBoundingClientRect().width, u = e.$vApp.$el.querySelector(".appbar")?.clientWidth || 0, I = i.sr;
      if (h || typeof h > "u" && !I.isWebMercator()) {
        const d = new T("pole", { x: -96, y: 90 }), L = await e.geo.proj.projectGeometry(I, d), v = e.geo.map.mapPointToScreenPoint(L);
        if (v.screenY < 0) {
          c.value = !0;
          const a = {
          ...
          ) * 180 / Math.PI, s.value = l.clientWidth / 2 + l.clientHeight * Math.tan(p.value * Math.PI / 180) - m / 2, s.value = Math.max(
            u - m / 2,
            Math.min(e.geo.map.getPixelWidth() - m / 2, s.value)
          );
        ...
        c.value = !0, p.value = 0, s.value = u + (l.clientWidth - u - m) / 2;
    };
    return (i, h) => (D(), V("div", {
