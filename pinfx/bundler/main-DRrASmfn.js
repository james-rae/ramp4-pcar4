import { n as e, t } from "./config-RKp-tUX3.js";
import { t as n } from "./_plugin-vue_export-helper-O4QQSku4.js";
import { t as r } from "./keyboard-Caaq63sc.js";
import { t as i } from "./store-Dd9FSKbm.js";
...
import { execute as h } from "@arcgis/core/geometry/operators/areaOperator";
import { execute as ee } from "@arcgis/core/geometry/operators/centroidOperator";
import { execute as te, isLoaded as ne, load as re } from "@arcgis/core/geometry/operators/geodeticAreaOperator";
import { execute as ie, isLoaded as ae, load as oe } from "@arcgis/core/geometry/operators/geodeticLengthOperator";
import * as se from "@arcgis/core/geometry/operators/intersectsOperator";
import { execute as ce } from "@arcgis/core/geometry/operators/lengthOperator";
import { fromJSON as le } from "@arcgis/core/geometry/support/jsonUtils";
import ue from "@arcgis/core/request";
import de from "@arcgis/core/symbols/PictureMarkerSymbol";
import fe from "@arcgis/core/symbols/SimpleFillSymbol";
import pe from "@arcgis/core/symbols/SimpleLineSymbol";
import me from "@arcgis/core/symbols/SimpleMarkerSymbol";
import he from "@arcgis/core/symbols/TextSymbol";
import { fromJSON as ge } from "@arcgis/core/symbols/support/jsonUtils";
import "@arcgis/map-components/components/arcgis-sketch";
import "@arcgis/map-components/components/arcgis-swipe";
import { merge as _e } from "es-toolkit";
import { applyConverter as ve } from "@terraformer/spatial";
import ye from "proj4";
import { TinyEmitter as be } from "tiny-emitter";
import { createPinia as xe, defineStore as g, storeToRefs as Se } from "pinia";
import { Fragment as Ce, Teleport as we, Transition as Te, TransitionGroup as Ee, computed as _, createApp as De, createBlock as v, createCommentVNode as y, createElementBlock as b, createElementVNode as x, createTextVNode as Oe, createVNode as S, defineAsyncComponent as ke, defineComponent as C, getCurrentInstance as Ae, h as je, inject as Me, markRaw as w, mergeProps as Ne, nextTick as Pe, normalizeClass as T, normalizeStyle as Fe, onBeforeUnmount as Ie, onMounted as Le, onUpdated as Re, openBlock as E, reactive as ze, ref as D, render as Be, renderList as Ve, renderSlot as He, resolveComponent as O, resolveDirective as k, resolveDynamicComponent as Ue, toDisplayString as A, toRaw as We, unref as j, useCssVars as Ge, useTemplateRef as Ke, vShow as qe, watch as Je, withCtx as M, withDirectives as N, withKeys as Ye, withModifiers as Xe } from "vue";
import { debounce as Ze, throttle as Qe } from "es-toolkit/function";
import { createI18n as $e, useI18n as P } from "vue-i18n";
import et from "screenfull";
import { cloneDeep as tt } from "es-toolkit/object";
import nt from "animejs";
import { createPopper as rt, detectOverflow as it } from "@popperjs/core";
import "tippy.js/animations/scale.css";
import at, { setDefaultProps as ot, useTippy as st } from "vue-tippy";
import ct from "linkify-html";
import lt from "redaxios";
import ut from "await-to-js";
import { geojsonToArcGIS as dt } from "@terraformer/arcgis";
import { csv2geojson as ft, dsv as pt } from "csv2geojson";
import mt from "svg.js";
...
var ht = /* @__PURE__ */ function(e) {
	...
	id;
	constructor(e, t, n) {
		this.geometry = e, t ? this.id = t : this.id = Js.sharedUtils.generateUUID(), n ? this.attributes = n : this.attributes = {};
	}
	_hover;
	get hover() {
		return this._hover;
	...
	invalid() {
		return this.type === F.NONE || this.type === F.UNKNOWN;
	}
	geoJsonFactory(e, t) {
		let n = {
			type: e,
...
}, kt = class e extends wt {
	rawArray;
	constructor(t, n, r, i) {
		if (super(t, n.sr || r), i) this.rawArray = e.arrayDeepCopy(n);
		else if (n instanceof e) this.rawArray = n.toArray();
		else if (n instanceof Et) this.rawArray = [n.toArray()];
		else if (Array.isArray(n)) {
			if (n.length === 0) throw Error("no lines provided");
			this.rawArray = n.map((e) => Et.parsePointSet(e));
		} else throw Error("invalid lines format for MulitLineString");
	}
	get lineArray() {
		return this.rawArray.map((e, t) => new Ot(this.childIdGenerator(t), e, this.sr, !0));
	}
	getAt(e) {
		return new Ot(this.childIdGenerator(e), this.rawArray[e], this.sr, !0);
	}
	updateAt(e, t) {
		this.rawArray[t] = Et.parsePointSet(e);
	}
	get length() {
		return this.rawArray.length;
	}
	...
	get ringArray() {
		return this.rawArray.map((e, t) => new At(this.childIdGenerator(t), e, this.sr, !0));
	}
	get type() {
		return F.POLYGON;
	}
	...
	static parsePolygon(t) {
		...
		if (t instanceof kt) n = t.toArray();
		else if (t instanceof Et) n = [t.toArray()];
		else if (Array.isArray(t)) {
			if (t.length === 0) throw Error("no rings provided");
			n = t.map((e) => Et.parsePointSet(e));
		} else throw Error("invalid input format for parsePolygon");
		return n.forEach((e) => At.closeRing(e)), n;
	}
	static arrayDeepCopy(e) {
		return e.map((e) => e.map((e) => e.slice()));
	}
	...
	get polygonArray() {
		return this.rawArray.map((e, t) => new jt(this.childIdGenerator(t), e, this.sr, !0));
	}
	get type() {
	...
	static parseMultiPolygon(t) {
		if (t instanceof e) return t.toArray();
		if (t instanceof jt) return [t.toArray()];
		if (t instanceof kt || t instanceof Et) return [jt.parsePolygon(t)];
		if (Array.isArray(t)) {
			if (t.length === 0) throw Error("no polygons provided");
			return t.map((e) => jt.parsePolygon(e));
	...
	toPolygon() {
		return new jt(this.id, this.toPolygonArray(), this.sr, !0);
	}
	...
	_angle;
	constructor(e) {
		if (e ||= { style: gt.CIRCLE }, super(), this._style = e.style || gt.CIRCLE, e.style === gt.ICON) {
			let t = e;
			this._icon = t.icon || "", this._height = Ft.convToPoints(t.height) || 16.5, this._width = Ft.convToPoints(t.width) || 16.5, this._size = 0, this._colour = new Pt(), this._outline = new Lt(), this._path = "";
		} else {
			let t = e;
			this._size = Ft.convToPoints(t.size) || 12, this._colour = new Pt(t.colour ?? "#ffffff40"), this._outline = new Lt(t.outline), this._path = this._style === gt.PATH && t.path || "", this._height = 0, this._width = 0, this._icon = "";
		}
		this._xOffset = Ft.convToPoints(e.xOffset) || 0, this._yOffset = Ft.convToPoints(e.yOffset) || 0, this._angle = e.angle || 0;
	}
	get colour() {
	...
	propGrouse(e) {
		this._style === gt.ICON !== e && console.warn(`Accessed a point style property that is invalid for the style type ${this._style}`);
	}
	toOptions() {
		let e = {
	...
	static fromESRI(t) {
		...
		if (t.type === "simple-marker") {
			let e = n;
			e.style = t.style, e.colour = t.color.toRgba(), e.size = t.size, e.path = t.path, e.outline = Lt.fromESRI(t.outline).toOptions();
		} else {
			let e = n;
			e.style = gt.ICON, e.width = t.width, e.height = t.height, e.icon = t.url;
		}
		return new e(n);
	}
	static fromArcServer(t) {
		return e.fromESRI(ge(t));
	}
	static isImageUrl(e) {
	...
	_colour;
	constructor(e) {
		e ||= {}, super(), this._style = e.style || _t.SOLID;
		let t = Ft.convToPoints(e.width);
		this._width = t === void 0 || t < 0 ? .75 : t, this._colour = new Pt(e.colour), this._miter = e.miter ?? 2, this._cap = e.cap || yt.ROUND, this._join = e.join || vt.ROUND;
	}
	get styleType() {
		return this._style;
	}
...
}, Rt = class e extends Ft {
	_outlineStyle;
	_fillColour;
	_fillStyle;
	constructor(e) {
		super(), e ||= {}, e.fill ||= {}, this._fillColour = new Pt(e.fill.colour), this._fillStyle = e.fill.style || bt.SOLID, this._outlineStyle = new Lt(e.outline);
	}
	get fillColour() {
	...
	toESRI() {
		let e = this.outline.toESRI(), t = new a(this.fillColour.rgba), n = new fe();
	...
	static fromESRI(t) {
		return new e({
			...
			outline: Lt.fromESRI(t.outline).toOptions()
		});
	}
	static fromArcServer(t) {
		return e.fromESRI(ge(t));
	...
	geomEsriToRamp(e, t) {
		switch (e.type) {
			...
			case "polyline": {
				let n = e;
				return n.paths.length === 1 ? Ot.fromESRI(n, t) : kt.fromESRI(n, t);
			}
			case "polygon": return jt.fromESRI(e, t);
			case "extent": return Nt.fromESRI(e, t);
			case "multipoint": return Dt.fromESRI(e, t);
			default: throw Error(`Encountered unhandled geometry type ${e.type}`);
		}
	}
	geomGeoJsonToRamp(e, t) {
		switch (e.type) {
			case z.POINT: return J.fromGeoJSON(e, t);
			case z.LINESTRING: return Ot.fromGeoJSON(e, t);
			case z.POLYGON: return jt.fromGeoJSON(e, t);
			case z.MULTIPOINT: return Dt.fromGeoJSON(e, t);
			case z.MULTILINESTRING: return kt.fromGeoJSON(e, t);
			case z.MULTIPOLYGON: return Mt.fromGeoJSON(e, t);
			default: throw Error(`Encountered unhandled geometry type ${e.type}`);
		}
	}
	geomRampToGeoJson(e) {
	...
	graphicGeoJsonToRamp(e, t) {
		...
		return Object.keys(e.properties.forEach((t) => r[t] = e.properties[t])), new Ct(n, "", r);
	}
	graphicRampToEsri(e) {
		let t = {
			attributes: {},
	...
	styleEsriToRamp(e) {
		switch (e.type) {
			case "picture-marker":
			case "simple-marker": return It.fromESRI(e);
			case "simple-line": return Lt.fromESRI(e);
			case "simple-fill": return Rt.fromESRI(e);
			default: return console.error(`Unsupported ESRI symbol type encountered: ${e.type}`), new It();
		}
	}
	serverGeomTypeToRampGeomType(e) {
		if (!e) return F.NONE;
		switch (e) {
			case "esriGeometryPolygon": return F.POLYGON;
			case "esriGeometryPolyline": return F.LINESTRING;
			case "esriGeometryPoint": return F.POINT;
			case "esriGeometryMultipoint": return F.MULTIPOINT;
			case "esriGeometryEnvelope": return F.EXTENT;
			default: return console.error(`Unrecognized server geometry type encountered: ${e}`), F.UNKNOWN;
		}
	}
	clientGeomTypeToRampGeomType(e) {
		if (!e) return F.NONE;
		switch (e) {
			case "polygon": return F.POLYGON;
			case "polyline": return F.LINESTRING;
			case "point": return F.POINT;
			case "multipoint": return F.MULTIPOINT;
			default: return console.error(`Unrecognized client geometry type encountered: ${e}`), F.UNKNOWN;
		}
	}
	geoJsonGeomTypeToEsriGeomType(e) {
		switch (e) {
			case z.POINT: return "point";
			case z.LINESTRING:
			case z.MULTILINESTRING: return "polyline";
			case z.POLYGON:
			case z.MULTIPOLYGON: return "polygon";
			case z.MULTIPOINT: return "multipoint";
			default: throw Error(`Encountered unhandled geometry type ${e}`);
		}
	}
	isImageUrl(e) {
		return It.isImageUrl(e);
	}
}, Bt = class {
	layerIdx;
	name;
	...
	static fromConfig(t) {
		return new e(t.id, Nt.fromConfig(`${t.id}-extent-default`, t.default), t.full === void 0 ? void 0 : Nt.fromConfig(`${t.id}-extent-full`, t.full), t.maximum === void 0 ? void 0 : Nt.fromConfig(`${t.id}-extent-maximum`, t.maximum));
	}
	clone() {
	...
	updateQuery(e) {
		let t = _e({}, this.queryMap);
		return _e(t, e), `${this.base}${Object.entries(t).filter(([, e]) => e !== void 0).map(([e, t], n) => `${n === 0 ? "?" : ""}${e}=${t}`).join("&")}`;
	}
}, Kt = "EPSG:4326", qt = class {
	espgWorker;
	constructor() {
		this.espgWorker = this.defaultEpsgLookup, ye.defs("EPSG:3978", "+proj=lcc +lat_1=49 +lat_2=77 +lat_0=49 +lon_0=-95 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"), ye.defs("EPSG:3979", "+proj=lcc +lat_1=49 +lat_2=77 +lat_0=49 +lon_0=-95 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"), ye.defs("EPSG:54004", "+proj=merc +lon_0=0 +k=1 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"), ye.defs("EPSG:102100", ye.defs("EPSG:3857")), ye.defs("EPSG:102187", "+proj=tmerc +lat_0=0 +lon_0=-114 +k=0.9999 +x_0=0 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"), ye.defs("EPSG:102190", "+proj=aea +lat_1=50 +lat_2=58.5 +lat_0=45 +lon_0=-126 +x_0=1000000 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs");
		let e = 1;
		for (; e <= 60;) {
			let t = e < 10 ? `0${e}` : e;
			ye.defs(`EPSG:326${t}`, `+proj=utm +zone=${e} +ellps=WGS84 +datum=WGS84 +units=m +no_defs`), e++;
		}
	}
	addProjection(e, t) {
		e = typeof e == "number" ? `EPSG:${e}` : e, ye.defs(e, t);
	}
	defaultEpsgLookup(e) {
		let t = String(e).match(/urn:ogc:def:crs:EPSG::(\d+)/) || String(e).match(/EPSG:(\d+)/) || [];
		if (t.length < 2) throw Error("Invalid code provided.");
		return new Promise((e, n) => {
			ue(`https://epsg.io/${t[1]}.proj4`, { responseType: "text" }).then((t) => {
				t.data ? e(t.data) : n();
			}, (e) => {
				n(e);
			});
	...
	async checkProj(e) {
		...
		let r = (e, t) => {
			e !== t && ye.defs(t, ye.defs(e));
		};
		if (ye.defs(t)) return !0;
		if (n && ye.defs(n)) return r(n, t), !0;
		let i = async (e) => {
			try {
				let t = await this.epsgLookup(e);
				return t === null || t === "" ? !1 : (ye.defs(e, t), !0);
			} catch {
				return !1;
			}
		};
	...
	async projectGeoJson(e, t, n) {
		let r, i;
		if (r = t ? this.normalizeProj(t) : q.parseGeoJsonCrs(e.crs), i = n ? this.normalizeProj(n) : Kt, i === r) return e;
		await this.checkProjBomber([r, i]);
		let a = ye(r, i).forward;
		return ve(e, a);
	}
	async projectGeometry(e, t) {
		if (t.type === F.EXTENT) return this.projectExtent(e, t);
		await this.checkProjBomber([e, t.sr]);
		let n = t.toGeoJSON(), r = await this.projectGeoJson(n, this.normalizeProj(t.sr), this.normalizeProj(e)), i = Js.geom.geomGeoJsonToRamp(r, t.id);
	...
	async projectExtent(e, t) {
		...
		let a = new jt("warpy", [i], t.sr, !0), o = await this.projectGeometry(e, a), s = o.toArray().pop() || [], c = s.map((e) => e[0]), l = s.map((e) => e[1]), u = Math.min.apply(null, c), d = Math.max.apply(null, c), f = Math.min.apply(null, l), p = Math.max.apply(null, l);
		return Nt.fromParams(t.id + "_projected", u, f, d, p, o.sr);
	}
}, Jt = class {
	...
	sharedUtils;
	constructor() {
		this.proj = new qt(), this.geom = new zt(), this.sharedUtils = new Wt();
	}
	defaultTileSchemas() {
...
var $t = class {
	...
	let e = D({}), t = D([]), n = D([]), r = _(() => t.value.map((t) => t.map((t) => e.value[t]).filter((e) => {
	...
	let e = D([]), t = D({}), n = D({}), r = D(), i = D(0), a = D(0), o = D(!0), s = D();
	function c(t) {
	...
	function r(n) {
		if (e.value = {
			...e.value,
			[n.id]: w(n)
		}, n.id in t.value) t.value[n.id].resolveMe(w(n));
		else {
			let e = new Y();
			e.resolveMe(w(n)), t.value = {
				...t.value,
				[n.id]: e
			};
		}
	...
	_funCounter;
	constructor(e) {
		super(e), this._eventBus = new be(), this._eventRegister = [], this._funCounter = 1, this._nameRegister = Object.values(Z).filter((e) => typeof e == "string" && e.indexOf("/") > -1);
	}
	findHandler(e) {
	...
	defaultHandlerFactory(e) {
		...
		switch (e) {
			...
			case Q.LAYER_REMOVE_CHECKS_GRID:
				n = (e) => {
					if (this.$iApi.fixture.get("grid")) {
						let t = tn(this.$vApp.$pinia), n = t.getGridId(e.id);
						if (n === void 0) return;
			...
			case Q.MAP_BASEMAP_CHECKS_TILE_PROJ:
				n = (e) => {
					e.schemaChanged && this.$iApi.geo.layer.allLayers().filter((e) => e.dataFormat === R.ESRI_TILE && e.schemaLocked).forEach((e) => {
						e.checkProj();
					});
			...
			case Q.MAP_CREATED_INITIALIZES_FIXTURES:
				n = () => {
					let e = on(this.$vApp.$pinia).items;
			...
			case Q.MAP_EXTENT_UPDATES_MAPTIP:
				...
				}, this.$iApi.event.on(Z.MAP_EXTENTCHANGE, Qe(() => n(), 50, { edges: ["leading"] }), e);
				break;
			...
			case Q.MAP_KEYDOWN_UPDATES_COORDS:
				this.$iApi.event.on(Z.MAP_KEYDOWN, Qe(() => {
					let e = nn(this.$vApp.$pinia);
			...
			case Q.MAP_MOUSE_UPDATES_COORDS:
				this.$iApi.event.on(Z.MAP_MOUSEMOVE, Qe((e) => {
					let t = nn(this.$vApp.$pinia);
			...
			case Q.MAP_MOUSE_UPDATES_MAPTIP:
				...
				}, this.$iApi.event.on(Z.MAP_MOUSEMOVE, Qe((e) => n(e), 50), e);
				break;
			case Q.MAP_MOUSEDOWN_UPDATES_MAPTIP:
				...
				}, this.$iApi.event.on(Z.MAP_MOUSEDOWN, Qe((e) => {
					let t = {
						screenX: e.offsetX,
						screenY: e.offsetY,
						button: e.button,
			...
			case Q.MAP_RESIZE_UPDATES_SCALEBAR:
				this.$iApi.event.on(Z.MAP_RESIZED, Ze(() => this.$iApi.geo.map.caption.updateScale(), 100), e);
				break;
			case Q.MAP_SCALE_UPDATES_SCALEBAR:
				this.$iApi.event.on(Z.MAP_SCALECHANGE, Ze(() => this.$iApi.geo.map.caption.updateScale(), 300), e);
				break;
			case Q.PANEL_CLOSE_UPDATES_APPBAR:
				n = (e) => {
					let t = en(this.$vApp.$pinia);
...
function pn() {
	let e = document.documentElement.getAttribute("lang") || un;
	return dn.includes(e) || (e = un), $e({
		legacy: !1,
		locale: e,
		fallbackLocale: un,
		globalInjection: !0,
...
var mn = g("maptip", () => {
	...
	__name: "esri-map",
	setup(e) {
		let t = mn(), n = Me("iApi"), r = _(() => t.maptipPoint), i = _(() => t.maptipInstance), a = _(() => t.content), o = ze([]), s = D(!1);
		o.push(Je(r, () => {
			if (r.value) {
	...
	constructor(e) {
		this.resizeObserver = new ResizeObserver((t) => {
			t.length && window.requestAnimationFrame(() => {
				t.forEach((t) => {
					let n = t.target.dataset.breakpoints, r = n ? JSON.parse(n) : e ?? gn;
	...
	setup(e) {
		let t = D(null), n = e, r = D(!1);
		Le(() => {
			n.panel.teleport && new _n({
				xs: 0,
				sm: 461,
		...
		return (n, r) => {
			let i = k("focus-container");
			return E(), b("div", {
				class: T(["shadow-tm bg-white h-full xs:mr-0 sm:mr-12 last:mr-0 pointer-events-auto min-w-0 shrink-0", e.panel.expanded ? "flex-grow max-w-full" : ""]),
				style: Fe(e.panel.style),
				"data-cy": e.panel.id,
				ref_key: "componentEl",
				ref: t
			}, [S(Te, {
				...
				default: M(() => [N((E(), v(Ue(e.panel.route.screen), Ne({ class: "h-full" }, e.panel.route.props, { panel: e.panel }), null, 16, ["panel"])), [[i]])]),
				_: 1
			})], 14, vn);
		};
	}
}), bn = g("panel", () => {
	let e = D(void 0), t = D(void 0), n = D(0), r = D(0), i = D(!1), a = D(!0), o = D({}), s = D({}), c = D([]), l = D([]), u = D([]), d = D(1), f = _(() => r.value);
	function p(e) {
...
}), xn = /* @__PURE__ */ n(/* @__PURE__ */ C({
	__name: "panel-stack",
	setup(e) {
		Ge((e) => ({ v3d36ee4c: i.value }));
		let t = bn(), n = Me("iApi"), r = D(), i = _(() => t.opacity), a = _(() => t.mobileView);
		Le(() => {
		...
		return (e, t) => (E(), v(Ee, {
			...
			default: M(() => [(E(!0), b(Ce, null, Ve(o(j(n).screenSize), (e) => (E(), v(yn, {
				key: `${e.id}`,
				panel: e
			}, null, 8, ["panel"]))), 128))]),
			_: 1
...
}), [["__scopeId", "data-v-717e4bd7"]]), Sn = ["content", "aria-label"], Cn = /* @__PURE__ */ C({
	...
	setup(e) {
		let t = D(!1), n = D(null), r = ze([]), i = D(), a = D(), o = D(), s = e;
		r.push(Je(t, () => {
			n.value.update();
		}));
		let c = () => {
		...
		return Le(() => {
			window.addEventListener("click", (e) => {
				...
				(!i.value || !i.value.contains(e.target)) && (t.value = !1);
			}), o.value.addEventListener("focus", l), o.value.addEventListener("blur", u), o.value.addEventListener("mouseover", l), o.value.addEventListener("mouseleave", u), Pe(() => {
				o.value && a.value && (n.value = rt(o.value, a.value, {
			...
			return E(), b("div", {
				...
				class: T(["rv-dropdown shadow-md border border-gray:200 py-8 bg-white rounded z-10", { "text-center": e.centered }]),
				ref_key: "dropdown",
				ref: a
			}, [He(r.$slots, "default", { close: () => t.value = !t.value })], 2), [[qe, t.value]])], 512);
		};
	}
}), wn = g("notification", () => {
	let e = 0, t = D([]), n = D({}), r = _(() => t.value.length >= 99 ? 99 : t.value.length);
	function i(n) {
		t.value = [{
			...n,
			id: `notif-${e++}`
	...
	function a(e) {
		if (t.value.includes(e)) {
			...
			e instanceof jn && l(e), n > -1 && t.value.splice(n, 1);
		}
	}
	function o(e) {
		n.value[e.id] = e;
...
}), Tn = { class: "h-full flex flex-col" }, En = { class: "w-full flex mb-6" }, Dn = ["content", "aria-label"], On = /* @__PURE__ */ C({
	__name: "screen",
	props: { panel: {
		type: Object,
		required: !0
	...
	setup(e) {
		let t = wn(), { t: n } = P(), r = _(() => t.notificationNumber), i = () => t.clearAll();
		return (t, a) => {
			let o = O("panel-screen"), s = k("tippy");
			return E(), v(o, { panel: e.panel }, {
				header: M(() => [Oe(A(j(n)("notifications.title")), 1)]),
				content: M(() => [x("div", Tn, [x("div", En, [N((E(), b("button", {
					type: "button",
					onClick: i,
					class: T(["p-4 ml-auto", [r.value ? "text-gray-500 hover:text-black" : "text-gray-300 cursor-default pointer-events-none"]]),
					content: j(n)("notifications.controls.clearAll"),
					"aria-label": j(n)("notifications.controls.clearAll")
				}, [...a[0] ||= [x("svg", {
					...
					x("path", { d: "M13 17h-9a4 4 0 0 0 2 -3v-3a7 7 0 0 1 4 -6a2 2 0 1 1 4 0a7 7 0 0 1 4 6v2" }),
					x("path", { d: "M9 17v1a3 3 0 0 0 4.194 2.753" }),
					x("path", { d: "M22 22l-5 -5" }),
					x("path", { d: "M17 22l5 -5" })
				], -1)]], 10, Dn)), [[s, {
					placement: "bottom",
					theme: "ramp4",
					animation: "scale"
				}]])]), S(Vn, { class: "overflow-y-auto" })])]),
				_: 1
			}, 8, ["panel"]);
		};
	}
	...
	constructor(e) {
		super(e), this.$iApi.panel.register({
			id: "notifications",
			config: {
				screens: { "notifications-screen": w(On) },
				alertName: "notifications.title"
			}
		}), this.notificationStore = wn(this.$vApp.$pinia);
	}
	show(e, t) {
	...
	notificationStore = wn(this.$vApp.$pinia);
	id;
	message;
	type;
	messageList = [];
...
}, Mn = ["content"], Nn = { class: "flex items-center text-left" }, Pn = { class: "select-text cursor-text" }, Fn = ["content", "aria-label"], In = {
	key: 0,
	class: "text-left"
}, Ln = /* @__PURE__ */ C({
	...
	setup(e, { emit: t }) {
		...
		return (t, i) => {
			let c = k("tippy");
			return N((E(), b("li", {
				class: T(["flex-col default-focus-style p-4", e.notification.messageList ? "cursor-pointer" : ""]),
				content: j(n)(a.value ? "notifications.controls.collapse" : "notifications.controls.expand"),
				onClick: i[1] ||= (e) => a.value = !a.value
			}, [x("div", Nn, [
				x("span", null, [Oe(A(o[e.notification.type]) + " ", 1), x("span", Pn, A(e.notification.message), 1)]),
				i[4] ||= x("span", { class: "flex-grow" }, null, -1),
				e.notification.messageList ? (E(), b("div", {
					key: 0,
					class: T(["dropdown-icon p-4 pointer-events-none", { "transform -rotate-180": a.value }])
				}, [...i[2] ||= [x("svg", {
					xmlns: "http://www.w3.org/2000/svg",
					height: "24",
					viewBox: "0 0 24 24",
					width: "24"
				}, [x("path", { d: "M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6 1.41-1.41z" })], -1)]], 2)) : y("", !0),
				N((E(), b("button", {
					type: "button",
					onClick: i[0] ||= Xe(() => r("remove"), ["stop"]),
					class: "mx-4 p-4 text-gray-500 hover:text-black",
					content: j(n)("notifications.controls.dismiss"),
					"aria-label": j(n)("notifications.controls.dismiss")
				}, [...i[3] ||= [x("svg", {
					class: "fill-current",
					height: "16",
					width: "16",
					...
					"fill-rule": "evenodd"
				}, [x("path", { d: "M11.17,37.16H94.65a8.4,8.4,0,0,1,2,.16,5.93,5.93,0,0,1,2.88,1.56,5.43,5.43,0,0,1,1.64,3.34,7.65,7.65,0,0,1-.06,1.44L94,117.31v0l0,.13,0,.28v0a7.06,7.06,0,0,1-.2.9v0l0,.06v0a5.89,5.89,0,0,1-5.47,4.07H17.32a6.17,6.17,0,0,1-1.25-.19,6.17,6.17,0,0,1-1.16-.48h0a6.18,6.18,0,0,1-3.08-4.88l-7-73.49a7.69,7.69,0,0,1-.06-1.66,5.37,5.37,0,0,1,1.63-3.29,6,6,0,0,1,3-1.58,8.94,8.94,0,0,1,1.79-.13ZM5.65,8.8H37.12V6h0a2.44,2.44,0,0,1,0-.27,6,6,0,0,1,1.76-4h0A6,6,0,0,1,43.09,0H62.46l.3,0a6,6,0,0,1,5.7,6V6h0V8.8h32l.39,0a4.7,4.7,0,0,1,4.31,4.43c0,.18,0,.32,0,.5v9.86a2.59,2.59,0,0,1-2.59,2.59H2.59A2.59,2.59,0,0,1,0,23.62V13.53H0a1.56,1.56,0,0,1,0-.31v0A4.72,4.72,0,0,1,3.88,8.88,10.4,10.4,0,0,1,5.65,8.8Zm42.1,52.7a4.77,4.77,0,0,1,9.49,0v37a4.77,4.77,0,0,1-9.49,0v-37Zm23.73-.2a4.58,4.58,0,0,1,5-4.06,4.47,4.47,0,0,1,4.51,4.46l-2,37a4.57,4.57,0,0,1-5,4.06,4.47,4.47,0,0,1-4.51-4.46l2-37ZM25,61.7a4.46,4.46,0,0,1,4.5-4.46,4.58,4.58,0,0,1,5,4.06l2,37a4.47,4.47,0,0,1-4.51,4.46,4.57,4.57,0,0,1-5-4.06l-2-37Z" })], -1)]], 8, Fn)), [[c, {
					theme: "ramp4",
					animation: "scale"
				}]])
			]), e.notification.messageList && a.value ? (E(), b("div", In, [(E(!0), b(Ce, null, Ve(e.notification.messageList, (t, n) => (E(), b("p", { key: e.notification.id + t + n }, A(t), 1))), 128))])) : y("", !0)], 10, Mn)), [[c, {
				onShow: s,
				theme: "ramp4",
				animation: "scale"
			}]]);
	...
	setup(e) {
		...
		let c = _(() => t.notificationStack);
		return (e, t) => {
			let a = k("focus-item"), o = k("focus-list"), l = k("tippy");
			return E(), b("div", null, [c.value.length > 0 ? N((E(), b("ul", {
				key: 0,
				content: j(n)("panels.controls.items"),
				ref_key: "el",
				ref: r,
				class: "h-full overflow-y-auto"
			}, [(E(!0), b(Ce, null, Ve(c.value, (e, t) => (E(), b(Ce, { key: e.id }, [t > 0 ? (E(), b("div", zn)) : y("", !0), N(S(Ln, {
				ref_for: !0,
				...
				t[0] ||= x("span", { class: "flex-grow" }, null, -1),
				t[1] ||= x("svg", {
					xmlns: "http://www.w3.org/2000/svg",
					viewBox: "0 0 24 24",
					...
					d: "M0 0h24v24H0z",
					fill: "none"
				}), x("path", { d: "M18 7l-1.41-1.41-6.34 6.34 1.41 1.41L18 7zm4.24-1.41L11.66 16.17 7.48 12l-1.41 1.41L11.66 19l12-12-1.42-1.41zM.41 13.41L6 19l1.41-1.41L1.83 12 .41 13.41z" })], -1),
				x("span", null, A(j(n)("notifications.empty")), 1),
				t[2] ||= x("span", { style: { "flex-grow": "6" } }, null, -1)
			]))]);
		};
	}
}), Hn = { class: "flex items-center hover:text-white" }, Un = ["aria-label"], Wn = {
	key: 0,
	class: "number rounded-full w-18 text-white"
}, Gn = { class: "notification-dropdown pointer-events-auto bg-white rounded text-center text-black w-500 h-256 flex flex-col p-0" }, Kn = { class: "pb-8 border-b border-gray-600" }, qn = { class: "absolute flex right-3 top-3" }, Jn = ["content", "aria-label"], Yn = /* @__PURE__ */ n(/* @__PURE__ */ C({
	__name: "caption-button",
	setup(e) {
		let t = wn(), { t: n } = P(), r = _(() => t.notificationNumber), i = () => t.clearAll();
		return (e, t) => {
			let a = O("close"), o = k("tippy");
			return E(), v(Cn, {
				position: "top-start",
				tooltip: j(n)("notifications.title"),
				tooltipPlacement: "top",
				class: "pointer-events-auto sm:flex ml-4 mr-8"
			}, {
				header: M(() => [x("div", Hn, [(E(), b("svg", {
					class: "fill-current w-24 h-24",
					xmlns: "http://www.w3.org/2000/svg",
					viewBox: "0 0 24 24",
					"aria-label": j(n)("notifications.open")
				}, [...t[0] ||= [x("path", { d: "M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" }, null, -1)]], 8, Un)), r.value && r.value > 0 ? (E(), b("span", Wn, A(r.value), 1)) : y("", !0)])]),
				default: M((e) => [x("div", Gn, [x("div", null, [x("h4", Kn, A(j(n)("notifications.title")), 1), x("div", qn, [N((E(), b("button", {
					type: "button",
					onClick: i,
					class: T(["p-4 mr-6", [r.value ? "text-gray-500 hover:text-black" : "text-gray-300 cursor-default pointer-events-none"]]),
					content: j(n)("notifications.controls.clearAll"),
					"aria-label": j(n)("notifications.controls.clearAll")
				}, [...t[1] ||= [x("svg", {
					xmlns: "http://www.w3.org/2000/svg",
					...
					animation: "scale",
					appendTo: "parent"
				}]]), S(a, { onClick: e.close }, null, 8, ["onClick"])])]), S(Vn, { class: "overflow-y-auto h-230" })])]),
				_: 1
			}, 8, ["tooltip"]);
		};
	}
}), [["__scopeId", "data-v-3623a7d2"]]), Xn = { class: "flex hover:text-white" }, Zn = ["aria-label"], Qn = { class: "about-ramp-dropdown pointer-events-auto bg-white rounded w-256 h-50" }, $n = ["aria-label", "content"], er = { class: "pb-8 pt-1 border-b border-gray-600 mb-10" }, tr = { class: "absolute right-5 top-5" }, nr = { class: "select-text" }, rr = { class: "font-bold cursor-text" }, ir = { class: "text-sm cursor-text" }, ar = { class: "font-bold cursor-text" }, or = { class: "ml-5 cursor-text" }, sr = { class: "mt-5" }, cr = ["href"], lr = /* @__PURE__ */ n(/* @__PURE__ */ C({
	...
	setup(e) {
		let { t } = P(), n = Me("iApi"), r = () => {
			n.reload();
		}, i = _(() => `${Ws.major}.${Ws.minor}.${Ws.patch}`), a = _(() => Ws.hash.slice(0, 9)), o = _(() => {
			let e = new Date(Ws.timestamp);
		...
		return (n, s) => {
			let c = O("close"), l = O("dropdown-menu"), u = k("tippy"), d = k("focus-item");
			return N((E(), v(l, {
				class: "pointer-events-auto sm:flex",
				position: e.position,
				tooltip: j(t)("ramp.about"),
				tooltipPlacement: e.position
			}, {
				header: M(() => [x("div", Xn, [(E(), b("svg", {
					xmlns: "http://www.w3.org/2000/svg",
					viewBox: "0 0 24 24",
					class: "fill-current w-20 h-20",
					"aria-label": j(t)("ramp.about.open")
				}, [...s[0] ||= [x("path", { d: "M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" }, null, -1)]], 8, Zn))])]),
				default: M((e) => [x("div", Qn, [x("div", null, [
					N((E(), b("button", {
						class: "absolute left-5 top-5 text-gray-500 hover:text-black focus:text-black p-8",
						onClick: r,
						"aria-label": j(t)("ramp.reload"),
						content: j(t)("ramp.reload")
					}, [...s[1] ||= [x("svg", {
						class: "fill-current w-16 h-16",
						xmlns: "http://www.w3.org/2000/svg",
						viewBox: "0 0 24 24"
					}, [x("path", { d: "M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z" })], -1)]], 8, $n)), [[u, {
						placement: "bottom-start",
						theme: "ramp4",
						animation: "scale"
					}]]),
					x("h4", er, A(j(t)("ramp.about")), 1),
					x("div", tr, [S(c, { onClick: e.close }, null, 8, ["onClick"])]),
					x("div", nr, [
						x("div", null, [x("span", rr, A(i.value), 1), x("span", ir, " [" + A(a.value) + "] ", 1)]),
						x("div", null, [x("span", ar, A(o.value[0]), 1), x("span", or, A(o.value[1]), 1)]),
						s[3] ||= x("div", { class: "mt-5" }, [x("a", {
...
}), [["__scopeId", "data-v-cf71f458"]]), ur = "__HELP__", dr = g("keyboardnav", () => {
	let e = D(), t = D({}), n = D(!1), r = D([]), i = D(), a = D("idle"), o = ["H", "?"];
	function s() {
	...
	function d(e, t, n) {
		if (e) {
			...
			if (e === "reset") {
				let e = n ?? se.activeNamespace ?? se.keyChain[1];
				if (!e) {
					h({ suppressHandler: !0 });
					return;
				}
				ee(["S", e]), re(void 0), ie("awaitAction"), se.activeNamespace = e;
			}
		}
	}
	function f(n, r, i) {
		e.value = n, !i?.suppressHandler && t.value[n]?.activeHandler?.(se, r);
	}
	function p(n, r) {
		e.value && !r?.suppressHandler && t.value[e.value]?.deactiveHandler?.(se, n), e.value = void 0;
	}
	function m(n, r) {
		let i = e.value;
		if (!i) return;
	...
	return oe;
}), fr = { class: "map-caption absolute bottom-0 h-29 sm:h-38 flex justify-end pointer-events-auto cursor-default select-none text-gray-200 bg-black-75 left-0 right-0 py-2 sm:py-6" }, pr = {
	key: 0,
	class: "relative ml-10 top-2 text-sm sm:text-base font-mono flex items-center gap-2",
	"aria-live": "polite"
}, mr = { class: "chain-content" }, hr = { class: "chain-keys" }, gr = {
	key: 0,
	class: "chain-cursor"
}, _r = {
	key: 1,
...
}, br = { class: "flex min-w-fit justify-end" }, xr = {
	key: 0,
	...
	"aria-label",
	"content"
], Cr = { class: "relative top-1 text-sm sm:text-base" }, wr = { class: "text-gray-200 hover:text-white text-sm sm:text-base pb-5" }, Tr = ["onClick"], Er = {
	key: 0,
	class: "sr-only"
}, Dr = /* @__PURE__ */ C({
	__name: "map-caption",
	setup(e) {
		let n = nn(), r = dr(), i = t(), { t: a } = P(), o = Me("iApi"), s = _(() => n.scale), c = _(() => n.attribution), l = _(() => n.coords), u = _(() => n.langtoggle), d = _(() => i.config.map), { keyChain: f, lastAction: p, chainState: m, namespaces: h, activeNamespace: ee } = Se(r), te = _(() => {
		...
		re.push(Je(d, (e) => {
			...
			Pe(() => {
				o.$i18n.locale.value && ne.value.length == 0 && (ne.value = o.$i18n.availableLocales);
			});
		});
		let ie = (e) => {
		...
		return (e, t) => {
			let n = O("dropdown-menu"), r = k("truncate"), i = k("tippy");
			return E(), b("div", fr, [
				S(lr, {
					class: "sm:block display-none ml-8 mr-4",
					position: "top-end"
				}),
				S(Yn, { class: "sm:block display-none" }),
				te.value ? (E(), b("span", pr, [x("span", mr, [
					t[0] ||= x("span", { class: "chain-colon" }, ":", -1),
					x("span", hr, A(te.value.keys), 1),
					te.value.cursor ? (E(), b("span", gr, "_")) : y("", !0),
					te.value.options ? (E(), b("span", _r, A(te.value.options), 1)) : y("", !0)
				]), te.value.description ? (E(), b("span", vr, "- " + A(te.value.description) + "...", 1)) : y("", !0)])) : c.value?.text.disabled ? y("", !0) : N((E(), b("span", yr, [Oe(A(c.value?.text.value), 1)])), [[r, { options: {
					placement: "top",
					hideOnClick: !1,
					theme: "ramp4",
					animation: "scale"
				} }]]),
				t[1] ||= x("span", { class: "flex-grow w-15" }, null, -1),
				x("div", br, [
					l.value?.disabled ? y("", !0) : N((E(), b("div", xr, [Oe(A(l.value?.formattedString), 1)])), [[r, { options: {
						hideOnClick: !1,
						theme: "ramp4",
						animation: "scale"
					} }]]),
					s.value?.disabled ? y("", !0) : N((E(), b("button", {
						key: 1,
						type: "button",
                ...
                `,
						...
						style: Fe({ width: s.value?.width })
					}, null, 4), x("span", Cr, A(s.value?.label), 1)], 8, Sr)), [[i, {
						delay: [300, 0],
						placement: "top",
						hideOnClick: !1,
						theme: "ramp4",
					...
					u.value?.disabled ? y("", !0) : (E(), v(n, {
						key: 2,
						class: "flex-shrink-0 pointer-events-auto focus:outline-none px-4 mr-4 relative top-2",
						position: "top-end",
						tooltip: j(a)("map.changeLanguage"),
						ariaLabel: `${j(a)("map.language.short")} - ${j(a)("map.changeLanguage")}`,
						tooltipPlacement: "top-start",
						tooltipPlacementAlt: "left-end"
					}, {
						header: M(() => [x("span", wr, A(j(a)("map.language.short")), 1)]),
						default: M(() => [(E(!0), b(Ce, null, Ve(ne.value, (e, t) => (E(), b("a", {
							key: `${e}-${t}`,
							class: T(["flex-auto items-center text-sm sm:text-base cursor-pointer", { "font-bold": e === j(o).$i18n.locale.value }]),
							href: "javascript:;",
							onClick: (t) => ie(e)
						}, [Oe(A(j(a)("map.language." + e)) + " ", 1), e === j(o).$i18n.locale.value ? (E(), b("span", Er, A(j(a)("map.language.curr")), 1)) : y("", !0)], 10, Tr))), 128))]),
						_: 1
					}, 8, ["tooltip", "ariaLabel"]))
				])
			]);
...
}), Or = ["content", "aria-label"], kr = {
	key: 0,
	class: "number absolute -top-2 -right-2 text-white w-18 rounded-full"
}, Ar = /* @__PURE__ */ n(/* @__PURE__ */ C({
	__name: "floating-button",
	setup(e) {
		let t = wn(), n = Me("iApi"), { t: r } = P(), i = _(() => t.notificationNumber);
		return (e, t) => {
			let a = k("tippy");
			return N((E(), b("button", {
				type: "button",
				onClick: t[0] ||= (e) => j(n).panel.get("notifications").open(),
				...
				content: j(r)("notifications.title"),
				"aria-label": j(r)("notifications.title")
			}, [t[1] ||= x("svg", {
				class: "fill-current w-24 h-24",
				xmlns: "http://www.w3.org/2000/svg",
				viewBox: "0 0 24 24"
			}, [x("path", { d: "M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z" })], -1), i.value && i.value > 0 ? (E(), b("span", kr, A(i.value), 1)) : y("", !0)], 8, Or)), [[a]]);
		};
	}
}), [["__scopeId", "data-v-a451db5c"]]), jr = { class: "flex items-center mb-20" }, Mr = { class: "text-xl" }, Nr = /* @__PURE__ */ C({
	__name: "keyboard-instructions",
	setup(e) {
		let t = Me("iApi"), { t: n } = P(), r = D(!1), i = D([
			"app",
			"lists",
			"map",
			"keyboardNav"
		]), a = D([]), o = D(null), s = D(null);
		Le(() => {
		...
		return (e, t) => {
			...
			return r.value ? (E(), b("div", {
				...
				onClick: t[2] ||= Xe(() => {}, ["stop", "prevent"]),
				tabindex: "0",
				ref_key: "firstEl",
				ref: o
			}, [
				x("div", jr, [x("h2", Mr, A(j(n)("keyboardInstructions.title")), 1), S(a, {
					class: "ml-auto",
					onClick: t[0] ||= (e) => r.value = !1
				})]),
				(E(!0), b(Ce, null, Ve(i.value, (e) => (E(), b("p", {
					class: "whitespace-pre-line pb-10",
					key: e
				}, A(j(n)(`keyboardInstructions.${e}`)), 1))), 128)),
				x("button", {
					type: "button",
					...
					ref_key: "lastEl",
					ref: s
				}, A(j(n)("keyboardInstructions.OK")), 513)
			], 512)], 32)) : y("", !0);
		};
	}
}), Pr = g("instance", () => ({ started: D(!1) })), Fr = { class: "h-full relative" }, Ir = { class: "inner-shell absolute top-0 left-0 h-full w-full pointer-events-none" }, Lr = { class: "absolute top-8 w-full flex justify-center" }, Rr = {
	key: 1,
	class: "w-full h-full"
}, zr = C({
	name: "App",
	components: { Shell: /* @__PURE__ */ n(/* @__PURE__ */ C({
		__name: "shell",
		setup(e) {
			let t = Me("iApi"), n = Pr(), r = on(), i = bn(), { t: a } = P(), o = _(() => r.items.appbar), s = () => {
				t.event.emit("openKeyboardInstructions");
			}, c = () => i.teleported;
			return (e, t) => (E(), b("div", Fr, [
				x("div", Ir, [
					t[0] ||= x("div", { class: "sr-only screen-reader-alert" }, null, -1),
					x("div", Lr, [x("button", {
						type: "button",
						class: "bg-white hidden-until-focus z-50 shadow-md px-10",
						onClick: s
					}, A(j(a)("keyboardInstructions.open")), 1)]),
					S(Nr, { class: "keyboard-instructions-modal" }),
					S(xn, { class: "panel-stack sm:flex absolute inset-0 overflow-hidden sm:p-12 z-10 sm:pl-80 xs:pl-40 sm:pb-48 xs:pb-28 xs:pr-0 sm:pr-40" }),
					o.value ? y("", !0) : (E(), v(Ar, { key: 0 })),
					S(Dr, { class: "z-30" })
				]),
				j(n).started ? (E(), v(hn, { key: 0 })) : (E(), b("div", Rr, [...t[1] ||= [x("div", { class: "spinner relative inset-x-1/2 inset-y-9/20" }, null, -1)]])),
				(E(!0), b(Ce, null, Ve(c(), (e) => (E(), v(we, {
	...
	setup() {
		let e = Ae();
		Le(() => {
			new _n().observe(e?.proxy?.$refs["app-size"]), ot({
...
function Hr(e, t, n, r, i, a) {
	...
	return E(), b("div", {
		...
		lang: e.$i18n.locale
	}, [x("div", Vr, [S(o)], 512)], 8, Br);
}
var Ur = /* @__PURE__ */ n(zr, [["render", Hr]]), Wr = /* @__PURE__ */ function(e) {
...
}(Wr || {}), Gr = "focus-list", Kr = "focus-item", qr = "focus-icon", Jr = `[${Gr}],[focus-container]`, Yr = "truncate-text", Xr = "show-truncate", Zr = "focused", Qr = `button,input,select,a,textarea,[contenteditable],[${Gr}],[${qr}],[tabindex]`, $r = {
	mounted(e, t) {
		+e.getAttribute("tabindex") <= 0 && e.setAttribute("tabindex", "0"), e.toggleAttribute(Gr, !0), new ti(e, t.value);
	},
	updated(e) {
		ei(e);
	}
...
function ei(e) {
	e.querySelectorAll(Qr).forEach((t) => {
		if (t.closest(Jr) === e || t.closest(Jr) === t && t.parentElement.closest(Jr) === e) if (t.closest(`[${Gr}],.${Zr}`).classList.contains(Zr)) t.setAttribute("tabindex", "0");
		else {
...
var ti = class {
	...
	setTabIndex(e, t = this.element) {
		t.querySelectorAll(Qr).forEach((t) => {
			(e === -1 || t.closest(Jr) === this.element || t.closest(Jr) === t && t.parentElement.closest(Jr) === this.element || t.closest(`[${Gr}],.${Zr}`).classList.contains(Zr)) && t.setAttribute("tabindex", e.toString());
		});
	}
	defocusItem(e) {
		e.classList.remove(Zr), this.setTabIndex(-1, e), e._tippy && e._tippy.hide(), e.getAttribute(Kr) === Xr && e.querySelector(`[${Yr}]`)?._tippy?.hide();
	}
	focusItem(e) {
		e.classList.add(Zr), this.setAriaActiveDescendant(e), this.setTabIndex(0, e), e.scrollIntoView({ block: "nearest" }), e._tippy && !this.isTapped && e._tippy.show(), e.getAttribute(Kr) === Xr && e.querySelector(`[${Yr}]`)?._tippy?.show(), this.isTapped = !1;
	}
	...
	onKeydown(e) {
		let t = this, n = Array.prototype.filter.call(this.element.querySelectorAll(`[${Kr}]`), (e) => e.closest(`[${Gr}]`) === t.element && !!e.offsetParent);
		if (n.length !== 0) switch (e.key) {
			...
			case Wr.Space:
				...
				break;
			case Wr.Tab:
				let t = this.highlightedItem.querySelectorAll(Qr).length === 0, r = this.element.isEqualNode(e.target);
	...
	onClick(e) {
		...
		let t = e.target;
		if (!t.hasAttribute(Gr)) for (; t.parentElement.closest(`[${Gr}]`) !== this.element;) t = t.parentElement.closest(`[${Gr}]`);
		this.highlightedItem = t.closest(`[${Kr}]`) || t.closest(`[${Gr}]`) || this.highlightedItem, t.hasAttribute(`${Kr}`) && this.element.focus(), this.highlightedItem === this.element ? this.element.removeAttribute("aria-activedescendant") : this.focusItem(this.highlightedItem);
	}
	onFocus() {
		if (this.highlightedItem === this.element) {
			let e = this.element.querySelectorAll(`[${Kr}]`);
			if (e.length === 1) {
				this.defocusItem(this.highlightedItem), this.highlightedItem = e[0], this.focusItem(e[0]);
				return;
			}
		}
		this.highlightedItem && !this.isClicked && (this.highlightedItem._tippy && this.highlightedItem._tippy.show(), this.highlightedItem.getAttribute(Kr) === Xr && this.highlightedItem.querySelector(`[${Yr}]`)?._tippy?.show()), this.isClicked = !1, this.element.hasAttribute("aria-activedescendant") || this.highlightedItem === this.element ? this.highlightedItem !== this.element && this.focusItem(this.highlightedItem) : this.setAriaActiveDescendant(this.highlightedItem), ei(this.element), this.highlightedItem && this.highlightedItem !== this.element && this.highlightedItem.setAttribute("tabindex", "-1");
	}
	onBlur() {
		this.highlightedItem && (this.highlightedItem._tippy && this.highlightedItem._tippy.hide(), this.highlightedItem.getAttribute(Kr) === Xr && this.highlightedItem.querySelector(`[${Yr}]`)?._tippy?.hide());
	}
	onMousedown() {
		this.isClicked = !0;
	}
...
}, ni = "focus-item", ri = { beforeMount(e, t) {
	e.hasAttribute("id") || e.setAttribute("id", ii()), t.value ? e.setAttribute(ni, t.value) : e.toggleAttribute(ni, !0);
} };
function ii() {
	let e;
	do
...
var ai = /* @__PURE__ */ function(e) {
...
}(ai || {}), oi = "focus-container", si = "focus-container-active", ci = "focus-list", li = "focus-icon", ui = `[${ci}],[${oi}]`, di = `button,input,select,a,textarea,[contenteditable],.ag-tab-guard,[${ci}],[${oi}],[${li}],[tabindex]`, fi = {
	childList: !0,
	subtree: !0,
	attributes: !0,
	attributeFilter: [
	...
	mounted(e) {
		[...document.querySelectorAll(".inner-shell")].some((t) => t.contains(e)) && pi.push(new hi(e));
	},
	beforeUnmount(e) {
		pi = pi.filter((t) => t.element === e ? (t.removeEventListeners(), !1) : !0);
	}
}, hi = class {
	element;
	isTabbingEnabled;
	...
	focusHandler;
	constructor(e) {
		this.element = e, this.isTabbingEnabled = !1, this.element.toggleAttribute(oi, !0), this.element.toggleAttribute(si, !1), this.element.tabIndex = 0, this.initialDisableTimeout = setTimeout(() => this.disableTabbing(), 600), this.keypressHandler = (e) => {
			this.onKeypress(e);
		}, this.clickHandler = () => {
			this.onClick();
	...
	onKeypress(e) {
		e.target === this.element && (e.key === ai.Enter || e.key === ai.Space) && this.enableTabbing()?.focus();
	}
	onClick() {
		this.enableTabbing();
	}
	...
	disableTabbing() {
		...
		let t = Array.prototype.filter.call(this.element.querySelectorAll(di), () => !0);
	...
	enableTabbing() {
		...
		return this.withPausedObserver(() => {
			Array.prototype.map.call(this.element.querySelectorAll(di), (t) => {
				(t.closest(ui) === this.element || t.closest(ui) === t && t.parentElement.closest(ui) === this.element) && t.offsetParent && (t.tabIndex !== 0 && (t.tabIndex = 0), e === void 0 && (e = t));
			});
		}), e;
	}
	setTabbingEnabled(e) {
		this.isTabbingEnabled = e, this.element.toggleAttribute(si, e);
	}
	observeMutations() {
		this.mutationObserver?.observe(this.element, fi);
	}
...
}, gi = "truncate-text", _i = "truncate-trigger", vi = {
	beforeMount(e, t) {
		!e.classList.contains("truncate") && !t.value?.noTruncateClass && e.classList.add("truncate"), e.toggleAttribute(gi, !0);
	},
	mounted(e, t) {
		let n;
		t.value && t.value.externalTrigger && (n = e.closest(`[${_i}]`)), st(e, {
			content: xi(e.textContent),
			onShow: yi,
			allowHTML: !0,
			placement: "bottom-start",
			maxWidth: 320,
	...
	updated(e, t) {
		e._tippy && (e._tippy.setContent(xi(e.textContent)), t.value && t.value.options && e._tippy.setProps(t.value.options));
	},
	unmounted(e) {
...
var Si = ["content"], Ci = {
...
}, ki = "focus-container", Ai = "focus-container-active", ji = /* @__PURE__ */ C({
	__name: "panel-screen",
	props: {
		header: {
			type: Boolean,
	...
	setup(e, { expose: t }) {
		let { t: n } = P(), i = bn(), a = en(), o = Me("iApi"), s = Ke("el"), c = Ke("contentEl"), l = D(), u = D(), d = D();
		t({ el: s });
		let f = e, p = _(() => o.fixture.get("appbar") ? a.temporary : []), m = _(() => i.mobileView), h = _(() => i.reorderable), ee = `[focus-list], [${ki}]`, te = (e) => e.scrollHeight > e.clientHeight, ne = (e) => e.querySelector(ee), re = () => {
			if (!c.value) return "-1";
			let e = c.value.closest(`[${ki}]`);
			if (e && !e.hasAttribute(Ai)) return "-1";
			let t = !!ne(c.value);
			return te(c.value) && !t ? "0" : "-1";
		}, ie = () => {
			if (!c.value) return;
			let e = re();
		...
		}, ce = () => !m.value && !f.panel.teleport, le = (e) => {
			f.panel.move(e), e === "left" && Pe(() => {
				(s.value?.querySelector(".move-left")).focus();
			});
		}, ue = _(() => f.screenId ? f.panel.screens[f.screenId][o.$i18n.locale.value ?? "en"] : null);
		return Le(() => {
			s.value?.addEventListener("blur", oe), s.value?.addEventListener("keyup", se), c.value && (c.value.addEventListener("focus", ae), l.value = new ResizeObserver(ie), l.value.observe(c.value), u.value = new MutationObserver(ie), u.value.observe(c.value, {
				childList: !0,
				subtree: !0,
				attributes: !0,
				attributeFilter: ["focus-list", "focus-container"]
			}), d.value = new MutationObserver(ie), d.value.observe(c.value, {
				attributes: !0,
				attributeFilter: ["tabindex"]
			}), ie());
		}), Ie(() => {
			s.value?.removeEventListener("blur", oe), s.value?.removeEventListener("keyup", se), c.value?.removeEventListener("focus", ae), l.value?.disconnect(), u.value?.disconnect(), d.value?.disconnect();
		}), (t, r) => {
			let i = O("back"), a = O("panel-options-menu"), o = O("left"), l = O("right"), u = O("pin"), d = O("expand"), f = O("minimize"), m = O("close"), ee = k("truncate"), te = k("focus-item"), ne = k("tippy");
			return N((E(), b("div", {
				class: "h-full flex flex-col items-stretch",
				content: `<div style='word-break: break-word;'>${j(n)(e.panel.alertName) + ". " + j(n)("panels.access")}</div>`,
				ref_key: "el",
				ref: s
			}, [
				e.header ? (E(), b("header", Ci, [
					S(i, {
						class: T(e.panel.teleport ? "display-none" : "block sm:display-none"),
						onClick: r[0] ||= (t) => e.panel.close()
					}, null, 8, ["class"]),
					N((E(), b("h2", wi, [t.$slots.header ? He(t.$slots, "header", { key: 0 }) : (E(), b("div", Ti, A(j(n)(e.panel.alertName)), 1))])), [[ee]]),
					t.$slots.controls ? (E(), v(a, { key: 0 }, {
						default: M(() => [He(t.$slots, "controls")]),
						_: 3
					})) : y("", !0),
					x("div", { class: T(e.panel.teleport ? "flex" : "display-none sm:flex") }, [
						e.panel.teleport ? y("", !0) : (E(), b("div", Ei, [
							h.value ? (E(), v(o, {
								key: 0,
								onClick: r[1] ||= (e) => le("left"),
								active: !e.panel.isLeftMostPanel
							}, null, 8, ["active"])) : y("", !0),
							h.value ? (E(), v(l, {
								key: 1,
								onClick: r[2] ||= (e) => le("right"),
							...
							e.panel.controls && e.panel.controls.expand ? (E(), v(d, {
								key: 2,
								onClick: r[4] ||= (t) => e.panel.expand(),
								active: e.panel.expanded
							}, null, 8, ["active"])) : y("", !0)
						])),
						e.panel.button && p.value?.includes(e.panel.id) ? (E(), v(f, {
							key: 1,
							onClick: r[5] ||= (t) => e.panel.minimize()
						})) : y("", !0),
						S(m, { onClick: r[6] ||= (t) => e.panel.close() })
					], 2)
				])) : y("", !0),
				e.content ? (E(), b("div", {
					key: 1,
					...
					ref: c,
					tabindex: "-1"
				}, [t.$slots.content ? He(t.$slots, "content", { key: 0 }) : ue.value ? (E(), b("div", {
					key: 1,
					innerHTML: ue.value.innerHTML
				}, null, 8, Di)) : y("", !0)], 512)) : y("", !0),
				e.footer ? N((E(), b("div", Oi, [He(t.$slots, "footer")])), [[te]]) : y("", !0)
			], 8, Si)), [[ne, {
				trigger: "manual",
				touch: !1,
				onShow: ce,
...
}), Mi = { class: "relative" }, Ni = ["content", "aria-label"], Pi = ["transform"], Fi = /* @__PURE__ */ C({
	__name: "pin",
	...
	setup(e) {
		...
		return (n, r) => {
			let i = k("tippy");
			return E(), b("div", Mi, [N((E(), b("button", {
				type: "button",
				class: T(["text-gray-500 hover:text-black focus:text-black p-8", { "text-gray-700": e.active }]),
				content: j(t)(e.active ? "panels.controls.unpin" : "panels.controls.pin"),
				"aria-label": j(t)(e.active ? "panels.controls.unpin" : "panels.controls.pin")
			}, [(E(), b("svg", {
				class: "fill-current w-16 h-16",
				xmlns: "http://www.w3.org/2000/svg",
				viewBox: "0 0 384 512",
				transform: `rotate(${e.active ? 30 : 0})`
			}, [...r[0] ||= [x("path", { d: "M298.028 214.267L285.793 96H328c13.255 0 24-10.745 24-24V24c0-13.255-10.745-24-24-24H56C42.745 0 32 10.745 32 24v48c0 13.255 10.745 24 24 24h42.207L85.972 214.267C37.465 236.82 0 277.261 0 328c0 13.255 10.745 24 24 24h136v104.007c0 1.242.289 2.467.845 3.578l24 48c2.941 5.882 11.364 5.893 14.311 0l24-48a8.008 8.008 0 0 0 .845-3.578V352h136c13.255 0 24-10.745 24-24-.001-51.183-37.983-91.42-85.973-113.733z" }, null, -1)]], 8, Pi))], 10, Ni)), [[i, {
				placement: "bottom",
				hideOnClick: !1
			}]])]);
		};
	}
}), Ii = { class: "relative" }, Li = ["content", "aria-label"], Ri = /* @__PURE__ */ C({
	__name: "close",
	props: { active: Boolean },
	setup(e) {
		let { t } = P();
		return (n, r) => {
			let i = k("tippy");
			return E(), b("div", Ii, [N((E(), b("button", {
				type: "button",
				class: T(["text-gray-500 hover:text-black focus:text-black p-8", { "text-gray-700": e.active }]),
				content: j(t)("panels.controls.close"),
				"aria-label": j(t)("panels.controls.close")
			}, [...r[0] ||= [x("svg", {
				class: "fill-current w-16 h-16",
				xmlns: "http://www.w3.org/2000/svg",
				viewBox: "0 0 352 512"
			}, [x("path", { d: "M242.72 256l100.07-100.07c12.28-12.28 12.28-32.19 0-44.48l-22.24-22.24c-12.28-12.28-32.19-12.28-44.48 0L176 189.28 75.93 89.21c-12.28-12.28-32.19-12.28-44.48 0L9.21 111.45c-12.28 12.28-12.28 32.19 0 44.48L109.28 256 9.21 356.07c-12.28 12.28-12.28 32.19 0 44.48l22.24 22.24c12.28 12.28 32.2 12.28 44.48 0L176 322.72l100.07 100.07c12.28 12.28 32.2 12.28 44.48 0l22.24-22.24c12.28-12.28 12.28-32.19 0-44.48L242.72 256z" })], -1)]], 10, Li)), [[i, {
				placement: "bottom",
				theme: "ramp4",
				animation: "scale"
			}]])]);
...
}), zi = { class: "relative" }, Bi = ["content", "aria-label"], Vi = /* @__PURE__ */ C({
	__name: "back",
	props: { active: Boolean },
	setup(e) {
		let { t } = P();
		return (n, r) => {
			let i = k("tippy");
			return E(), b("div", zi, [N((E(), b("button", {
				type: "button",
				class: T(["text-gray-500 hover:text-black focus:text-black p-8", { "text-gray-700": e.active }]),
				content: j(t)("panels.controls.back"),
				"aria-label": j(t)("panels.controls.back")
			}, [...r[0] ||= [x("svg", {
				xmlns: "http://www.w3.org/2000/svg",
				class: "fill-current w-16 h-16",
				viewBox: "0 0 16 16"
...
}), Hi = { class: "w-32 h-32" }, Ui = ["content", "aria-label"], Wi = {
	key: 0,
	xmlns: "http://www.w3.org/2000/svg",
	viewBox: "0 0 24 24",
	...
	setup(e) {
		...
		return (n, r) => {
			let i = k("tippy");
			return E(), b("div", Hi, [N((E(), b("button", {
				type: "button",
				class: "text-gray-500 hover:text-black focus:text-black w-full h-full flex justify-center items-center",
				content: j(t)(`panels.controls.${e.active ? "collapse" : "expand"}`),
				"aria-label": j(t)(`panels.controls.${e.active ? "collapse" : "expand"}`)
			}, [e.active ? (E(), b("svg", Gi, [...r[1] ||= [
				x("path", {
					d: "M0 0h24v24H0z",
					fill: "none"
				}, null, -1),
				...
				x("path", { d: "M8 19h3v3h2v-3h3l-4-4-4 4zm8-15h-3V1h-2v3H8l4 4 4-4zM4 9v2h16V9H4z" }, null, -1),
				x("path", { d: "M4 12h16v2H4z" }, null, -1)
			]])) : (E(), b("svg", Wi, [...r[0] ||= [
				x("path", {
					d: "M0 0h24v24H0z",
					fill: "none"
				...
				x("path", { d: "M4 20h16v2H4zM4 2h16v2H4zm9 7h3l-4-4-4 4h3v6H8l4 4 4-4h-3z" }, null, -1)
			]]))], 8, Ui)), [[i, {
				placement: "bottom",
				theme: "ramp4",
				animation: "scale",
...
}), qi = { class: "relative" }, Ji = ["content", "aria-label"], Yi = /* @__PURE__ */ C({
	__name: "minimize",
	props: { active: Boolean },
	setup(e) {
		let { t } = P();
		return (n, r) => {
			let i = k("tippy");
			return E(), b("div", qi, [N((E(), b("button", {
				type: "button",
				class: T(["text-gray-500 hover:text-black focus:text-black p-6", { "text-gray-700": e.active }]),
				content: j(t)("panels.controls.minimize"),
				"aria-label": j(t)("panels.controls.minimize")
			}, [...r[0] ||= [x("svg", {
				class: "fill-current w-20 h-20",
				xmlns: "http://www.w3.org/2000/svg",
				viewBox: "0 0 24 24"
				...
				d: "M0 0h24v24H0V0z",
				fill: "none"
			}), x("path", { d: "M6 19h12v2H6z" })], -1)]], 10, Ji)), [[i, {
				placement: "bottom",
				theme: "ramp4",
				animation: "scale"
			}]])]);
...
var Zi = { class: "relative" }, Qi = ["content", "aria-label"], $i = /* @__PURE__ */ C({
	__name: "right",
	props: { active: Boolean },
	setup(e) {
		let { t } = P(), n = e, r = D(null), { syncTooltipAfterKeyboardMove: i } = Xi(r, () => !!n.active);
		return (n, a) => {
			let o = k("tippy");
			return E(), b("div", Zi, [N((E(), b("button", {
				ref_key: "buttonRef",
				...
				onClick: a[0] ||= (...e) => j(i) && j(i)(...e),
				content: j(t)("panels.controls.moveRight"),
				"aria-label": j(t)("panels.controls.moveRight")
			}, [...a[1] ||= [x("svg", {
				class: "fill-current w-16 h-16",
				xmlns: "http://www.w3.org/2000/svg",
				viewBox: "4 4 16 16"
			}, [x("path", { d: "M 8.59 16.34 L 13.17 11.75 L 8.59 7.16 L 10 5.75 L 16 11.75 L 10 17.75 Z" })], -1)]], 10, Qi)), [[o, {
				placement: "bottom",
				theme: "ramp4",
				animation: "scale"
			}]])]);
...
}), ea = { class: "relative" }, ta = ["content", "aria-label"], na = /* @__PURE__ */ C({
	__name: "left",
	props: { active: Boolean },
	setup(e) {
		let t = e, { t: n } = P(), r = D(null), { syncTooltipAfterKeyboardMove: i } = Xi(r, () => !!t.active);
		return (t, a) => {
			let o = k("tippy");
			return E(), b("div", ea, [N((E(), b("button", {
				ref_key: "buttonRef",
				ref: r,
				type: "button",
				class: T(["p-8 move-left", {
					"text-gray-500 hover:text-black focus:text-black": e.active,
					"text-gray-300": !e.active
				}]),
				onClick: a[0] ||= (...e) => j(i) && j(i)(...e),
				content: j(n)("panels.controls.moveLeft"),
				"aria-label": j(n)("panels.controls.moveLeft")
			}, [...a[1] ||= [x("svg", {
				class: "fill-current w-16 h-16",
				xmlns: "http://www.w3.org/2000/svg",
				viewBox: "4 4 16 16"
			}, [x("path", { d: "M 15.41 16.09 L 10.83 11.5 L 15.41 6.91 L 14 5.5 L 8 11.5 L 14 17.5 Z" })], -1)]], 10, ta)), [[o, {
				placement: "bottom",
				theme: "ramp4",
				animation: "scale"
			}]])]);
...
}), ra = /* @__PURE__ */ n(/* @__PURE__ */ C({
	__name: "panel-options-menu",
	setup(e) {
		let { t } = P();
		return (e, n) => (E(), v(Cn, {
			class: "flex",
			tooltip: j(t)("panels.controls.optionsMenu"),
			popperOptions: { strategy: "fixed" },
			position: "bottom-end"
		}, {
			header: M(() => [...n[0] ||= [x("div", { class: "p-6" }, [x("svg", {
				xmlns: "http://www.w3.org/2000/svg",
				viewBox: "0 0 24 24",
				class: "fill-current w-20 h-20"
			}, [x("path", { d: "M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z" })])], -1)]]),
			default: M(() => [He(e.$slots, "default", {}, void 0, !0)]),
			_: 3
		}, 8, ["tooltip"]));
	}
}), [["__scopeId", "data-v-51e22a4a"]]), ia = {
	key: 0,
	xmlns: "http://www.w3.org/2000/svg",
	viewBox: "0 0 24 24",
	...
	setup(e) {
		...
		return (e, i) => {
			let a = O("mapnav-button");
			return E(), v(a, {
				onClickFunction: r,
				tooltip: j(t)("mapnav.fullscreen")
			}, {
				default: M(() => [j(n).isFullscreen ? (E(), b("svg", ia, [...i[0] ||= [x("path", {
					d: "M0 0h24v24H0z",
					fill: "none"
				}, null, -1), x("path", { d: "M5 16h3v3h2v-5H5v2zm3-8H5v2h5V5H8v3zm6 11h2v-3h3v-2h-5v5zm2-11V5h-2v5h5V8h-3z" }, null, -1)]])) : (E(), b("svg", aa, [...i[1] ||= [x("path", {
					d: "M0 0h24v24H0z",
					fill: "none"
				}, null, -1), x("path", { d: "M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z" }, null, -1)]]))]),
				_: 1
			}, 8, ["tooltip"]);
		};
	}
	...
	setup(e) {
		let { t } = P(), n = Me("iApi"), r = ze([]), i = async () => {
			if (r.length) a(r);
			else {
				let e = await o({
					maximumAge: Infinity,
					...
					e.code === GeolocationPositionError.PERMISSION_DENIED ? n.notify.show(kn.ERROR, t("mapnav.geolocator.error.permission")) : n.notify.show(kn.ERROR, t("mapnav.geolocator.error.internal"));
				});
		...
		return (e, n) => {
			let r = O("mapnav-button");
			return E(), v(r, {
				onClickFunction: i,
				tooltip: j(t)("mapnav.geolocator")
			}, {
				default: M(() => [...n[0] ||= [x("svg", {
					xmlns: "http://www.w3.org/2000/svg",
					viewBox: "0 0 24 24",
					class: "fill-current w-32 h-20"
				}, [x("path", { d: "M12 8c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4zm8.94 3c-.46-4.17-3.77-7.48-7.94-7.94V1h-2v2.06C6.83 3.52 3.52 6.83 3.06 11H1v2h2.06c.46 4.17 3.77 7.48 7.94 7.94V23h2v-2.06c4.17-.46 7.48-3.77 7.94-7.94H23v-2h-2.06zM12 19c-3.87 0-7-3.13-7-7s3.13-7 7-7 7 3.13 7 7-3.13 7-7 7z" })], -1)]]),
				_: 1
			}, 8, ["tooltip"]);
		};
	}
	...
	setup(e) {
		...
		return (e, n) => {
			let i = O("mapnav-button");
			return E(), v(i, {
				onClickFunction: r,
				tooltip: j(t)("mapnav.home")
			}, {
				default: M(() => [...n[0] ||= [x("svg", {
					xmlns: "http://www.w3.org/2000/svg",
					viewBox: "0 0 24 24",
					class: "fill-current w-32 h-20"
				}, [x("path", { d: "M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" }), x("path", {
					d: "M0 0h24v24H0z",
					fill: "none"
				})], -1)]]),
				_: 1
	...
	setup(e) {
		return (t, n) => {
			let r = k("focus-item"), i = k("tippy");
			return E(), b("div", la, [N((E(), b("button", {
				type: "button",
				class: T(["w-full h-full default-focus-style", [e.showOutline ? "focus:outline focus:outline-2 focus:outline-blue-400 focus:absolute focus:z-50" : "focus:outline-none"]]),
				onClick: n[0] ||= (t) => e.onClickFunction(),
				content: e.tooltip,
				"aria-label": e.ariaLabel || (typeof e.tooltip == "string" ? e.tooltip : ""),
				"aria-pressed": e.ariaPressed
			}, [He(t.$slots, "default")], 10, ua)), [[r], [i, { placement: "left" }]])]);
		};
	}
}), fa = {
	class: "relative",
	tabindex: "-1"
}, pa = ["aria-label", "content"], ma = /* @__PURE__ */ n(/* @__PURE__ */ C({
	__name: "button",
	props: {
		onClickFunction: {
			type: Function,
	...
	setup(e) {
		let t = Me("iApi"), n = e, r = () => t.event.emit(Z.APPBAR_BUTTON_CLICK, n.buttonId);
		return (t, n) => {
			let i = k("focus-item"), a = k("tippy");
			return E(), b("div", fa, [N((E(), b("button", {
				type: "button",
			...
			}, [He(t.$slots, "default", {}, void 0, !0)], 8, pa)), [[i], [a, {
				allowHTML: !0,
				placement: "right"
			}]])]);
		};
	}
}), [["__scopeId", "data-v-1cb84ae1"]]), ha = g("areas-of-interest", () => ({ areas: D([]) })), ga = g("export", () => {
	let e = D({
		title: !0,
		map: !0,
		mapElements: !0,
	...
	fsaToProvince(e) {
		...
		return t = r === "X" ? n === "X0A" || n === "X0B" || n === "X0C" ? 62 : 61 : va[r], this.codeToProvince(t);
	}
};
function xa(e) {
	return new ba(e);
	...
	constructor(e, t) {
		lt.get(t).then((t) => {
			t.data.definitions.forEach((t) => {
				Sa[e][t.code] = t.term.split(`${t.code}-`)[1];
			}), Object.keys(Sa[e]).forEach((t) => {
				this.allTypes[t] = Sa[e][t], this.validTypes[t] = Sa[e][t];
			}), this.typesFetched = !0;
		});
	}
	filterValidTypes(e) {
...
async function Ea(e, t) {
	let n, r;
	return /^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?)(\s*[,|;\s]\s*)[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)[*]$/.test(t) && !e.disabledSearchTypes.includes("LAT/LNG") ? (r = t.slice(0, -1), n = new Ta(e, r), await Fa(n)) : /^[ABCEGHJKLMNPRSTVXY]\d[A-Z]/.test(t) && !e.disabledSearchTypes.includes("FSA") ? (r = t.substring(0, 3).toUpperCase(), n = new Ta(e, r), await Ia(n)) : /^\d{2,3}[A-P]/.test(t) && !e.disabledSearchTypes.includes("NTS") ? (r = t.substring(0, 6).toUpperCase(), n = new Ta(e, r), await La(n)) : (r = encodeURIComponent(t.trim()), n = new Ta(e, r), await Ra(n)), e.customSources.length && await ja(n, e.customSources), n;
}
var Da = async (e) => {
	let [t, n] = await ut(lt.get(e));
...
}, Aa = (e, t) => t.filter((t) => e.types.validTypes[t.concise.code]).map((t) => ({
	name: t.name,
	flav: "nme",
	bbox: t.bbox,
	...
	let [t, n] = await ut(Da(ka(e, !0)));
	return t ? (console.error("Geolocation service failed"), e.failedServs.push("geolocation"), []) : n;
}, Na = async (e) => {
	let [t, n] = await ut(Da(ka(e, !1))), r;
	t ? (console.error("GeoName service targeting Name failed"), e.failedServs.push("geoname"), r = []) : r = n.items, e.addResult(Aa(e.config, r));
}, Pa = async (e, t, n) => {
	let [r, i] = await ut(Da(ka(e, !1, t, n))), a;
	r ? (console.error("GeoName service targeting Lat Lon failed"), e.failedServs.push("geoname"), a = []) : a = i.items, e.addResult(Aa(e.config, a));
}, Fa = async (e) => {
	...
	await Pa(e, n, r), e.addResult(i);
}, Ia = async (e) => {
	let t = e.config, n = await Ma(e);
	if (n.length) {
	...
	let t = e.config;
	if (await Na(e), t.categories.length === 0 || t.categories.includes("ADDR")) {
		let n = await Ma(e), r = t.sortOrder.indexOf("ADDR"), i = r >= 0 ? r : t.sortOrder.length, a = n.filter((e) => e.type?.includes("Street")).map((e) => {
...
}, za = "~FSA~", Ba = "https://geogratis.gc.ca/services/geolocation/@{language}/locate", Va = "https://geogratis.gc.ca/services/geoname/@{language}/geonames.json", Ha = "https://geogratis.gc.ca/services/geoname/@{language}/codes/province.json", Ua = "https://geogratis.gc.ca/services/geoname/@{language}/codes/concise.json", Wa = class {
	config;
	constructor(e, t) {
		let n, r, i, a, o = t?.serviceUrls;
		o ? (n = o.geoLocation ? o.geoLocation : Ba, r = o.geoNames ? o.geoNames : Va, i = o.geoProvince ? o.geoProvince : Ha, a = o.geoTypes ? o.geoTypes : Ua) : (n = Ba, r = Va, i = Ha, a = Ua), n = n.replace("@{language}", e), r = r.replace("@{language}", e), i = i.replace("@{language}", e), a = a.replace("@{language}", e);
		let s = "", c = t?.fsaBoundaries;
		if (c && c.serviceUrl) {
			let e = c.keyField || "CFSAUID";
			s = `${c.serviceUrl}/query/?where=${e}%3D'${za}'&outFields=${e}&returnGeometry=true&f=json`;
		}
		...
		l ? (u = l.categories ? l.categories : [], d = l.sortOrder ? l.sortOrder : [], f = l.disabledSearchTypes ? l.disabledSearchTypes : [], p = l.maxResults > 0 ? l.maxResults : 100, m = !!l.officialOnly) : (u = [], d = [], f = [], p = 100, m = !1), h = t?.customSources ?? [], h = h.map((e) => ({
			...e,
			onSearch: async (t) => {
				let n = e.data ?? [], r = Ka(), i = decodeURIComponent(r.cleanVal(t)).replace("*", "");
	...
	async query(e) {
		let t = await Ea(this.config, e.toUpperCase()), n = this.config.sortOrder.length, r = t.results.filter((e) => e.order < n), i = t.results.filter((e) => e.order >= n);
...
var Ka = g("geosearch", () => {
	let e = D(new Wa("en", void 0)), t = D({
		type: "",
		province: "",
		extent: void 0
	}), n = D(!1), r = D(""), i = D(""), a = D(""), o = D([]), s = D([]), c = D(!1), l = D([]), u = _(() => e.value.fetchProvinces().then((e) => (e.sort((e, t) => e.name.localeCompare(t.name, void 0, { sensitivity: "case" })), e))), d = _(() => new Promise((t) => {
		...
		if (!u) o.value = [], s.value = [], c.value = !1;
			...
			let r = setInterval(() => {
				e.value.config.provinces.listFetched && e.value.config.types.typesFetched && (clearInterval(r), e.value.query(`${u}*`).then((e) => {
					l.value = e.failedServs, a.value = u, s.value = e.results, o.value = Ga(n.value, t.value, s.value) || [], c.value = !1;
				}));
			}, 250);
		} else o.value = Ga(n.value, t.value, s.value) || [], c.value = !1;
	}
	function h(e) {
	...
	_visibleChildren;
	constructor(e, t, n) {
		super(e), this._uid = Js.sharedUtils.generateUUID(), this._name = t.name, this._type = t.type ?? Ja.Placeholder, this._parent = n, this._children = [], this._loadPromise = new Y(), this._hidden = t.hidden ?? !1, this._expanded = t.expanded ?? !0, this._visibility = !0, this._exclusive = t.exclusive ?? !1, this._controls = t.controls?.slice() ?? [qa.Visibility, qa.Expand], this._disabledControls = t.disabledControls?.slice(), this._visibleChildren = [];
	}
	get uid() {
		return this._uid;
	}
	...
	toggleVisibility(e, t = !0) {
		if (this.visibility !== e) {
			...
			else if (this.visibility) if (this._lastVisible && (!(this._lastVisible instanceof Xa) || this._lastVisible.layerControlAvailable(G.Visibility))) this._lastVisible.toggleVisibility(!0);
			else {
				let e = this.children.find((e) => !(e instanceof Xa) || e.layerControlAvailable(G.Visibility));
	...
	checkVisibilityRules() {
		(this.parent && !this.parent.visibility || this.parent?.exclusive && this.parent.children.some((e) => e.visibility && e !== this && e.type === Ja.Item)) && this.toggleVisibility(!1, !1);
	}
	checkVisibility(e) {
		this instanceof Xa && !this.layerControlAvailable(G.Visibility) || (this.exclusive ? e.visibility ? (this.children.forEach((t) => {
			t.uid !== e.uid && t.toggleVisibility(!1, !1);
		}), this._lastVisible = e, this._visibility = !0, this instanceof Xa && this.layer && this.layer.layerExists && (this.layer.visibility = !0)) : (this._visibility = !1, this instanceof Xa && this.layer && this.layer.layerExists && (this.layer.visibility = !1), this._lastVisible = e) : this.children.some((e) => e.visibility) ? (this._visibility = !0, this._visibleChildren = this.children.filter((e) => e.visibility), this instanceof Xa && this.layer && this.layer.layerExists && (this.layer.visibility = !0)) : (this._visibility = !1, this._visibleChildren = [], this instanceof Xa && this.layer && this.layer.layerExists && (this.layer.visibility = !1)), this.parent && this.parent.checkVisibility(this));
	}
	getConfig() {
		let e = {
			name: this._name,
	...
	load() {
		this._type = Ja.Item, this._loadPromise.resolveMe(), this.checkVisibilityRules();
	}
	reload() {
		this._type = Ja.Placeholder, this._loadPromise = new Y();
	}
	error() {
		this._type !== Ja.Error && (this._type = Ja.Error, this._loadPromise.getPromise().catch(() => {}), this._loadPromise.rejectMe(), this.checkVisibilityRules());
	}
}, Xa = class extends Ya {
	_layerId;
	_layerIdx;
	...
	load(e) {
		let t = e instanceof Do ? e : this.$iApi.geo.layer.getLayer(this._layerId ?? this._layerUid);
		t && (this.layer = t, this._layerRedrawing = t.mapLayer && t.drawState !== U.UP_TO_DATE, t.loadPromise().then(() => {
			this._layerInitVis = this._layerInitVis === void 0 ? t.visibility : this._visibility, super.load(), this.toggleVisibility(this._layerInitVis, !0, !0), t.visibility || this.setSymbologyVisibility(void 0, !1), this.handlers.push(this.$iApi.event.on(Z.LAYER_VISIBILITYCHANGE, (e) => {
				e.layer.uid === this.layer.uid && this._type === Ja.Item && this.toggleVisibility(e.visibility, !0, !0);
			})), this.handlers.push(this.$iApi.event.on(Z.LAYER_DRAWSTATECHANGE, (e) => {
				this.layer.uid === e.layer.uid && (e.layer.drawState === U.REFRESH ? this.layerRedrawing = !0 : setTimeout(() => {
					this.layerRedrawing = e.layer.drawState === U.REFRESH;
				}, 500));
	...
	_content;
	constructor(e, t, n) {
		super(e, t, n), this._infoType = t.infoType ?? Za.Title, this._content = t.content ?? "", t.infoType === Za.Template && this.$element.component(`${this._uid}-info-section`, { template: this._content }), (t.infoType || t.content) && (this._controls = t.controls?.slice() ?? [qa.Expand]), super.load();
	}
	get infoType() {
	...
	let e = D(), t = D([]), n = D([]), r = D(!0), i = D(3);
	function a(e) {
		if (e.parent === void 0) t.value.push(e.item);
		else {
			if (!(e.item instanceof Qa) && !(e.item instanceof Xa)) {
	...
	function o(e) {
		...
		}), t = t.filter((e) => !(e instanceof Qa && !e.children.length && e.content === "")), t);
		t.value = n(t.value);
	}
	function s(e) {
	...
	let e = D({
		...
		tileSchemas: [],
		initialBasemapId: ""
	}), t = D({}), n = D(!0), r = D(1.5), i = D("#FF0000"), a = D(1), o = D("#000000"), s = D(.25);
	function c(t) {
	...
	let e = {
		...
		layerType: I.UNKNOWN,
		url: ""
	}, t = D(), n = D(""), r = D(""), i = D(!0), a = D(null), o = D({
		config: e,
		configOptions: []
	}), s = D($.UPLOAD);
	function c(t) {
	...
	_isFullscreen;
	constructor(e, t, n) {
		this.event = new cn(this);
		let r = co(e, this);
		this.$vApp = r.app, this.$element = r.element, this.$i18n = r.i18n, this.$rootEl = r.app.$root?.$el, this.fixture = new Ss(this), this.panel = new ws(this), this.geo = new bs(this), this.dev = new js(this), this.ui = {
			...
			escapeHtml: () => "",
			isPlainText: () => !0
		}, this.notify = new An(this), this._isFullscreen = et.isEnabled && !!this.$vApp.$root && et.isFullscreen && et.element === this.$vApp.$root.$el, et.isEnabled && et.onchange(() => {
			this._isFullscreen = et.isEnabled && !!this.$vApp.$root && et.isFullscreen && et.element === this.$vApp.$root.$el;
		}), this.initialize(!0, t, n);
	}
	initialize(e, n, r) {
		let i = t(this.$vApp.$pinia), a = bn(this.$vApp.$pinia), o = mn(this.$vApp.$pinia);
		if (n?.configs !== void 0) {
	...
	reloadWorker(n, r) {
		let i = Pr(this.$vApp.$pinia), a = wn(this.$vApp.$pinia), o = t(this.$vApp.$pinia), s = on(this.$vApp.$pinia), c = e(this.$vApp.$pinia), l = tn(this.$vApp.$pinia), u = !!n;
	...
	useStore(n) {
		if (!([
			...
			case "appbar": return en(this.$vApp.$pinia);
			case "areas-of-interest": return ha(this.$vApp.$pinia);
			case "details": return an(this.$vApp.$pinia);
			case "export": return ga(this.$vApp.$pinia);
			case "extentguard": return _a(this.$vApp.$pinia);
			case "geosearch": return Ka(this.$vApp.$pinia);
			case "grid": return tn(this.$vApp.$pinia);
			case "help": return i(this.$vApp.$pinia);
			case "keyboardnav": return dr(this.$vApp.$pinia);
			case "legend": return $a(this.$vApp.$pinia);
			case "mapnav": return eo(this.$vApp.$pinia);
			case "metadata": return to(this.$vApp.$pinia);
			case "northarrow": return no(this.$vApp.$pinia);
			case "overviewmap": return ro(this.$vApp.$pinia);
			case "panguard": return io(this.$vApp.$pinia);
			case "scrollguard": return ao(this.$vApp.$pinia);
			case "wizard": return oo(this.$vApp.$pinia);
			case "config": return t(this.$vApp.$pinia);
			case "fixture": return on(this.$vApp.$pinia);
			case "instance": return Pr(this.$vApp.$pinia);
			case "layer": return e(this.$vApp.$pinia);
			case "map-caption": return nn(this.$vApp.$pinia);
			case "maptip": return mn(this.$vApp.$pinia);
			case "notification": return wn(this.$vApp.$pinia);
			case "panel": return bn(this.$vApp.$pinia);
			default:
	...
	setLanguage(e) {
		...
		let n = t(this.$vApp.$pinia), r = wn(this.$vApp.$pinia), i = n.registeredLangs;
	...
	toggleFullscreen() {
		et.isEnabled && et.toggle(this.$element._container || void 0);
	}
	get isFullscreen() {
		return this._isFullscreen;
	}
	get started() {
		return Pr(this.$vApp.$pinia).started;
	}
	updateAlert(e) {
...
function co(e, t) {
	...
	n.use(({ store: e }) => {
		let t = tt(e.$state);
		e.$reset = () => e.$patch(tt(t));
	});
	let r = pn(), i = De(Ur).use(r).use(at, {
		directive: "tippy",
		component: "tippy"
	}).use(n);
	return i.directive("focus-container", mi), i.directive("focus-list", $r), i.directive("focus-item", ri), i.directive("truncate", vi), i.component("panel-screen", ji), i.component("pin", Fi), i.component("close", Ri), i.component("back", Vi), i.component("expand", Ki), i.component("panel-options-menu", ra), i.component("dropdown-menu", Cn), i.component("minimize", Yi), i.component("right", $i), i.component("left", na), i.component("fullscreen-nav-button", oa), i.component("geolocator-nav-button", sa), i.component("home-nav-button", ca), i.component("mapnav-button", da), i.component("appbar-button", ma), i.component("transition", Te), i.component("transition-group", Ee), i.config.globalProperties.$iApi = t, i.config.globalProperties.$pinia = n, i.config.idPrefix = `ramp-${e.id || Js.sharedUtils.generateUUID()}`, i.provide("iApi", t), {
		element: i,
		app: i.mount(e),
		i18n: r.global
	};
...
var lo = class e {
	...
	static async create(t) {
		let n = new e(t), r = t.layers.map((e) => {
			...
			if (e.layerType === I.TILE) return K.TileLayer(t);
			if (e.layerType === I.VECTORTILE) return K.VectorTileLayer(t);
			if (e.layerType === I.MAPIMAGE) return K.MapImageLayer(t);
			if (e.layerType === I.OSM) return K.OpenStreetMapLayer(t);
	...
	async createMap(e, t) {
		let n = (e) => {
			this.$iApi.notify.show(kn.ERROR, this.$iApi.$i18n.t("layer.error", { id: e.name }));
		}, r = e.basemaps.map((e) => lo.create(e)), i = (await Promise.all(r)).filter((e) => (e.createErr && n(e), e.esriBasemap.baseLayers.length > 0));
		i.forEach((e) => {
		...
		}), this._basemapStore = i, e.labelsDefault && (this.labelsDefault.visible = e.labelsDefault.visible), this.esriMap = w(await K.Map({})), this.pointZoomScale = e.pointZoomScale && e.pointZoomScale > 0 ? e.pointZoomScale : 5e4, this._targetDiv = t, this.createMapView(e.initialBasemapId);
	}
	destroyMap() {
	...
	async applyBasemap(e) {
		...
		t.esriBasemap.id !== n && (this.esriMap.basemap = We(t.esriBasemap));
	}
	async setBasemap(e) {
	...
	async zoomMapTo(e, t, n = !0, r = 200, i = "ease") {
		if (this.esriView) {
			...
			a.type === F.POINT && (o.scale = t || this.pointZoomScale);
			let s = {
				animate: n,
				duration: r,
				easing: i
	...
	getExtent() {
		return this.esriView ? Nt.fromESRI(this.esriView.extent) : (this.noMapErr(), Nt.fromParams("i_am_error", 0, 1, 0, 1));
	}
	getExtentSet() {
		return this._rampExtentSet ? this._rampExtentSet : (this.noMapErr(), Ut.fromConfig({
	...
	maptipStore;
	constructor(e) {
		super(e), this.maptipStore = mn(this.$vApp.$pinia);
	}
	#e = void 0;
	#t = void 0;
	async checkAtCoord(e) {
		...
		if (n?.geomType != F.POLYGON && this.#e && this.#e.layerId === t.layerId && this.#e.oid === t.oid && this.#e.layerIdx === t.layerIdx) return;
	...
	constructor(e) {
		super(e, "overview"), this.overviewGraphicLayer = this.$iApi.geo.layer.createLayer({
			...
			cosmetic: !0,
			system: !0
		}), this.overviewmapStore = ro(this.$vApp.$pinia);
	}
	async createMapView(e) {
		...
		this.esriView = w(await K.MapView({
			map: this.esriMap,
	...
	async addMapGraphicLayer() {
		...
		let e = new Ct(this.$iApi.geo.map.getExtent(), "overview-graphic"), t = this.overviewmapStore.borderColour ?? "#FF0000", n = this.overviewmapStore.borderWidth ?? 1, r = this.overviewmapStore.areaColour ?? "#000000", i = this.overviewmapStore.areaOpacity ?? .25;
		e.style = new Rt({
	...
	async mapDrag(e) {
		if (e.native.pointerType === "mouse") {
			if (e.action === "start") await this.cursorHitTest(e) && (this.startExtent = w(this.overviewGraphicLayer.getEsriGraphic("overview-graphic").geometry));
	...
	updateAttribution(e) {
		...
		if (e ? (e.text ? e.text.disabled ? n.text.disabled = !0 : n.text.value = e.text.value || t.value : n.text.value = t.value, nn(this.$vApp.$pinia).setAttribution(n)) : n.text.value = t.value, !e || e.text?.disabled || !e.text) {
			...
			if (r) {
				...
				Promise.all(i).then((r) => {
					e = r.filter((e) => e?.copyright).map((e) => e.copyright).join(" | "), n.text.value = e || n.text.value || t.value, nn(this.$vApp.$pinia).setAttribution(n);
				});
			}
		}
	}
	updateScale() {
		let e = nn(this.$vApp.$pinia), t = e.scale;
	...
	constructor(e) {
		super(e, "main"), this.maptip = new fo(e), this.caption = new mo(e), this.mapMouseThrottle = 0;
	}
	async createMap(e, t) {
	...
	async createMapView(e) {
		...
		this._rampExtentSet = Ut.fromConfig(a), this._rampSR = this._rampExtentSet.sr.clone();
		...
		this.esriView = w(await K.MapView({
			...
			handler: this.esriView.on("pointer-move", Ze((e) => {
	...
	async applyBasemap(e) {
		...
		this.esriMap.basemap = We(n.esriBasemap);
	...
	async setBasemap(e) {
		...
		if (i) {
			...
			let t = await K.ColorBackground({ color: new Pt(r.backgroundColour).toESRI() });
	...
	addLayer(t, n = void 0) {
		return new Promise((r, i) => {
			if (this.esriMap || (this.noMapErr(), i()), t.initiationState !== V.INITIATING && t.initiationState !== V.INITIATED && t.layerState !== H.ERROR && t.initiate(), t.mapLayer && n === void 0) {
			...
			let a = Date.now(), o = 0, s = setInterval(() => {
				o += 250, o >= t.expectedTime.fail || t.layerState === H.ERROR ? (clearInterval(s), t.lastCancel < a && console.error(`Failed to add layer: ${t.id}.`), i()) : t.initiationState === V.INITIATED && (t.esriLayer || !t.mapLayer) && (clearInterval(s), t.mapLayer ? this.insertToEsriMap(t) : t.onLoad(), r());
			}, 250);
		});
	}
	insertToEsriMap(e) {
	...
	removeLayer(t) {
		...
		let n;
		if (n = t instanceof Do ? t : this.$iApi.geo.layer.getLayer(t), !n) throw Error("Layer could not be found for removal.");
		if (n.isSublayer) {
		...
		r.removeLayer(n), r.removeLayerConfig(n.id), n.removeEsriLayer(), n.initiationState === V.INITIATED && n.terminate(), n.isRemoved = !0, this.$iApi.event.emit(Z.LAYER_REMOVE, n);
	}
	...
	runIdentify(e) {
		...
		r.type === F.POINT && t.some((e) => e.identifyMode === B.HYBRID || e.identifyMode === B.SYMBOLIC) && (i = this.symbolIdentify(n.screenX, n.screenY));
		let a = {
			geometry: r,
			hitTest: i
	...
	async getGraphicAtCoord(e) {
		...
		let t = this.$iApi.geo.layer.allLayersOnMap(!1).filter((e) => e.supportsFeatures || e.layerType === I.GRAPHIC);
		...
		if (r.some((e) => {
			...
			if (n) {
				if (!(n.isCosmetic || n.layerType === I.GRAPHIC || e.graphic.isAggregate)) i = n, a = e.graphic.getObjectId();
...
var So = class extends X {
	async fetchFileData(e, t) {
		let n = await lt.get(e, { responseType: "arrayBuffer" });
		switch (t) {
			case I.GEOJSON:
			case I.DATAJSON: return JSON.parse(this.arbToStr(n.data));
			case I.SHAPEFILE:
			case I.GEOJSONZIPPED:
			case I.FLATGEOBUF:
			case I.FLATGEOBUFZIPPED: return n.data;
			case I.CSV:
			case I.DATACSV: return this.arbToStr(n.data);
	...
	extractCsvFields(e, t = ",") {
		return pt.dsvFormat(t).parseRows(e)[0].map((e) => ({
			name: e,
			type: ht.STRING
		}));
	}
	filterCsvLatLonFields(e, t = ",") {
		let n = pt.dsvFormat(t).parseRows(e), r = n[0], i = {
	...
	async geoJsonToEsriJson(e, t) {
		...
		vo(e);
		let o = e.features[0].geometry.type, s = _o[o], c = JSON.parse(JSON.stringify(go[s]));
		if (t) {
	...
	inferType(e) {
		return typeof e == "number" ? ht.DOUBLE : ht.STRING;
	}
	...
	async loadWfsData(e, t = -1, n = 0, r = 1e3, i = {
		...
		let c = new Gt(e).updateQuery(s), [l, u] = await ut(lt.get(c, { signal: o }));
		...
		} else return a && i.features.length > 0 && i.features[0].geometry.type === z.POINT && i.features.forEach((e) => {
	...
	ogc;
	constructor(e) {
		super(e), this.files = new So(e), this.ogc = new wo(e);
	}
	createLayer(e) {
		let t;
		switch (e.layerType) {
	...
	allActiveLayers() {
		return this.allLayers().filter((e) => e.layerState !== H.ERROR && e.initiationState === V.INITIATED);
	}
	allLayersOnMap(e = !0) {
		let t = this.allLayers().filter((e) => e.mapLayer && e.initiationState === V.INITIATED);
		if (e) {
	...
	allDataLayers() {
		return this.allLayers().filter((e) => !e.mapLayer && e.initiationState === V.INITIATED);
	}
	allErrorLayers() {
		return this.allLayers().filter((e) => e.layerState === H.ERROR);
	}
	allInitiatingLayers() {
		return this.allLayers().filter((e) => e.initiationState === V.INITIATING);
	}
	getLayerControls(e) {
		let t = this.getLayer(e);
		if (!t) return;
	...
	async loadLayerMetadata(e) {
		...
		let [t, n] = await ut(ue(e, { query: { f: "json" } }));
		...
		} else i.dataFormat = R.ESRI_RASTER;
		return i;
	}
	...
	_sublayers;
	constructor(e, t) {
		super(t), this.config = e, this.id = "", this.uid = "", this.name = "error", this.layerState = H.NEW, this.drawState = U.NOT_LOADED, this.initiationState = V.NEW, this.layerIdx = -1, this.layerFormat = L.UNKNOWN, this.layerType = I.UNKNOWN, this.dataFormat = R.UNKNOWN, this.supportsIdentify = !1, this.identifyMode = B.NONE, this.supportsFeatures = !1, this.mapLayer = !0, this.featureCount = 0, this.fields = [], this.fieldList = "", this.nameField = "", this.maptipField = "", this.oidField = "", this.supportsSublayers = !1, this.isSublayer = !1, this.isRemoved = !1, this.isFile = !1, this.isCosmetic = !1, this.isSystem = !1, this.userAdded = !1, this.identify = !1, this.maptips = !1, this.geomType = F.UNKNOWN, this.legend = [], this._sublayers = [], this.expectedTime = {
			draw: 0,
			load: 0,
			fail: 0
	...
	getLayerTree() {
		return new Bt(0, "Fake tree", "getLayerTree() was not implemented in layer");
	}
	getSR() {
		return q.latLongSR();
	}
	...
	getGraphic(e, t) {
		return Promise.resolve(new Ct(new Tt()));
	}
	getIcon(e) {
		return Promise.resolve("");
	}
	...
	constructor(e, t) {
		super(e, t), this.name = e.name || "", this.geomType = F.NONE, this.dataFormat = R.UNKNOWN, this.layerType = I.UNKNOWN, this.layerFormat = L.UNKNOWN;
		...
		this.expectedTime.draw = e.expectedDrawTime ?? n.draw, this.expectedTime.load = e.expectedLoadTime ?? n.load, this.expectedTime.fail = e.maxLoadTime || n.fail, this.lastCancel = 0, this.timers = {
			draw: void 0,
			load: void 0
		}, this.origRampConfig = e, this.id = e.id || "", this.uid = this.$iApi.geo.shared.generateUUID(), this.isCosmetic = !1, this.isSystem = e.system || !1, this.isRemoved = !1, this.isSublayer = !1, this.supportsIdentify = !1, this.mapLayer = !0, this.identifyMode = B.NONE, this.supportsFeatures = !1, this.maptips = !1, this.supportsSublayers = !1, this.isFile = !1, this.layerState = H.NEW, this.initiationState = V.NEW, this.drawState = U.NOT_LOADED, this.loadDefProm = new Y(), this.url = this.origRampConfig.url, this.canReload = !!(this.url || this.origRampConfig.caching), this.loadPromDone = !1, this.nameArcadeFormula = "", this.maptipArcadeFormula = "", this.layerTree = new Bt(0, this.uid, this.name, !0);
	}
	updateInitiationState(e) {
	...
	updateDrawState(e) {
		this.drawState = e, e === U.REFRESH ? this.startTimer(Oo.DRAW) : e === U.UP_TO_DATE && this.stopTimer(Oo.DRAW), this.$iApi.event.emit(Z.LAYER_DRAWSTATECHANGE, {
			state: e,
			layer: this
		});
	}
	async initiate() {
		this.updateInitiationState(V.INITIATING), this.startTimer(Oo.LOAD);
		let e = (e) => {
			this.layerState !== H.ERROR && (console.error(e), this.onError());
		}, t = Date.now(), n = setTimeout(() => {
			t > this.lastCancel && (this.lastCancel = Date.now(), e("Layer timed out during initialize. Id: " + this.id));
		}, this.expectedTime.fail), [r] = await ut(this.onInitiate());
		clearTimeout(n), t > this.lastCancel && (this.drawState !== U.UP_TO_DATE && this.startTimer(Oo.DRAW), r ? e(`Init error on layer id: ${this.id} . ${r.message}`) : this.updateInitiationState(V.INITIATED));
	}
	async onInitiate() {
		if (this.isSublayer) return console.warn("Attempted to initiate a sublayer as a CommonLayer"), Promise.resolve();
		this.initiationState === V.INITIATED && console.error(`Encountered layer initialize while already initiated, layer id ${this.id}`);
	}
	async terminate() {
		this.updateInitiationState(V.TERMINATING), await Promise.all(this.sublayers.map((e) => e.terminate())), this.loadDefProm = new Y(), this.loadPromDone = !1, this.updateLayerState(H.NEW), this.updateDrawState(U.NOT_LOADED), this.updateInitiationState(V.TERMINATED);
	}
	onLoad() {
		...
		try {
			...
			Promise.all(r).then(() => {
				clearTimeout(n), t ? this.visibility = !1 : (this.stopTimer(Oo.LOAD), e > this.lastCancel && (this.loadPromDone && (this.loadDefProm = new Y()), this.loadDefProm.resolveMe(), this.loadPromDone = !0, this.sublayers.forEach((e) => e.onLoad()), this.updateLayerState(H.LOADED)));
			}).catch(() => {
	...
	onError(e = !0) {
		this.layerState !== H.ERROR && (this.initiationState === V.INITIATING && this.updateInitiationState(V.NEW), this.loadPromDone && (this.loadDefProm = new Y()), this.loadDefProm.rejectMe(), this.loadPromDone = !0, this.sublayers.forEach((t) => t.onError(e)), e && this.$iApi.notify.show(kn.ERROR, this.$iApi.$i18n.t("layer.error", { id: this.id })), this.stopTimer(Oo.DRAW), this.stopTimer(Oo.LOAD), this.updateLayerState(H.ERROR, !e));
	}
	onLoadActions() {
		return [];
	}
	cancelLoad() {
		this.isLoaded || this.initiationState === V.NEW || this.initiationState === V.TERMINATING || this.initiationState === V.TERMINATED || (this.lastCancel = Date.now(), this.esriLayer && this.esriLayer.loadStatus === "loading" && this.esriLayer.cancelLoad(), this.removeEsriLayer(), this.onError(!1));
	}
	loadPromise() {
		return this.loadDefProm.getPromise();
	}
	get isLoaded() {
		return this.layerState === H.LOADED;
	}
	canIdentify() {
	...
	getGraphic(e, t) {
		return this.stubError(), Promise.resolve(new Ct(new Tt()));
	}
	getIcon(e) {
		return this.stubError(), Promise.resolve("");
	}
	...
	startTimer(e) {
		this.stopTimer(e), this.expectedTime[e] > 0 && (this.timers[e] = window.setTimeout(() => this.$iApi.notify.show(kn.WARNING, this.$iApi.$i18n.t(`layer.long${e}`, { id: this.name || this.id })), this.expectedTime[e]));
	}
	stopTimer(e) {
	...
	esriWatches;
	constructor(e, t) {
		super(e, t), this._scaleSet = new Vt(), this._mouseTolerance = e.mouseTolerance == null ? 5 : e.mouseTolerance, this._touchTolerance = e.touchTolerance == null ? 15 : e.touchTolerance, this._drawOrder = [], this._serverVisibility = void 0, this.isCosmetic = e.cosmetic || !1, this.extent = e.extent ? Nt.fromConfig(`${this.id}_extent`, e.extent) : void 0, this.viewDefProm = new Y(), this.esriWatches = [];
	}
	noLayerErr() {
	...
	async onInitiate() {
		...
		this.esriWatches.push(s(() => this.esriLayer.visible, (e) => {
			...
			this.esriView = w(e.layerView), this.esriWatches.push(s(() => e.layerView.updating, (e) => {
				this.updateDrawState(e ? U.REFRESH : U.UP_TO_DATE);
	...
	get layerExists() {
		return this.initiationState === V.INITIATED && !!this.esriLayer;
	}
	get drawOrder() {
	...
	filter;
	constructor(e, t) {
		super(e, t), this.supportsIdentify = !0, this.geomType = F.UNKNOWN, this.serviceUrl = "", this.fieldList = "", this.canModifyLayer = !0, this.filter = new Ht(e.permanentFilteredQuery || "", e.initialFilteredQuery || ""), e.state?.hovertips !== void 0 && console.warn("hovertips layer configuration property is deprecated. Please adjust to use maptips instead"), this.maptips = e.state?.maptips ?? e.state?.hovertips ?? !0, this.attribs = new is();
	}
	makeEsriLayerConfig(e) {
	...
	async loadLayerMetadata(e = {}) {
		...
		if (!(t < this.lastCancel)) if (this.geomType = n.geometryType, this.attribs.quickCache = new us(this.geomType), this.scaleSet.minScale = n.minScale, this.scaleSet.maxScale = n.maxScale, this.dataFormat = n.dataFormat, this.extent = this.extent ?? n.extent, this._serverVisibility = n.defaultVisibility, this.origRampConfig.name || (this.name = n.name ?? this.id), this.dataFormat === R.ESRI_FEATURE) {
			this.supportsFeatures = !0, this.canModifyLayer = this.layerType === I.SUBLAYER ? n.canModifyLayer : !0, this.fields = n.fields, this.nameField = n.displayField, this.oidField = n.objectIdField, this.sourceSR = n.sourceSR, this.drawOrder.forEach((e) => {
			...
			this.attribs.attLoader = new ss(this.$iApi, r);
	...
	async getGraphic(e, t) {
		let n = {}, r = new Tt(), i = this.$iApi.geo.map, a = !1, o = !1, s = 0;
		...
		if (t.getGeom) {
			...
			if (n) r = n;
			else if (this.layerType === I.FEATURE && !(this.attribs.quickCache.isPoint && t.forZoom)) {
		...
		let c = new Ct(r, "", t.getAttribs ? n : void 0);
		if (t.getStyle) {
			let e = We(this.renderer.getGraphicSymbol(n));
	...
	setSqlFilter(e, t) {
		if (t !== this.filter.getSql(e)) if (this.filter.setSql(e, t), this.$iApi.event.emit(Z.FILTER_CHANGE, {
			uid: this.uid,
			filterKey: e
		}), this.layerFormat === L.MAPIMAGE) this.applySqlFilter();
		else {
			let n = `${this.uid}-${e}-${t}`;
	...
	constructor(e, t) {
		super(e, t), this.dataFormat = R.ESRI_FEATURE, this.layerFormat = L.GRAPHIC, this.maptips = !1;
	}
	_graphics = [];
	makeEsriLayerConfig(e) {
		return super.makeEsriLayerConfig(e);
	...
	async addGraphic(e) {
		...
		let n = t.filter((e) => this._graphics.findIndex((t) => t.id === e.id) === -1 ? (this._graphics.push(e), !0) : (console.error(`Attempting to add graphic with id '${e.id}' that has already been added.`), !1)), r = this.$iApi.geo.map.getSR(), i = n.map((e) => this.$iApi.geo.proj.projectGeometry(r, e.geometry)), a = await Promise.all(i), o = n.map((e, t) => {
			let n = new Ct(a[t], e.id, e.attributes);
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.GRAPHIC;
	}
	async onInitiate() {
		this.esriLayer = w(await K.GraphicsLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
		return super.makeEsriLayerConfig(e);
	}
	onLoadActions() {
		let e = super.onLoadActions();
		return this.layerTree.name = this.name, this.updateDrawState(U.UP_TO_DATE), e;
	}
}, Po = class extends jo {
	esriJson;
	sourceGeoJson;
	constructor(e, t) {
		super(e, t), this.supportsIdentify = !0, this.isFile = !0, this.dataFormat = R.ESRI_FEATURE, this.layerFormat = L.FEATURE, this.layerIdx = 0, e.identifyMode && e.identifyMode !== B.NONE ? this.identifyMode = e.identifyMode : this.identifyMode = B.HYBRID;
	}
	async reload() {
	...
	async onInitiate() {
		...
		this.esriJson = await this.$iApi.geo.layer.files.geoJsonToEsriJson(this.sourceGeoJson, e), this.esriLayer = w(await K.FeatureLayer(this.makeEsriLayerConfig(this.origRampConfig))), this.esriJson = void 0, this.origRampConfig.caching || delete this.origRampConfig.rawData, delete this.sourceGeoJson, await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
	...
	runIdentify(e) {
		...
		if (this.identifyMode === B.HYBRID || this.identifyMode === B.GEOMETRIC) {
			let t = { includeGeometry: !1 };
			this.geomType !== F.POLYGON && e.geometry.type === F.POINT ? t.filterGeometry = this.$iApi.geo.query.makeClickBuffer(e.geometry, e.tolerance) : t.filterGeometry = e.geometry, t.filterSql = this.getCombinedSqlFilter(), i = this.queryFeatures(t).then((e) => {
				a = e;
			});
		}
		return e.hitTest && (this.identifyMode === B.HYBRID || this.identifyMode === B.SYMBOLIC) && (r = i.then(async () => {
			...
			a.forEach((e) => {
				n.items.push(To.makeRawItem(St.ESRI, e.attributes));
			}), o.forEach((e) => {
				n.items.push(To.makeOidItem(e, this));
	...
	extractLayerMetadata() {
		...
		this.supportsFeatures = !0, this.geomType = this.$iApi.geo.geom.clientGeomTypeToRampGeomType(e.geometryType), this.scaleSet.minScale = e.minScale || 0, this.scaleSet.maxScale = e.maxScale || 0, this.extent = this.extent ?? Nt.fromESRI(e.fullExtent, this.id + "_extent"), this.fields = w(e.fields.slice()).map((e) => ({
			name: e.name,
			alias: e.alias,
			type: e.type,
			length: e.length
		...
		this.attribs.attLoader = new cs(this.$iApi, t);
	}
	async getGraphic(e, t) {
		let n;
		if (!t.getGeom && this.attribs.attLoader.isLoaded()) {
			let t = await this.attribs.attLoader.getAttribs();
			n = new Ct(new Tt(), "", t.features[t.oidIndex[e]]);
		} else {
			let r = {
				filterOIDs: [e],
		...
		if (t.getStyle) {
			let e = We(this.renderer.getGraphicSymbol(n.attributes));
	...
	applySqlFilter(e = []) {
		...
		K.FeatureFilter({ where: t }).then((e) => {
			We(this.esriView).filter = e;
		});
	}
}, Fo = class extends Po {
	constructor(e, t) {
		super(e, t), this.layerType = I.GEOJSON;
	}
	async onInitiate() {
		let e, t = Date.now();
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.SHAPEFILE;
	}
	async onInitiate() {
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.CSV;
	}
	async onInitiate() {
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.GEOJSONZIPPED;
	}
	async onInitiate() {
		let e = Date.now(), t = await this.$iApi.geo.layer.files.binaryInitHelper(this.origRampConfig);
		if (e > this.lastCancel) {
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.FLATGEOBUF;
	}
	async onInitiate() {
		let e = Date.now(), t = await this.$iApi.geo.layer.files.binaryInitHelper(this.origRampConfig);
		if (e > this.lastCancel) {
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.FLATGEOBUFZIPPED;
	}
	async onInitiate() {
		let e = Date.now(), t = await this.$iApi.geo.layer.files.binaryInitHelper(this.origRampConfig);
		if (e > this.lastCancel) {
	...
	constructor(e, t) {
		super(e, t), this.dataFormat = R.ESRI_FEATURE, this.supportsIdentify = !0, this.layerType = I.FEATURE, this.layerFormat = L.FEATURE, e.identifyMode && e.identifyMode !== B.NONE ? this.identifyMode = e.identifyMode : this.identifyMode = B.HYBRID;
	}
	async onInitiate() {
		w(this.esriLayer = await K.FeatureLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
	...
	runIdentify(e) {
		...
		if (this.identifyMode === B.HYBRID || this.identifyMode === B.GEOMETRIC) {
			...
			this.geomType !== F.POLYGON && e.geometry.type === F.POINT ? t.filterGeometry = this.$iApi.geo.query.makeClickBuffer(e.geometry, e.tolerance) : t.filterGeometry = e.geometry, t.filterSql = this.getCombinedSqlFilter(), i = this.queryOIDs(t).then((e) => {
				a = e;
			});
		}
		return e.hitTest && (this.identifyMode === B.HYBRID || this.identifyMode === B.SYMBOLIC) && (r = i.then(async () => {
	...
	async getGraphicExtent(e) {
		...
		else if (this.geomType === F.POINT) console.error(`Attempted to query extent for invalid geometry type ${this.geomType}.`);
		else {
			let n = this.attribs.quickCache.getExtent(e);
			if (n) t = n;
			else {
				let [n, r] = await ut(this.esriLayer.queryExtent({
					objectIds: [e],
					outSpatialReference: this.$iApi.geo.map.getSR().toESRI()
				}));
				!n && r.extent && (t = Nt.fromESRI(r.extent), this.attribs.quickCache.setExtent(e, t));
			}
		}
		return t;
	}
	...
	async zoomToFeature(e) {
		let t;
		return this.geomType !== F.POINT && (t = this.attribs.quickCache.getAnyScaleGeom(e), t || (t = await this.getLocalGeometry(e), t ? this.geomType !== F.MULTIPOINT && this.attribs.quickCache.setGeom(e, t, this.$iApi.geo.map.getScale()) : t = await this.getGraphicExtent(e))), t && !t.invalid() ? (await this.$iApi.geo.map.zoomMapTo(t), !0) : await super.zoomToFeature(e);
	}
}, Ho = class extends Ao {
	isDynamic;
	origState;
	constructor(e, t) {
		super(e, t), this.supportsIdentify = !0, this.supportsSublayers = !0, this.layerType = I.MAPIMAGE, this.layerFormat = L.MAPIMAGE, this.isDynamic = !1, this.maptips = !1, this.layerTree.layerIdx = -1, this.identifyMode = B.GEOMETRIC;
	}
	actualViz = !1;
	get visibility() {
	...
	set visibility(e) {
		...
		if (e || Uo(this, "1=2"), this.actualViz = e, e && !this.esriLayer.visible) this.esriLayer.visible = !0;
		else if (!e && this.esriLayer.visible) {
			let e = "", t = setTimeout(() => {
				this.$iApi.event.off(e), this.actualViz || (this.esriLayer.visible = !1, Uo(this, ""));
			}, 1600);
			e = this.$iApi.event.on(Z.LAYER_DRAWSTATECHANGE, (n) => {
				n.layer.id === this.id && n.state === U.UP_TO_DATE && (clearTimeout(t), this.$iApi.event.off(e), this.actualViz || (this.esriLayer.visible = !1, Uo(this, "")));
			});
		}
		e && Uo(this, "");
	}
	async onInitiate() {
		this.esriLayer = w(await K.MapImageLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
		let t = super.makeEsriLayerConfig(e);
	...
	onLoadActions() {
		...
		let t = Date.now();
		if (this.layerTree.name = this.name, !this.esriLayer.capabilities.exportMap) throw this.$iApi.notify.show(kn.WARNING, this.$iApi.$i18n.t("layer.noexportmap", { name: this.name || this.id })), Error("Service does not support Map Image Layer, Map Export is not enabled");
		this.isDynamic = this.esriLayer.capabilities.exportMap.supportsDynamicLayers, this.extent = this.extent ?? Nt.fromESRI(this.esriLayer.fullExtent, this.id + "_extent");
		let n = (e) => {
	...
	runIdentify(e) {
		...
		if (t.length === 0) return [];
		let n;
		return e.geometry.type === F.POINT && (n = this.$iApi.geo.query.makeClickBuffer(e.geometry, e.tolerance)), t.map((t) => {
			let r = new Y(), i = {}, a = ze({
				items: [],
			...
			return t.geomType !== F.POLYGON && n ? i.filterGeometry = n : i.filterGeometry = e.geometry, i.filterSql = t.getCombinedSqlFilter(), i.sourceSR = t.sourceSR, t.queryOIDs(i).then((e) => {
				e.forEach((e) => {
					a.items.push(To.makeOidItem(e, t));
	...
	fetchEsriSublayer(e) {
		...
		this.esriSubLayer = w(e.esriLayer.allSublayers.find((e) => e.id === this.layerIdx));
	}
	onLoadActions() {
	...
	async onInitiate() {
		this.initiationState = V.INITIATED;
	...
	get layerExists() {
		return !!(this.parentLayer?.layerExists && this.esriSubLayer && this.initiationState === V.INITIATED);
	}
	get visibility() {
	...
	constructor(e, t) {
		super(e, t), this.supportsIdentify = !1, this.layerType = I.OSM, this.layerFormat = L.OSM, this.dataFormat = R.OSM_TILE, this.supportsFeatures = !1;
	}
	async onInitiate() {
		this.esriLayer = w(await K.OpenStreetMapLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
		return super.makeEsriLayerConfig(e);
	}
	...
	constructor(e, t) {
		super(e, t), this.supportsIdentify = !1, this.layerType = I.IMAGERY, this.layerFormat = L.IMAGERY, this.dataFormat = R.ESRI_RASTER;
	}
	async onInitiate() {
		this.esriLayer = w(await K.ImageryLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
		return super.makeEsriLayerConfig(e);
	}
	...
	schemaLocked = !0;
	constructor(e, t) {
		super(e, t), this.supportsIdentify = !1, this.dataFormat = R.ESRI_TILE;
	}
	onLoadActions() {
		let e = super.onLoadActions();
	...
	async checkProj() {
		let e = this.getSR(), t = this.$iApi.geo.map.getSR().isEqual(e), n = () => this.$iApi.notify.show(kn.WARNING, this.$iApi.$i18n.t("layer.mismatch", { name: this.name || this.id }));
		if (this.layerState === H.LOADED && !t) n(), this.onError(), this.updateDrawState(U.NOT_LOADED);
		else if (this.layerState === H.ERROR && t) this.reload();
		else if (this.layerState !== H.ERROR && !t) return n(), Promise.reject();
	}
}, Jo = class extends qo {
	constructor(e, t) {
		super(e, t), this.layerType = I.IMAGERYTILE, this.layerFormat = L.IMAGERYTILE, this.schemaLocked = !1;
	}
	async onInitiate() {
		this.esriLayer = w(await K.ImageryTileLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
		return super.makeEsriLayerConfig(e);
	}
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.TILE, this.layerFormat = L.TILE;
	}
	async onInitiate() {
		this.esriLayer = w(await K.TileLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
		return super.makeEsriLayerConfig(e);
	}
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.VECTORTILE, this.layerFormat = L.VECTORTILE;
	}
	async onInitiate() {
		this.esriLayer = w(await K.VectorTileLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
		return super.makeEsriLayerConfig(e);
	}
	...
	mimeType;
	constructor(e, t) {
		super(e, t), this.supportsIdentify = !0, this.layerType = I.WMS, this.layerFormat = L.WMS, this.mimeType = e.featureInfoMimeType || "", this.sublayerNames = [], this.dataFormat = R.OGC_RASTER, this.identifyMode = B.GEOMETRIC;
	}
	async onInitiate() {
		this.esriLayer = w(await K.WMSLayer(this.makeEsriLayerConfig(this.origRampConfig))), await super.onInitiate();
	}
	makeEsriLayerConfig(e) {
	...
	runIdentify(e) {
		if (e.geometry.type !== F.POINT || !this.canIdentify()) return [];
		let t = new Y(), n = ze({
			items: [],
			loading: t.getPromise(),
			loaded: !1,
		...
		return this.getFeatureInfo(this.sublayerNames, e.geometry, this.mimeType).then((e) => {
			if (e) {
				let t = !0, r;
				typeof e == "string" ? e.indexOf("Search returned no results") === -1 && e !== "" ? r = St.TEXT : (t = !1, r = St.UNKNOWN) : r = St.JSON, t && n.items.push(To.makeRawItem(r, e));
			}
			n.loaded = !0, t.resolveMe();
		}).catch(() => {
			n.errored = !0, t.resolveMe();
	...
	getFeatureInfo(e, t, n) {
		...
		let h = {
			...
			LAYERS: l,
			INFO_FORMAT: n
		}, ee = i.customLayerParameters;
		return ee && Object.keys(ee).forEach((e) => {
			e.toLowerCase() !== "styles" && (h[e] = ee[e]);
		}), Object.keys(h).forEach((e) => o[e] = h[e]), ue(i.url.split("?")[0], {
			query: o,
			responseType: p
		});
	}
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.WFS;
	}
	async onInitiate() {
		let e = Date.now(), { offset: t, limit: n } = new Gt(this.config.url).queryMap, r = await this.$iApi.geo.layer.ogc.loadWfsData(this.config.url, -1, parseInt(t) || 0, parseInt(n) || 1e3, void 0, this.config.xyInAttribs);
	...
	_visibility;
	constructor(e, t) {
		super(e, t), this.dataFormat = R.ESRI_FEATURE, this.layerFormat = L.NOLAYER, this.drawState = U.NOT_VISUAL, this.attribs = new is(), this.supportsFeatures = !0, this.mapLayer = !1, this.isFile = !1, this._visibility = e.state?.visibility ?? !0, this.expectedTime.draw = 0;
	}
	async onInitiate() {
		if (this.sourceJson) {
			...
			this.attribs.attLoader = new ls(this.$iApi, i), await this.attribs.attLoader.getAttribs(), this.featureCount = e.data.length;
	...
	async reload() {
		this.$iApi.event.emit(Z.LAYER_RELOAD_START, this), await this.terminate(), await this.initiate(), this.layerState !== H.ERROR && this.onLoad(), setTimeout(() => {
	...
	constructor(e, t) {
		super(e, t), this.layerType = I.DATAJSON;
	}
	async onInitiate() {
		let e = Date.now(), t;
	...
	serviceUrl;
	constructor(e, t) {
		super(e, t), this.layerType = I.DATATABLE, this.serviceUrl = e.url, this.filter = new Ht(e.permanentFilteredQuery || "", e.initialFilteredQuery || "");
	}
	async onInitiate() {}
	onLoadActions() {
		let e = super.onLoadActions(), t = Date.now(), n = this.$iApi.geo.shared.parseUrlIndex(this.serviceUrl).index || 0, r = this.$iApi.geo.layer.loadLayerMetadata(this.serviceUrl).then((e) => {
			...
			this.attribs.attLoader = new ss(this.$iApi, n), this.attribs.quickCache = new us(this.geomType);
	...
	async arcGisBatchLoad(e, t) {
		...
		let n = e.permanentFilter ? ` AND ${e.permanentFilter}` : "", r = { query: {
			...
			f: "json"
		} }, [i, a] = await ut(ue(e.serviceUrl + "/query", r));
	...
	async loadGraphicsAttributes(e, t) {
		...
		let n = {
			features: e.sourceGraphics.map((e) => We(e).attributes).toArray(),
			oidIndex: {}
		};
	...
	async loadSingleFeature(e) {
		...
		let [n, r] = await ut(ue(e.serviceUrl + "/query", t));
	...
	async generateTabularAttributesWorker(e, t) {
		if (e.dataFormat === R.ESRI_RASTER) throw Error("Attempting to get attributes on a raster layer.");
		...
		let r = e.fields.filter((e) => Object.prototype.hasOwnProperty.call(n.features[0], We(e).name)).map((e) => ({
			data: We(e).name,
			title: We(e).alias || We(e).name
		})), i = n.features.map((t) => {
			let n = _e({}, t);
	...
	constructor(e, t) {
		super(e), this.aac = new as(), this.details = t;
	}
	updateFieldList(e) {
	...
	getAttribs() {
		return this.loadPromise ||= (this.aac = new as(), this.loadPromiseGenerator()), this.loadPromise;
	}
	abortAttribLoad() {
	...
	isPoint;
	constructor(e) {
		this.attribs = {}, this.geoms = {}, this.extents = {}, this.isPoint = e === F.POINT || e === F.MULTIPOINT;
	}
	getScaleStore(e) {
	...
	falseRenderer;
	constructor(e, t, n = !1) {
		this.innerRenderer = e, this.symbolUnits = [], this.falseRenderer = n, this.type = xt.Unknown;
	}
	makeSearchParams(e) {
		return e;
	}
	...
	parent;
	constructor(e) {
		this.parent = e, this.symbol = new me();
	}
	match(e) {
		return !!e;
	}
	...
	keyFields;
	constructor(e, t, n = !1) {
		super(e, t, n), this.type = xt.Unique, this.delim = e.fieldDelimiter || ", ";
		let r = (e) => e.replace(/'/g, "''");
		this.keyFields = [
			e.field,
			e.field2,
	...
	constructor(e, t, n = !1) {
		if (super(e, t, n), this.valField = this.cleanFieldName(e.field, t), e.classBreakInfos.forEach((e, t) => {
			let n = t === 0, r = new _s(this, e.minValue, e.maxValue, n);
	...
	makeRenderer(e, t, n = !1) {
		switch (e.type) {
			case this.SIMPLE: return new ps(e, t);
			case this.CLASS_BREAKS: return new gs(e, t, n);
			case this.UNIQUE_VALUE: return new ms(e, t, n);
			default: throw Error(`Unknown renderer type encountered - ${e.type}`);
		}
	}
	async generateWMSSymbology(e) {
		let t = mt(document.createElement("div")).size(this.CONTAINER_SIZE, this.CONTAINER_SIZE).viewbox(0, 0, 0, 0), n = {
			svgcode: "",
			imgHeight: "",
			imgWidth: ""
		};
	...
	async renderSymbologyImage(e, t = null) {
		t === null && (t = mt(document.createElement("div")).size(this.CONTAINER_SIZE, this.CONTAINER_SIZE).viewbox(0, 0, 0, 0));
	...
	async renderSymbologyIcon(e, t = null) {
		if (t === null) {
			...
			e.setAttribute("style", "opacity:0;position:fixed;left:100%;top:100%;overflow:hidden"), document.body.appendChild(e), t = mt(e).size(this.CONTAINER_SIZE, this.CONTAINER_SIZE).viewbox(0, 0, this.CONTAINER_SIZE, this.CONTAINER_SIZE);
		}
	...
	generatePlaceholderSymbology(e, t = "#000") {
		let n = mt(document.createElement("div")).size(this.CONTAINER_SIZE, this.CONTAINER_SIZE).viewbox(0, 0, this.CONTAINER_SIZE, this.CONTAINER_SIZE);
	...
	async generateBlankSymbology() {
		return mt(document.createElement("div")).size(this.CONTAINER_SIZE, this.CONTAINER_SIZE).svg();
	}
	async symbolToSvg(e) {
		...
		let i = mt(r).size(this.CONTAINER_SIZE, this.CONTAINER_SIZE).viewbox(0, 0, this.CONTAINER_SIZE, this.CONTAINER_SIZE), a = {
			path({ size: e, path: t }) {
			...
			[_t.DASH]: "5.333,4",
			[_t.DOT]: "1.333,4",
			[_t.DASHDOT]: "5.333,4,1.333,4",
			[_t.LONGDASH]: "10.666,4",
			[_t.LONGDASHDOT]: "10.666,4,1.333,4",
			[_t.LONGDASHDOTDOT]: "10.666,4,1.333,4,1.333,4",
			[_t.SHORTDOT]: "1.333,1.333",
			[_t.SHORTDASH]: "5.333,1.333",
			[_t.SHORTDASHDOT]: "5.333,1.333,1.333,1.333",
			[_t.SHORTDASHDOTDOT]: "5.333,1.333,1.333,1.333,1.333,1.333"
		}, s = {
			color: "#000",
			opacity: 1,
			width: 1,
	...
	async getMapServerLegend(e) {
		...
		let t = { query: { f: "json" } }, n = { layers: [] }, [r, i] = await ut(ue(`${e}/legend`, t));
	...
	async geoJsonQuery(e) {
		...
		return (await e.layer.esriLayer.queryFeatures(t)).features.map((e, n) => {
			let r;
			return r = t.returnGeometry ? this.$iApi.geo.geom.geomEsriToRamp(e.geometry, `queryResult${n}`) : new Tt(), new Ct(r, "", e.attributes);
		});
	}
	async queryGeometryHelper(e, t, n, r) {
		return !t && e.type === F.EXTENT && r && !r.isEqual(e.sr) && !(n && n > 2e7 && e.sr.wkid === 3978 && r.wkid === 4326) ? (await this.$iApi.geo.proj.projectExtent(r, e)).toESRI() : e.toESRI();
	}
	makeClickBuffer(e, t = 5) {
		let n = this.$iApi.geo.map, r = n.getExtent(), i = t * (r.xmax - r.xmin) / n.getPixelWidth();
		return new Nt("ze_buffer", [e.x - i, e.y - i], [e.x + i, e.y + i], e.sr);
	}
}, bs = class extends X {
	attributes;
	geom;
	...
	symbology;
	constructor(e) {
		super(e), this.geom = Js.geom, this.proj = Js.proj, this.shared = Js.sharedUtils, this.map = new ho(e), this.layer = new Eo(e), this.attributes = new rs(e), this.query = new ys(e), this.symbology = new vs(e), Array.isArray(o.request.interceptors) || (o.request.interceptors = []), o.request.interceptors.push({ before: (e) => {
...
}, xs = /* @__PURE__ */ Object.assign({
	"../fixtures/appbar/index.ts": () => import("./appbar-DdWfi_Xi.js"),
	"../fixtures/areas-of-interest/index.ts": () => import("./areas-of-interest-BqbvGFyP.js"),
	"../fixtures/basemap/index.ts": () => import("./basemap-u-BFhSTD.js"),
	"../fixtures/crosshairs/index.ts": () => import("./crosshairs-DZmZKDOY.js"),
	"../fixtures/details/index.ts": () => import("./details-BF8StEC5.js"),
	"../fixtures/draw/index.ts": () => import("./draw-BaC9Sk3P.js"),
	"../fixtures/export/index.ts": () => import("./export-CNTxZI0I.js"),
	"../fixtures/export-footnote/index.ts": () => import("./export-footnote-Cj-ou4ec.js"),
	"../fixtures/export-legend/index.ts": () => import("./export-legend-tS5JYJ6c.js"),
	"../fixtures/export-map/index.ts": () => import("./export-map-DWPHmv5r.js"),
	"../fixtures/export-northarrow/index.ts": () => import("./export-northarrow-B-J7P-87.js"),
	"../fixtures/export-scalebar/index.ts": () => import("./export-scalebar-C4EvrpTF.js"),
	"../fixtures/export-timestamp/index.ts": () => import("./export-timestamp-CpIVO3Hk.js"),
	"../fixtures/export-title/index.ts": () => import("./export-title-DIRki0oQ.js"),
	"../fixtures/extentguard/index.ts": () => import("./extentguard-B_uWmV9i.js"),
	"../fixtures/gazebo/index.ts": () => import("./gazebo-1gsy8hMi.js"),
	"../fixtures/geosearch/index.ts": () => import("./geosearch-CRFHWqdY.js"),
	"../fixtures/grid/index.ts": () => import("./grid-D4wa8Nrg.js"),
	"../fixtures/help/index.ts": () => import("./help-Do0726jR.js"),
	"../fixtures/hilight/index.ts": () => import("./hilight-74PfFaC-.js"),
	"../fixtures/keyboardnav/index.ts": () => import("./keyboardnav-faMaljp6.js"),
	"../fixtures/layer-reorder/index.ts": () => import("./layer-reorder-DfJWkXvr.js"),
	"../fixtures/legend/index.ts": () => import("./legend-DbxaougF.js"),
	"../fixtures/mapnav/index.ts": () => import("./mapnav-Bxt-27Sm.js"),
	"../fixtures/metadata/index.ts": () => import("./metadata-CZWPrXUi.js"),
	"../fixtures/northarrow/index.ts": () => import("./northarrow-Bqne4D33.js"),
	"../fixtures/overviewmap/index.ts": () => import("./overviewmap-SJvn_2eC.js"),
	"../fixtures/panguard/index.ts": () => import("./panguard-DEpuydU3.js"),
	"../fixtures/pindrop/index.ts": () => import("./pindrop-DGEdp05Z.js"),
	"../fixtures/scrollguard/index.ts": () => import("./scrollguard-C5QpHwZl.js"),
	"../fixtures/settings/index.ts": () => import("./settings-BcoCqt_x.js"),
	"../fixtures/snowman/index.ts": () => import("./snowman-CtvwH5mn.js"),
	"../fixtures/swipe/index.ts": () => import("./swipe-05j0usRZ.js"),
	"../fixtures/wizard/index.ts": () => import("./wizard-DBjByfd7.js")
}), Ss = class extends X {
	constructor(e) {
		super(e);
	}
	exists(e) {
		return e in on(this.$vApp.$pinia).items;
	}
	async add(e, t) {
		let n;
		if (e in on(this.$vApp.$pinia).items) return this.get(e);
		if (t) {
			if (typeof t != "function") throw Error("malformed fixture constructor");
			n = Cs.updateBaseToInstance(new t(), e, this.$iApi);
		} else {
			let t = (await xs[`../fixtures/${e}/index.ts`]()).default;
			n = new t(e, this.$iApi);
		}
		return on(this.$vApp.$pinia).addFixture(n), this.$iApi.event.emit(Z.FIXTURE_ADDED, n), this.$iApi.geo.map.created && n.initialized?.(), n;
	}
	remove(e) {
		let t = this.get(e);
		if (!t) throw Error(`Could not find fixture ${e} for removal`);
		return on(this.$vApp.$pinia).removeFixture(t), this.$iApi.event.emit(Z.FIXTURE_REMOVED, t), t;
	}
	flush() {
		let e = on(this.$vApp.$pinia);
	...
	get(e) {
		...
		let n = t.map((e) => {
			let t = on(this.$vApp.$pinia).items[e];
			if (t) return t;
		});
		return n.length === 1 ? n[0] : n;
	}
	isLoaded(e) {
		let t = on(this.$vApp.$pinia), n = Array.isArray(e) ? e : [e];
		n.forEach((e) => {
	...
	mount(e, { props: t, children: n, element: r, app: i } = {}) {
		let a = r, o = je(e, t, n);
		return i && i._context && (o.appContext = i._context), a ||= document.createElement("div"), Be(o, a), {
			vNode: o,
			destroy: () => {
				a && Be(null, a), a = null, o = null;
			},
			el: a
		};
	}
	...
	handlePanelTeleports(e) {
		if (this.config?.panelTeleport) {
			let t = bn(this.$vApp.$pinia), n = !!this.config.panelTeleport.target;
			e.forEach((e) => {
	...
	register(e, t) {
		...
		let r = Object.entries(n).reduce((e, [t, n]) => (e.push(new As(this.$iApi, t, n)), e), []);
	...
	open(e) {
		let t, n, r;
		if (typeof e == "string" || e instanceof As ? t = this.get(e) : (t = this.get(e.id), {screen: n, props: r} = e), !t) return t;
	...
	toggle(e, t) {
		let n;
		return n = typeof e == "string" || e instanceof As ? this.get(e) : this.get(e.id), n && (t = t === void 0 ? !n.isVisible : t, t === n.isVisible ? n : t ? this.open(e) : this.close(n));
	}
	toggleMinimize(e, t) {
		let n;
		return n = typeof e == "string" || e instanceof As ? this.get(e) : this.get(e.id), n && (t = t === void 0 ? !n.isVisible : t, t === n.isVisible ? n : t ? this.open(n) : this.minimize(n));
	}
	pin(e, t) {
		let n = this.get(e);
...
var Es = {};
function Ds(e, t) {
	let n = O("panel-screen");
	return E(), v(n, {
		header: !1,
		class: "screen-spinner"
	}, {
		content: M(() => [...t[0] ||= [x("div", { class: "loader" }, null, -1)]]),
		_: 1
	});
}
var Os = /* @__PURE__ */ n(Es, [["render", Ds], ["__scopeId", "data-v-a3c61802"]]), ks = /* @__PURE__ */ Object.assign({
	"../fixtures/areas-of-interest/screen.vue": () => import("./screen-CzLR5qiQ.js"),
	"../fixtures/basemap/screen.vue": () => import("./screen-Dq5-_7lK.js"),
	"../fixtures/export/screen.vue": () => import("./screen-nicAjflp.js"),
	"../fixtures/geosearch/screen.vue": () => import("./screen-CDtpplFQ.js"),
	"../fixtures/grid/screen.vue": () => import("./screen-B1OebKXC.js"),
	"../fixtures/help/screen.vue": () => import("./screen-CfI1p5vz.js"),
	"../fixtures/layer-reorder/screen.vue": () => import("./screen-Bj02BrsS.js"),
	"../fixtures/legend/screen.vue": () => import("./screen-dXZ9vUwe.js"),
	"../fixtures/metadata/screen.vue": () => import("./screen-BJHS-09A.js"),
	"../fixtures/settings/screen.vue": () => import("./screen-Dy3I9lFg.js"),
	"../fixtures/wizard/screen.vue": () => import("./screen-CkQNZskS.js")
}), As = class extends X {
	id;
	screens;
	loadedScreens = [];
	...
	registerScreen(e) {
		...
		if (Xt(t) || Yt(t)) n = t, this.loadedScreens.push(e);
		else if (Zt(t)) n = { template: `<panel-screen :panel="this" :screenId="'${e}'">
                           </panel-screen>` };
		else {
			let r;
			r = typeof t == "string" ? ks[`../fixtures/${t}`]() : t();
			let i = new Promise((t, n) => {
				r.then((n) => {
					this.loadedScreens.push(e), t(Qt(n) ? n.default : n);
				}), r.catch((e) => n(e));
			});
			n = ke({
				loader: () => i,
...
function Ns(e) {
	...
	return Bs(e.services, t), Ps(e.map, t), Vs(e.ui, t), e.plugins && Hs(e.plugins, t), t;
}
function Ps(e, t) {
	if (e.layers && e.layers.forEach((e) => {
		t.layers.unshift(Ls(e));
	...
	e.extentSets && (t.map.extentSets = [], e.extentSets.forEach((e) => {
		...
		e.overviewUrl && (t.fixtures.overviewmap || (t.fixtures.overviewmap = { basemaps: {} }, t.fixturesEnabled.push("overviewmap")), t.fixtures.overviewmap.basemaps[e.id] = {
			...
			layers: [{
				...
				layerType: e.overviewUrl.layerType === "esriDynamic" ? I.MAPIMAGE : I.TILE,
		...
		e.attribution && (n.attribution = { text: {} }, e.attribution.text && (n.attribution.text.disabled = !e.attribution.text.enabled, n.attribution.text.value = e.attribution.text.value)), e.layers.forEach((t, r) => {
			let i = {
				id: t.id || `${e.id}-${r}`,
				layerType: t.layerType === "esriDynamic" ? I.MAPIMAGE : I.TILE,
				url: t.url,
				opacity: t.opacity ?? 1
			};
			n.layers.push(i);
	...
	})) : t.fixtures.legend = { root: Fs(e.legend.root) });
}
function Fs(e) {
	let t = {
		name: e.name,
	...
	return e.controls && e.controls.length > 0 && (t.controls = zs(e.controls, n), e.controls.includes("visibility") && t.controls.push("visibilityButton"), (e.controls.length !== 1 || e.controls[0] !== "visibility") && console.warn("Legend entry groups currently support only the visibility control. All other controls are currently not supported."), t.controls.push("expandButton")), e.disabledControls && e.disabledControls.length > 0 && (t.disabledControls = zs(e.disabledControls, n), e.disabledControls.includes("visibility") && t.disabledControls.push("visibilityButton"), (e.disabledControls.length !== 1 || e.disabledControls[0] !== "visibility") && console.warn("Legend entry groups currently support only the visibility control. All other controls are currently not supported.")), e.children.forEach((e) => {
		if (e.layerId) t.children.push(Is(e));
			...
			e.collapse !== void 0 && console.warn(`collapse property in visibilitySet in children list of legend entry group ${t.name} cannot be mapped and will be skipped.`), e.exclusiveVisibility.forEach((e) => {
				e.layerId ? n.children.push(Is(e)) : n.children.push(Fs(e));
			}), t.children.push(n);
		} else t.children.push(Fs(e));
	}), t;
}
function Is(e) {
	let t = e, n = [
	...
	return e.controls && e.controls.length > 0 && (t.layerControls = zs(e.controls, n), t.layerControls.push("symbology")), e.disabledControls && e.disabledControls.length > 0 && (t.disabledLayerControls = zs(e.disabledControls, n)), e.controlledIds && (console.warn(`controlledIds property defined in legend entry ${e.layerId} cannot be mapped and will be skipped.`), delete t.controlledIds), e.entryIndex && (t.sublayerIndex = e.entryIndex, delete e.entryIndex), e.entryId && console.warn(`entryId property defined in legend entry ${e.layerId} cannot be mapped and will be skipped.`), t;
}
function Ls(e) {
	let t = Rs(e);
...
console.info(`RAMP v${{
	...
	minor: "21",
	patch: "0-beta",
	timestamp: "Thu May 7 14:25:41 2026 -0400",
	hash: "3d5aaaaa415403618b21505270d88e6dfe3625ab"
}.major}.${{
	major: "4",
	minor: "21",
	patch: "0-beta",
...
var qs = (e, t, n) => new so(e, t, n), Js = new Jt();
//#endregion
export { J as $, za as A, bn as B, Za as C, V as Ct, Ya as D, gt as Dt, qa as E, I as Et, dr as F, en as G, an as H, ur as I, Gt as J, $t as K, lr as L, _a as M, ga as N, Ja as O, ha as P, jt as Q, kn as R, $a as S, St, Xa as T, H as Tt, rn as U, Z as V, tn as W, It as X, Y, Nt as Z, io as _, me as _t, Ws as a, re as at, to as b, W as bt, Yo as c, oe as ct, Do as d, ce as dt, q as et, To as f, d as ft, ao as g, pe as gt, $ as h, fe as ht, Ks as i, ne as it, Da as j, Ka as k, No as l, c as lt, oo as m, p as mt, qs as n, ee as nt, As as o, ie as ot, po as p, f as pt, X as q, Js as r, te as rt, Cs as s, ae as st, Gs as t, h as tt, Mo as u, se as ut, ro as v, he as vt, Qa as w, G as wt, eo as x, ht as xt, no as y, Ct as yt, wn as z };
