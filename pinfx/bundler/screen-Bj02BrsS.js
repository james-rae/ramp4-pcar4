import { Tt as e, V as t } from "./main-DRrASmfn.js";
...
var O = [
	...
	setup(n) {
		let a = f("iApi"), { t: d } = E(), y = _([]), O = _({}), H = _({}), U = _([]), W = _([]), G = _([]), K = i(() => a.animate), q = () => {
			...
			let n = a.geo.layer.layerOrderIds(), r = [...S(a.geo.layer.allLayersOnMap(!0))].filter((t) => !t.isCosmetic && t.layerState !== e.ERROR);
			y.value = r.reverse().map((e, r) => {
				...
				return {
					...
					isExpanded: t[e.id] || !1,
					isLoaded: !1,
					supportsSublayers: e.supportsSublayers,
					sublayers: []
				};
			}), r.forEach((e) => {
				e.loadPromise().then(() => {
		...
		return h(async () => {
			q(), W.value.push(a.event.on(t.LAYER_REMOVE, () => {
				q();
			})), W.value.push(a.event.on(t.LAYER_LAYERSTATECHANGE, () => {
				q();
			})), W.value.push(a.event.on(t.MAP_REORDER, () => {
				q();
			}));
		}), m(() => {
