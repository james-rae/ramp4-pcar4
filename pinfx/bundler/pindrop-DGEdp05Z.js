import { Dt as e, Et as t, V as n, X as r, s as i, yt as a } from "./main-DRrASmfn.js";
//#region src/fixtures/pindrop/api/pin-defs.ts
var o = "Ramp-Pin-Drop", s = {
	style: e.ICON,
	width: 12,
	height: 16,
	...
	xOffset: 0,
	yOffset: 8
}, c = "ramp-pin-identify", l = "ramp-pin-panel-open", u = "ramp-pin-panel-close", d = "ramp-pin-panel-min", f = class extends i {
	_pinSymbol;
	_linkIdentify = !1;
	_linkDetails = !1;
	_pinPoint;
	...
	set linkIdentify(e) {
		...
		}, c) : this._cleanIdentify());
	}
	get linkDetails() {
		return this._linkDetails;
	}
	set linkDetails(e) {
		e !== this._linkDetails && (this._linkDetails = e, e ? (this.$iApi.event.on(n.PANEL_CLOSED, async (e) => {
			e.id === "details" && this.removePin();
		}, u), this.$iApi.event.on(n.PANEL_MINIMIZED, (e) => {
			e.id === "details" && this.hidePin();
		}, d), this.$iApi.event.on(n.PANEL_OPENED, (e) => {
			e.id === "details" && !this._pinVisible && this._pinPoint && this.restorePin();
		}, l)) : this._cleanDetails());
	}
	initialized() {
		console.log("PIN INIT"), this.initPinLayer();
	}
	cleanup() {
	...
	_parseConfig(e) {
		...
		let t = !0;
		e && (e.pinSymbol && (this.pinSymbol = e.pinSymbol, t = !1), this.linkDetails = !!e.linkDetails, this.linkIdentify = !!e.linkIdentify), t && (this.pinSymbol = s);
	}
	async initPinLayer() {
		let e = this.$iApi.geo, n = e.layer.createLayer({
			id: o,
			layerType: t.GRAPHIC,
			cosmetic: !0,
			system: !0,
	...
	getPindropLayer() {
		return this.$iApi.geo.layer.getLayer(o);
	}
	...
	_insertPin() {
		...
		if (e && this._pinPoint) {
			...
			r[i] !== "Ramp-Pin-Drop" && (console.log("im reordering"), n.map.reorder(e, i, !1));
		}
	}
	_erasePin(e) {
		if (this._pinVisible) {
	...
	_cleanIdentify() {
		this.$iApi.event.off(c);
	}
	_cleanDetails() {
		let e = this.$iApi.event;
		e.off(u), e.off(l), e.off(d);
	}
	isDetailVisible() {
		let e = this.$iApi.panel.get("details");
		return !!(e && (e.teleport || e.isVisible));
	}
}, p = class extends f {
	async added() {
