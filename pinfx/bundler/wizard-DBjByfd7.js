import { Et as e, J as t, m as n, q as r, s as i } from "./main-DRrASmfn.js";
...
var s = class extends i {
	...
	async fetchFileInfo(t, n, r) {
		switch (r ||= await this.$iApi.geo.layer.files.fetchFileData(t, n), n) {
			case e.GEOJSON: return this.getGeojsonInfo(t, r);
			case e.SHAPEFILE: return this.getShapfileInfo(t, r);
			case e.CSV: return this.getCsvInfo(t, r);
			default: console.error(`Unsupported file type passed to fetchFileInfo - '${n}'`);
		}
	}
	async getGeojsonInfo(t, n) {
		return n instanceof ArrayBuffer && (n = JSON.parse(new TextDecoder("utf-8").decode(new Uint8Array(n)))), {
			config: {
			...
			fields: [{
				name: "OBJECTID",
				type: "oid"
			}].concat(this.$iApi.geo.layer.files.extractGeoJsonFields(n)),
			configOptions: [
				"name",
				"nameField",
				"maptipField",
	...
	async getCsvInfo(t, n) {
		return n instanceof ArrayBuffer && (n = new TextDecoder("utf-8").decode(new Uint8Array(n))), {
			config: {
			...
			fields: [{
				name: "OBJECTID",
				type: "oid"
			}].concat(this.$iApi.geo.layer.files.extractCsvFields(n)),
			latLonFields: this.$iApi.geo.layer.files.filterCsvLatLonFields(n),
			configOptions: [
				"name",
				"nameField",
				"maptipField",
	...
	async fetchServiceInfo(t, n, r, i) {
		switch (n) {
			case e.FEATURE: return this.getFeatureInfo(t);
			case e.MAPIMAGE: return this.getMapImageInfo(t, r);
			case e.TILE: return this.getTileInfo(t);
			case e.IMAGERY: return this.getImageryInfo(t);
			case e.WFS: return this.getWfsInfo(t, i);
			case e.WMS: return this.getWmsInfo(t, r);
		}
	}
	async getFeatureInfo(t) {
		let n = await o.get(t, { params: { f: "json" } });
		return {
			config: {
				id: `${e.FEATURE}#${++this.layerCount}`,
				url: t,
				layerType: e.FEATURE,
				name: n.data.name,
				nameField: n.data.displayField,
				maptipField: n.data.displayField,
				state: {
					opacity: 1,
					visibility: !0
				}
	...
	async getMapImageInfo(t, n) {
		let r = await o.get(t, { params: { f: "json" } });
		return {
			config: {
				id: `${e.MAPIMAGE}#${++this.layerCount}`,
				url: t,
				layerType: e.MAPIMAGE,
				name: r.data.mapName,
				sublayers: [],
				state: {
					opacity: 1,
			...
			layers: this.createLayerHierarchy(r.data.layers, n),
	...
	async getImageryInfo(t) {
		let n = await o.get(t, { params: { f: "json" } });
		return {
			config: {
				id: `${e.IMAGERY}#${++this.layerCount}`,
				url: t,
				layerType: e.IMAGERY,
				name: n.data.name,
				state: {
	...
	async getWfsInfo(e, n) {
		let { offset: r, limit: i } = new t(e).queryMap, a = await this.$iApi.geo.layer.ogc.loadWfsData(e, -1, parseInt(r) || 0, parseInt(i) || 1e3, void 0, !1, n);
		return this.getGeojsonInfo(e.match(/\/([^/]+)\/items/)?.[1] || "Layer", a);
	}
	async getWmsInfo(t, n) {
		let r = await this.$iApi.geo.layer.ogc.parseCapabilities(t);
		return {
			config: {
				id: `${e.WMS}#${++this.layerCount}`,
				url: t,
				layerType: e.WMS,
				name: t,
			...
			layers: this.mapWmsLayerList(r.layers, n),
			configOptions: ["name", "sublayers"],
			layersRaw: r.layers
		};
	}
	...
	guessFormatFromURL(t) {
		switch (t.match(/\.(zip|csv|geojson|json)$/)?.[1]) {
			case "zip": return e.SHAPEFILE;
			case "csv": return e.CSV;
			case "geojson":
			case "json": return e.GEOJSON;
		}
		return t.match(/\/ImageServer\/?$/gi) ? e.IMAGERY : t.match(/\/collections\//gi) ? e.WFS : t.match(/arcgis\/rest\/services\//gi) ? t.match(/\/\d+\/?$/g) ? e.FEATURE : e.MAPIMAGE : t.match(/service=|version=|\/wms/gi) ? e.WMS : "";
	}
}, l = {
	en: {
	...
	added() {
		this.$iApi.panel.register({ wizard: {
			screens: { "wizard-screen": () => a(import("./screen-CkQNZskS.js")) },
			button: {
		...
		let e = new c(this.$iApi), t = n(this.$vApp.$pinia);
		t.layerSource = e, this.removed = () => {
			this.$iApi.panel.remove("wizard"), e = void 0, t.$reset();
		};
	}
};
//#endregion
