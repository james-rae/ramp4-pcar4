import { B as e, C as t, Ct as n, D as r, E as i, O as a, S as o, T as s, V as c, w as l, wt as u } from "./main-DRrASmfn.js";
import { n as d } from "./config-RKp-tUX3.js";
import { t as f } from "./_plugin-vue_export-helper-O4QQSku4.js";
...
var B = [
	...
	setup(e) {
		...
		let a = () => {
			n.value instanceof r ? n.legendItem.toggleVisibility() : n.legendItem instanceof s && (n.legendItem.clickSymbology(n.value.uid), i.value = !0);
		};
		return (n, r) => {
			let o = k("tippy");
			return T(), _("div", {
	...
	emits: ["focusItem"],
	setup(t, { emit: n }) {
		let { t: r } = z(), i = ee("iApi"), a = E(), o = E(!1), l = E(e().mobileView), d = n, f = E(0), p = t, g = () => {
			p.legendItem.layerControlAvailable(u.Symbology) && p.legendItem.toggleSymbology();
		}, y = () => {
			p.legendItem.layerControlAvailable(u.Datatable) && L("grid") && i.event.emit(c.GRID_TOGGLE, p.legendItem.layer);
		}, x = () => {
			p.legendItem.layerControlAvailable(u.Settings) && L("settings") && i.event.emit(c.SETTINGS_TOGGLE, p.legendItem.layer);
		}, C = () => {
			if (p.legendItem.layerControlAvailable(u.Metadata) && L("metadata")) {
				...
				if (e.url) {
					...
					i.event.emit(c.METADATA_TOGGLE, {
						type: o,
						layerName: t,
						url: e.url,
						catalogueUrl: n,
			...
			p.legendItem.layerControlAvailable(u.BoundaryZoom) && p.legendItem?.layer?.zoomToLayerBoundary();
		}, D = () => {
			p.legendItem.layerControlAvailable(u.Remove) && i.geo.map.removeLayer(p.legendItem.layerUid);
		}, A = (e) => {
			P.value && M(p.legendItem.layer).reload().then(() => {
				f.value += 1, e.pointerType !== "mouse" && d("focusItem");
			});
		}, P = m(() => p.legendItem.layerControlAvailable(u.Reload) && p.legendItem instanceof s && M(p.legendItem.layer)?.canReload), te = (e) => {
		...
		return (e, n) => {
			let i = O("dropdown-menu"), s = k("tippy");
			return T(), _("div", {
				onClick: n[3] ||= R(() => {}, ["stop"]),
				...
				default: F(() => [
					v("a", {
						href: "javascript:;",
						class: S(["flex leading-snug items-center text-left w-auto", { disabled: !t.legendItem.layerControlAvailable(N(u).Metadata) || !L("metadata") }]),
						onClick: C,
						role: "button",
					...
					v("a", {
						href: "javascript:;",
						class: S(["flex leading-snug items-center text-left w-auto", { disabled: !t.legendItem.layerControlAvailable(N(u).Settings) || !L("settings") }]),
						onClick: x,
						role: "button",
					...
					v("a", {
						href: "javascript:;",
						class: S(["flex leading-snug items-center text-left w-auto", { disabled: !t.legendItem.layerControlAvailable(N(u).Datatable) || !L("grid") }]),
						onClick: y,
						role: "button",
					...
					v("a", {
						href: "javascript:;",
						class: S(["flex leading-snug items-center text-left w-auto", { disabled: !t.legendItem.layerControlAvailable(N(u).Symbology) }]),
						onClick: g,
						role: "button",
					...
					v("a", {
						href: "javascript:;",
						class: S(["flex leading-snug items-center text-left w-auto", { disabled: !t.legendItem.layerControlAvailable(N(u).BoundaryZoom) }]),
						onClick: w,
						role: "button",
					...
					v("a", {
						href: "javascript:;",
						class: S(["flex leading-snug items-center text-left w-auto", { disabled: !t.legendItem.layerControlAvailable(N(u).Remove) }]),
						onClick: D,
						role: "button",
					...
					I((T(), _("a", {
						...
						class: "setting-svg",
						viewBox: "0 0 24 24"
					}, [v("path", { d: "M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z" })], -1), b(" " + j(N(r)("legend.layer.controls.reload")), 1)], 42, K)), [[s, {
						placement: "top-start",
	...
	setup(r) {
		let f = d(), x = e(), { t: C } = z(), w = ee("iApi"), F = E(), L = r, B = o(), H = B.multilineItems, ie = (L.legendItem instanceof s && L.legendItem.maxLines) ?? B.maxLines, U = E(x.mobileView), ae = m(() => f.layerConfigs), W = E([]), G = E(!1), K = E(!1), q = P("legendFocusItem"), J = m(() => L.legendItem instanceof s ? M(L.legendItem.layer)?.layerType : ""), Y = m(() => L.legendItem instanceof s && M(L.legendItem.layer)?.canModifyLayer), se = m(() => {
			if (L.legendItem instanceof s) {
				let e = M(L.legendItem.layer);
				return e ? e.canReload : !0;
			} else return !1;
		}), ce = m(() => w.animate), X = m(() => L.legendItem.children.length > 0 || L.legendItem instanceof s && M(L.legendItem.layer)?.sublayers.length > 0), Z = (e) => e === i.Expand || e === i.Visibility ? L.legendItem.controlAvailable(e) : L.legendItem.layerControlAvailable(e), le = (e) => re(e, { async: !1 }), rt = () => {
			L.legendItem.children.length === 0 || !Z(i.Expand) || (L.legendItem.toggleExpanded(), w.updateAlert(C(`legend.alert.group${L.legendItem.expanded ? "Expanded" : "Collapsed"}`)));
		}, it = () => {
			if (Z(u.Symbology)) {
			...
			Z(u.Datatable) && Q() && w.event.emit(c.GRID_TOGGLE, L.legendItem.layer);
		}, ot = (e) => {
			...
			if (e.type === a.Error) {
				...
				let r = setInterval(() => {
					let i = e.layer;
					(i && (i.layerExists || i.initiationState === n.NEW || i.initiationState === n.TERMINATING || i.initiationState === n.TERMINATED) || t > 1200) && (clearInterval(r), i && w.geo.map.removeLayer(i), f.removeLayerConfig(e.layerId), w.fixture.get("legend")?.removeLayerItem(e.layerId)), t++;
				}, 250);
			} else {
				L.legendItem.error();
				let n = setInterval(() => {
		...
		return L.legendItem instanceof s && ($(), te(() => L.legendItem.symbologyStack, () => {
			$();
		})), (e, n) => {
			let o = O("item", !0), c = k("tippy"), d = k("truncate"), f = k("focus-item");
			return r.legendItem.hidden ? g("", !0) : (T(), _("div", {
				...
				v("div", de, [I((T(), _("div", {
					class: S(["flex items-center hover:bg-gray-200 default-focus-style !p-5", [
						r.legendItem.type === N(a).Item ? "loaded-item" : r.legendItem.type === N(a).Error ? "non-loaded-item bg-red-200" : "non-loaded-item",
						X.value && Z(N(i).Expand) || !X.value && r.legendItem instanceof N(s) && Z(N(u).Datatable) && Q() && r.legendItem.type === N(a).Item ? "cursor-pointer" : "cursor-default",
						N(H) ? "multilined" : "singlelined"
					]]),
					onMouseover: n[10] ||= R((e) => ut(e.currentTarget), ["stop"]),
					onMouseout: n[11] ||= (e) => (U.value || e.currentTarget?._tippy?.hide(), K.value = !1),
					onClick: n[12] ||= () => {
						!X.value && r.legendItem instanceof N(s) && Z(N(u).Datatable) && Q() && r.legendItem.type === N(a).Item ? at() : X.value && rt();
					},
					content: X.value && Z(N(i).Expand) ? N(C)(r.legendItem.expanded ? "legend.group.collapse" : "legend.group.expand") : r.legendItem instanceof N(s) && r.legendItem.type === N(a).Item && Z(N(u).Datatable) && Q() ? N(C)("legend.layer.data") : "",
					"truncate-trigger": "",
					ref_key: "legendFocusItem",
					ref: q
				}, [
					r.legendItem.type === N(a).Item ? g("", !0) : (T(), _("div", pe, [r.legendItem.type === N(a).Placeholder ? (T(), _("svg", me, [...n[14] ||= [y("<path d=\"M0 0h24v24H0V0z\" fill=\"none\" data-v-4152c061></path><path d=\"M0 0h24v24H0V0z\" fill=\"none\" data-v-4152c061></path><circle cx=\"15.5\" cy=\"9.5\" r=\"1.5\" data-v-4152c061></circle><circle cx=\"8.5\" cy=\"9.5\" r=\"1.5\" data-v-4152c061></circle><circle cx=\"15.5\" cy=\"9.5\" r=\"1.5\" data-v-4152c061></circle><circle cx=\"8.5\" cy=\"9.5\" r=\"1.5\" data-v-4152c061></circle><path d=\"M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm0-2.5c2.33 0 4.32-1.45 5.12-3.5h-1.67c-.69 1.19-1.97 2-3.45 2s-2.75-.81-3.45-2H6.88c.8 2.05 2.79 3.5 5.12 3.5z\" data-v-4152c061></path>", 7)]])) : (T(), _("svg", he, [...n[15] ||= [y("<path d=\"M0 0h24v24H0V0z\" fill=\"none\" data-v-4152c061></path><path d=\"M0 0h24v24H0V0z\" fill=\"none\" data-v-4152c061></path><circle cx=\"15.5\" cy=\"9.5\" r=\"1.5\" data-v-4152c061></circle><circle cx=\"8.5\" cy=\"9.5\" r=\"1.5\" data-v-4152c061></circle><circle cx=\"15.5\" cy=\"9.5\" r=\"1.5\" data-v-4152c061></circle><circle cx=\"8.5\" cy=\"9.5\" r=\"1.5\" data-v-4152c061></circle><path d=\"M 20,12C 20,7.6 16.4,4 12,4C 7.6,4 4,7.6 4,12C 4,16.4 7.6,20 12,20C 16.4,20 20,16.4 20,12 Z M 22,12C 22,17.5 17.5,22 12,22C 6.5,22 2,17.5 2,12C 2,6.5 6.5,2.00001 12,2.00001C 17.5,2.00001 22,6.5 22,12 Z M 15.5,8C 16.3,8 17,8.7 17,9.5C 17,10.3 16.3,11 15.5,11C 14.7,11 14,10.3 14,9.5C 14,8.7 14.7,8 15.5,8 Z M 10,9.5C 10,10.3 9.3,11 8.5,11C 7.7,11 7,10.3 7,9.5C 7,8.7 7.7,8 8.5,8C 9.3,8 10,8.7 10,9.5 Z M 12,14C 13.7524,14 15.2943,14.7212 16.1871,15.8129L 14.7697,17.2302C 14.3175,16.5078 13.2477,16 12,16C 10.7523,16 9.68254,16.5078 9.23024,17.2302L 7.81291,15.8129C 8.7057,14.7212 10.2476,14 12,14 Z\" data-v-4152c061></path>", 7)]]))])),
					r.legendItem.type === N(a).Item && r.legendItem instanceof N(s) && r.legendItem.layer.legend.length > 0 ? (T(), _("div", {
						key: 1,
						class: "relative w-32 h-32 mr-15",
						onMouseover: n[0] ||= R(() => {}, ["stop"])
					}, [I((T(), _("button", {
						type: "button",
						onClick: R(it, ["stop"]),
						class: S([Z(N(u).Symbology) ? "cursor-pointer" : "cursor-default"]),
						disabled: !Z(N(u).Symbology),
						content: r.legendItem instanceof N(s) && r.legendItem.symbologyExpanded ? N(C)("legend.symbology.hide") : N(C)("legend.symbology.expand"),
						"aria-label": r.legendItem instanceof N(s) && r.legendItem.symbologyExpanded ? N(C)("legend.symbology.hide") : N(C)("legend.symbology.expand")
					}, [r.legendItem.coverIcon ? (T(), _("img", {
						key: 1,
						class: S([{ "pointer-events-none": !Z(N(u).Symbology) }, "w-32 h-32 hover:scale-105"]),
						src: r.legendItem.coverIcon,
						alt: "Cover icon not found :("
					}, null, 10, _e)) : (T(), h(ue, {
						key: 0,
						class: S([{ "pointer-events-none": !Z(N(u).Symbology) }, "w-32 h-32"]),
						visible: r.legendItem instanceof N(s) && r.legendItem.symbologyExpanded,
					...
					r.legendItem instanceof N(s) && N(H) ? I((T(), _("div", {
						key: 3,
						...
						externalTrigger: !0,
						noTruncateClass: !0
					}]]) : r.legendItem instanceof N(s) ? I((T(), _("div", ye, [v("span", null, j(r.legendItem.name ?? (!r.legendItem.layer || r.legendItem?.layer?.name === "" ? r.legendItem.layerId : r.legendItem.layer?.name)), 1)])), [[d, { externalTrigger: !0 }]]) : r.legendItem instanceof N(l) ? (T(), _("div", be, [r.legendItem.infoType === N(t).Title && r.legendItem.content ? (T(), _("h3", xe, j(r.legendItem.content), 1)) : r.legendItem.infoType === N(t).Title ? (T(), _("h3", Se, j(r.legendItem.name), 1)) : r.legendItem.infoType === N(t).Text ? (T(), _("p", Ce, j(r.legendItem.content), 1)) : r.legendItem.infoType === N(t).Image ? (T(), _("img", {
						key: 3,
						src: r.legendItem.content,
						alt: "InfoType image not found :("
					}, null, 8, we)) : r.legendItem.infoType === N(t).Markdown ? (T(), _("div", {
						key: 4,
						class: "ramp-markdown",
						innerHTML: le(r.legendItem.content)
					}, null, 8, Te)) : (T(), h(A(`${r.legendItem.uid}-info-section`), { key: 5 }))])) : g("", !0),
					r.legendItem.type === N(a).Error && se.value ? (T(), _("div", Ee, [I((T(), _("button", {
						type: "button",
						class: "text-gray-500 hover:text-black",
						content: N(C)("legend.layer.controls.reload"),
						onMouseover: n[1] ||= R(() => {}, ["stop"]),
						onClick: R(st, ["stop"]),
						"aria-label": N(C)("legend.layer.controls.reload")
					}, [...n[17] ||= [v("div", { class: "flex p-8" }, [v("svg", {
					...
					(r.legendItem.type === N(a).Item || r.legendItem.type === N(a).Placeholder && N(H)) && r.legendItem instanceof N(s) ? (T(), h(oe, {
						key: 7,
						class: S({ invisible: r.legendItem.type === N(a).Placeholder }),
						legendItem: r.legendItem,
						onFocusItem: n[2] ||= () => e.$emit("focusItem", q.value)
					}, null, 8, ["class", "legendItem"])) : g("", !0),
					r.legendItem.type !== N(a).Item && r.legendItem instanceof N(s) ? (T(), _("div", Oe, [I((T(), _("button", {
						type: "button",
						class: "text-gray-500 hover:text-black",
						content: r.legendItem.type === N(a).Error ? N(C)("legend.layer.controls.remove") : N(C)("legend.layer.controls.cancel"),
						onMouseover: n[3] ||= R(() => {}, ["stop"]),
						onClick: R(lt, ["stop"]),
						"aria-label": r.legendItem.type === N(a).Error ? N(C)("legend.layer.controls.remove") : N(C)("legend.layer.controls.cancel")
					}, [v("div", Ae, [r.legendItem.type === N(a).Placeholder ? (T(), _("svg", je, [...n[18] ||= [v("path", { d: "M242.72 256l100.07-100.07c12.28-12.28 12.28-32.19 0-44.48l-22.24-22.24c-12.28-12.28-32.19-12.28-44.48 0L176 189.28 75.93 89.21c-12.28-12.28-32.19-12.28-44.48 0L9.21 111.45c-12.28 12.28-12.28 32.19 0 44.48L109.28 256 9.21 356.07c-12.28 12.28-12.28 32.19 0 44.48l22.24 22.24c12.28 12.28 32.2 12.28 44.48 0L176 322.72l100.07 100.07c12.28 12.28 32.2 12.28 44.48 0l22.24-22.24c12.28-12.28 12.28-32.19 0-44.48L242.72 256z" }, null, -1)]])) : (T(), _("svg", Me, [...n[19] ||= [v("path", { d: "M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z" }, null, -1)]]))])], 40, ke)), [[c, { placement: "top-start" }]])])) : g("", !0),
					r.legendItem.type === N(a).Item && r.legendItem instanceof N(s) && r.legendItem.layerOffscale ? I((T(), _("div", {
						key: 9,
					...
					r.legendItem.type === N(a).Item && r.legendItem instanceof N(s) && !r.legendItem.layer?.mapLayer ? I((T(), _("div", {
						key: 10,
						class: "relative p-4 cursor-default",
						content: N(C)("legend.layer.data.only"),
						onMouseover: n[6] ||= R(() => {}, ["stop"]),
						onClick: n[7] ||= R(() => {}, ["stop"]),
						"focus-icon": ""
					}, [...n[21] ||= [v("svg", {
						xmlns: "http://www.w3.org/2000/svg",
					...
					r.legendItem.type === N(a).Item && r.legendItem instanceof N(s) && r.legendItem.layerOffscale ? (T(), _("div", Fe, [I((T(), _("button", {
						type: "button",
						class: "p-4",
						content: N(C)("legend.layer.zoomToVisible"),
						onMouseover: n[8] ||= R(() => {}, ["stop"]),
						onClick: n[9] ||= R((e) => r.legendItem.layer.zoomToVisibleScale(), ["stop"])
					}, [...n[22] ||= [v("svg", {
						class: "m-auto",
						xmlns: "http://www.w3.org/2000/svg",
						height: "18",
						...
						v("path", { d: "M12 10h-2v2H9v-2H7V9h2V7h1v2h2v1z" })
					], -1)]], 40, Ie)), [[c, { placement: "top-start" }]])])) : r.legendItem.type === N(a).Item && Z(N(i).Visibility) ? (T(), h(V, {
						key: 12,
						checked: r.legendItem.visibility,
						value: r.legendItem,
						isRadio: r.legendItem.parent && r.legendItem.parent.exclusive,
						legendItem: r.legendItem,
						disabled: r.legendItem instanceof N(s) && !r.legendItem.layerControlAvailable(N(u).Visibility),
					...
					trigger: "manual focus",
					aria: "describedby"
				}]]), r.legendItem.type === N(a).Placeholder || r.legendItem instanceof N(s) && r.legendItem.layerRedrawing && r.legendItem.visibility ? (T(), _("div", Le, [...n[23] ||= [v("div", { class: "progress-line" }, null, -1)]])) : g("", !0)]),
				r.legendItem instanceof N(s) && r.legendItem.symbologyExpanded ? I((T(), _("div", Re, [W.value.length > 0 ? (T(), _("div", ze, [r.legendItem instanceof N(s) && r.legendItem.description ? (T(), _("p", Be, j(r.legendItem.description), 1)) : g("", !0), (T(!0), _(p, null, D(W.value, (e) => (T(), _("div", {
					class: "m-5",
					key: e.uid
				...
				r.legendItem.expanded ? (T(), _("div", nt, [(T(!0), _(p, null, D(r.legendItem.children, (t) => (T(), h(o, {
					legendItem: t,
					key: t.uid,
					onFocusItem: n[13] ||= (t) => e.$emit("focusItem", t)
