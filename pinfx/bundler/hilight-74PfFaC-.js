import { Et as e, V as t, c as n, l as r, q as i, s as a, u as o } from "./main-DRrASmfn.js";
import { t as s } from "./config-RKp-tUX3.js";
import { i as c, n as l, r as u, t as d } from "./hilight-defs-Bxj5UF-q.js";
...
var f = class extends i {
	...
	lastAdd = 0;
	constructor(e, n) {
		super(e, n), this.onOpacity = e.options?.onOpacity ?? .75, this.offOpacity = e.options?.offOpacity > .02 ? e.options.offOpacity : .02, this.$iApi.geo.map.created ? this.hilightSetup() : this.handlers.push(this.$iApi.event.on(t.MAP_CREATED, () => {
			this.hilightSetup();
		})), this.handlers.push(this.$iApi.event.on(t.MAP_BASEMAPCHANGE, () => {
	...
	async hilightSetup() {
		let t = s(this.$vApp.$pinia).activeBasemapConfig;
		try {
			let n = this.$iApi.geo.layer.createLayer({
				id: l,
				layerType: e.TILE,
				cosmetic: !0,
				system: !0,
			...
			await this.$iApi.geo.map.addLayer(n), n.opacity = this.offOpacity, await this.reorderFogLayer();
		} catch {
	...
	handlers = [];
	constructor(e, n) {
		super(e, n), this.hilightSetup(e), this.handlers.push(this.$iApi.event.on(t.MAP_CREATED, () => {
			this.hilightSetup(e);
		}));
	}
	hilightSetup(e) {
	...
	async initHilightLayer() {
		...
		await this.$iApi.geo.map.addLayer(t);
	}
	async addHilight(e) {
