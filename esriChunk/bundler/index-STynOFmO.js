...
import { G as a, _ as P, F as R } from "./main-DCVw7L11.js";
import "pinia";
...
const D = { en: { "panguard.instructions": "Use two fingers to pan the map" }, fr: { "panguard.instructions": "Utilisez deux doigts pour faire un panoramique de la carte" } }, B = { class: "pg-label" }, F = /* @__PURE__ */ _({
  __name: "map-panguard",
  setup(f) {
    const { t: m } = S(), t = E("iApi"), i = d(), n = d(-1), r = h([]), p = h([]);
    w(() => {
      c(), p.push(
      ...
      t.geo.map.viewPromise.then(() => {
        r.push(
          ...
          t.geo.map.esriView.on("pointer-move", (o) => {
            const { pointerId: v, pointerType: g, x: u, y: l } = o, s = e.get(v);
            if (!s || g !== "touch" || e.size !== 1) {
              i.value.classList.remove("pg-active");
              return;
            }
            Math.sqrt(Math.pow(u - s.x, 2) + Math.pow(l - s.y, 2)) < 20 || (i.value.classList.add("pg-active"), n.value !== -1 && clearTimeout(n.value), n.value = window.setTimeout(() => {
              i.value.classList.remove("pg-active");
