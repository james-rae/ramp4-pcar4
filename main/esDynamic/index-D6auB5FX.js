import { bx as f, iH as p, iB as a, iw as l, iI as y, ha as g, iJ as w } from "./main-CqTmyBdW.js";
import h from "./screen-DKinsNBL.js";
class m extends f {
  /**
...
class z extends p {
  ...
  async fetchFileInfo(e, r, i) {
    switch (i || (i = await this.$iApi.geo.layer.files.fetchFileData(
      e,
      r
    )), r) {
      case a.GEOJSON:
      ...
      default:
        console.error(
          `Unsupported file type passed to fetchFileInfo - '${r}'`
        );
    }
  }
  async getGeojsonInfo(e, r) {
    return r instanceof ArrayBuffer && (r = JSON.parse(
      new TextDecoder("utf-8").decode(new Uint8Array(r))
    )), {
      config: {
  ...
  async getCsvInfo(e, r) {
    return r instanceof ArrayBuffer && (r = new TextDecoder("utf-8").decode(
      new Uint8Array(r)
    )), {
      config: {
      ...
      fields: [{ name: "OBJECTID", type: "oid" }].concat(
        this.$iApi.geo.layer.files.extractCsvFields(r)
      ),
  ...
  async getShapfileInfo(e, r) {
    const i = await this.$iApi.geo.layer.files.shapefileToGeoJson(
      r
    );
    return this.getGeojsonInfo(e, i);
  }
  ...
  createLayerHierarchy(e, r) {
    ...
    const i = (o, s) => {
      ...
      e.filter(
        (o) => o.subLayerIds && o.subLayerIds.length > 0
      ).map((o) => o.id)
    );
    for (const o of e)
  ...
  async getWmsInfo(e, r) {
    const i = await this.$iApi.geo.layer.ogc.parseCapabilities(
      e
    );
    return {
      config: {
...
//# sourceMappingURL=index-D6auB5FX.js.map
