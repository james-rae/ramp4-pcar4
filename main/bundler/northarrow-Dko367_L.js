import { $ as e, V as t, X as n, lt as r, s as i, tt as a, ut as o, y as s } from "./main-DgnOnp_l.js";
import { t as c } from "./config-BhPbUOhN.js";
...
var x = class extends i {
	...
	setup(i) {
		...
		g(() => {
			let e = f.config.map;
			F.value = e.tileSchemas, D?.value && (M.value = `<img width='25' src='${D.value}'>`), w.geo.map.esriView?.ready && L(w.geo.map.getExtent()), P.push(w.event.on(t.MAP_EXTENTCHANGE, b(L, 300)));
		}), h(() => {
		...
		let L = async (t) => {
			...
			let s = w.$vApp.$el.querySelector(".inner-shell"), c = E.value.querySelector(".northarrow").getBoundingClientRect().width, l = w.$vApp.$el.querySelector(".appbar")?.clientWidth || 0, u = t.sr;
			if (i || i === void 0 && !u.isWebMercator()) {
				let t = new e("pole", {
					x: -96,
					y: 90
				}), i = await w.geo.proj.projectGeometry(u, t), d = w.geo.map.mapPointToScreenPoint(i);
				if (d.screenY < 0) {
					j.value = !0;
					let e = {
