import { H as e, R as t, V as n } from "./main-DgnOnp_l.js";
import { n as r } from "./config-BhPbUOhN.js";
import { t as i } from "./_plugin-vue_export-helper-B3ysoDQm.js";
import { t as a } from "./toggle-switch-control-BnkO6LC-.js";
...
var I = {
	...
	setup(e) {
		...
		}, null, 8, U)) : l("", !0)])])) : l("", !0)], 2), d("div", W, [e.result.loaded ? (S(), u("div", G, O(e.result.items.length), 1)) : l("", !0)])])) : (S(), u("div", ee, [...r[0] ||= [d("div", { class: "symbologyIcon h-32 w-32" }, [d("div", { class: "relative animate-spin spinner h-24 w-24" })], -1)]]));
	}
}), [["__scopeId", "data-v-414be674"]]), q = ["content"], J = { class: "symbologyLayerName truncate" }, Y = /* @__PURE__ */ m({
	...
	setup(t) {
		let n = e(), r = s(() => n.properties), i = t, a = () => {
			let e = i.layer;
		...
		return (e, n) => {
			let r = E("tippy");
			return M((S(), u("button", {
				class: _(["flex flex-grow justify-start items-center px-7 py-10 default-focus-style symbologyStackButton truncate", t.selected ? "detailsButtonSelected" : "px-11"]),
				onClick: n[0] ||= N(() => {}, ["stop"]),
				content: a()
			}, [p(K, {
				...
				layer: t.layer,
				result: t.result
			}, null, 8, ["layer", "result"]), d("div", J, O(a()), 1)], 10, q)), [[r, {
				placement: "right",
				sticky: !0
			}]]);
		};
	}
}), X = ["content"], te = /* @__PURE__ */ m({
	...
	setup(e, { emit: t }) {
		let { t: n } = P(), i = r(), a = C(), s = () => {
			...
			e.target?.matches(":focus-visible") && (g.value || (h.value = !0), g.value = !0);
		}, F = () => {
			h.value = g.value = !1;
		};
		return y(() => {
	...
	setup(i) {
		let a = r(), f = i, p = h("iApi"), m = C([]), _ = e(), { t: v } = P(), x = C(!1), w = C(""), T = C("none"), j = C(), N = () => a.getLayerByUid(f.uid), I = s(() => _.properties), L = s(() => _.defaultTemplates), R = s(() => N()?.supportsFeatures ?? !1), z = s(() => N()?.mapLayer ?? !1), B = s(() => {
			let e = N(), t = e && f.data.loaded ? e.nameValue(f.data.data) : p.$i18n.t("details.items.title");
			return p.ui.isPlainText(t) && (t = p.ui.escapeHtml(t)), t;
		}), V = (e) => {
			if (typeof e == "string") {
			...
			let e = Date.now();
			_.lastestDetailView = e;
			let t = () => {
				!f.inList && e === _.lastestDetailView && p.event.emit(n.DETAILS_VIEW, {
					identifyItem: f.data,
					uid: f.uid
				});
			};
			K("none"), f.data.loaded ? (t(), U()) : f.data.load().then(() => {
				t(), U();
			});
		}, U = () => {
			if (w.value = "", !(f.data && f.data.loaded)) return;
			let e = N();
			if (e === void 0) {
				console.warn(`could not find layer for uid ${f.uid} during icon lookup`);
				return;
			}
			if (e.supportsFeatures) {
				let t = e.oidField;
				e.getIcon(f.data.data[t]).then((e) => {
					w.value = e;
				});
			}
		}, W = s(() => {
			let e = N(), n = e && I.value[e.id] && I.value[e.id].template;
			if (n) {
				if (typeof D(n) != "string") return n;
				g(() => p.notify.show(t.WARNING, p.$i18n.t("details.template.notFound", { layer: n })));
			}
			return L.value && L.value[f.data.format] ? L.value[f.data.format] : R.value ? ie : ae;
		}), G = s(() => R.value && N()?.fields || []), ee = s(() => {
			let e = N();
			if (e && I.value[e.id] && I.value[e.id].fields) return I.value[e.id].fields;
		}), K = (e) => {
			e === "zoomed" || e === "error" ? setTimeout(() => {
				T.value = e, j.value?._tippy.show(), setTimeout(() => {
					j.value?._tippy.hide(), T.value = "none";
				}, 3e3);
			}, 300) : T.value = e;
		}, q = () => {
			...
			if (e === void 0 || !e.isLoaded) {
				console.warn(`Could not find layer for uid ${f.uid} during zoom geometry lookup`), K("error");
				return;
			}
			if (!f.data.loaded) {
			...
			let t = f.data.data[e.oidField];
			e.zoomToFeature(t).then((e) => {
				e ? (K("zoomed"), p.updateAlert(p.$i18n.t("details.item.alert.zoom"))) : K("error");
			});
		};
		return y(() => {
			m.value.push(A(f, () => {
			...
			return S(), u(o, null, [d("div", oe, [R.value ? (S(), u("div", se, [
				d("div", ce, [i.data.loaded && w.value ? (S(), u("span", {
					key: 0,
					class: "flex-none symbologyIcon",
					innerHTML: w.value
				}, null, 8, $)) : (S(), u("div", le, [...t[3] ||= [d("div", { class: "animate-spin spinner h-20 w-20" }, null, -1)]]))]),
				i.data.loaded ? M((S(), u("span", {
					key: 0,
					class: "pl-3 text-left flex-grow itemName",
					content: B.value,
					innerHTML: V(B.value),
					onTouchstart: t[0] ||= (e) => x.value = !0,
					onTouchend: t[1] ||= (e) => x.value = !1,
					tabindex: i.inList ? -1 : 0
				}, null, 40, ue)), [[n, { options: {
					placement: "top-start",
					offset: () => x.value ? [0, 25] : [0, 0]
				} }]]) : (S(), u("div", de, O(k(v)("details.loading")), 1)),
				i.data.loaded ? (S(), u("span", fe, [z.value ? M((S(), u("button", {
					key: 0,
					type: "button",
					content: k(v)(`details.item.zoom${T.value === "none" ? "" : `.${T.value}`}`),
					"aria-label": k(v)(`grid.cells.zoom${T.value === "none" ? "" : `.${T.value}`}`),
					ref_key: "zoomButton",
					ref: j,
					onClick: t[2] ||= (e) => {
						e.stopPropagation(), q();
				...
				}, [T.value === "zooming" ? (S(), u("div", me)) : T.value === "zoomed" ? (S(), u("svg", he, [...t[4] ||= [d("path", {
					"stroke-linecap": "round",
					"stroke-linejoin": "round",
					d: "M4.5 12.75l6 6 9-13.5"
				}, null, -1)]])) : T.value === "error" ? (S(), u("svg", ge, [...t[5] ||= [d("path", {
					"stroke-linecap": "round",
					"stroke-linejoin": "round",
					d: "M6 18L18 6M6 6l12 12"
				}, null, -1)]])) : (S(), u("span", {
					key: 3,
					innerHTML: k(p).ui.getZoomIcon()
				}, null, 8, _e))], 8, pe)), [[r, { placement: "bottom" }]]) : l("", !0)])) : l("", !0)
			])) : l("", !0)]), i.open ? (S(), c(D(W.value), {
				key: 0,
				identifyData: i.data,
				fields: G.value,
				fixtureFields: ee.value,
				class: "p-8"
			}, null, 8, [
				"identifyData",
				"fields",
	...
	setup(t, { emit: i }) {
		let m = C(), g = () => {
		...
		}, D = h("iApi"), j = e(), N = r(), F = i, I = t, { t: L } = P(), R = C(!1), z = C(D.fixture.get("details")), B = C(!0), V = C(!1), H = C(0), U = C(20), W = C([]), G = C([]), ee = s(() => j.activeGreedy), K = s(() => j.properties), q = s(() => H.value + U.value), J = () => N.getLayerByUid(I.uid), Y = () => I.results.find((e) => e.uid === I.uid), X = s(() => Y()?.loaded ?? !1), te = s(() => Y()?.requestTime), ne = s(() => R.value && (!V.value && Z().length > 1 || V.value && Z().length > U.value)), re = s(() => {
			let e = J();
			return e && K.value[e.id] && K.value[e.id].name ? K.value[e.id].name : e?.name ?? "";
		}), ie = s(() => I.uid), Z = () => {
			let e = Y();
			return e ? e.items : [];
		}, Q = s(() => Z()[H.value]), ae = s(() => {
			...
			if (B.value && ae.value) {
				...
				let t = Z();
				if (X.value && t.length > 0) {
					if (V.value) z.value.hilightDetailsItems(t.slice(H.value, q.value), I.uid);
					else {
						let e = t[H.value];
		...
		return x(() => {
			W.value.push(D.event.on(n.LAYER_REMOVE, (e) => {
				let t = D.panel.get("details");
				I.uid === e.uid && t && t.close();
			})), W.value.push(D.event.on(n.PANEL_CLOSED, (e) => {
				e.id === "details" && ue();
			})), W.value.push(D.event.on(n.PANEL_MINIMIZED, (e) => {
				e.id === "details" && de();
			})), W.value.push(D.event.on(n.MAP_BASEMAPCHANGE, (e) => {
			...
			return X.value && ee.value === 0 ? (S(), u("div", {
				key: 0,
				...
				ne.value ? (S(), u("div", Se, [d("div", Ce, [V.value ? l("", !0) : (S(), u("button", {
					...
					M((S(), u("button", {
						...
						disabled: !V.value && H.value === Z().length - 1 || V.value && q.value >= Z().length
					}, [...n[4] ||= [d("svg", {
						height: "24",
						width: "24",
						viewBox: "0 0 23 23"
				...
				R.value ? (S(), u("div", Oe, [Z().length > 0 ? (S(), u("div", ke, [V.value ? M((S(), u("div", {
					...
					ref_key: "el",
					ref: m
				}, [(S(!0), u(o, null, w(Z().slice(H.value, q.value), (e, n) => M((S(), u("button", {
	...
	setup(t) {
		let { t: n } = P(), r = h("iApi"), i = e(), a = C([]), o = C([]), p = C([]), m = C(!1), v = C(null), x = C(""), w = s(() => i.activeGreedy), E = s(() => i.payload), D = (e) => {
			x.value = e;
		...
		return y(() => {
			...
			return S(), c(a, { panel: t.panel }, {
				header: j(() => [f(O(k(i).origin === "toggleEvent" ? k(n)("details.layers.title.gridOrigin") : k(n)("details.layers.title.identifyOrigin")), 1)]),
				...
				}, O(p.value.length >= 1 ? k(n)("details.layers.results.empty") : k(n)("details.layers.results.empty.noLayers")), 3)) : (S(), c(Pe, {
					key: 0,
					uid: x.value,
					results: p.value,
