import { defineComponent as D, inject as oe, resolveComponent as Pe, openBlock as E, createBlock as Xe, unref as h, withCtx as se, createElementVNode as o, createElementBlock as k, Fragment as Je, renderList as et, createTextVNode as H, toDisplayString as T, createCommentVNode as R, ref as C, onBeforeMount as ve, withDirectives as F, normalizeClass as z, withModifiers as V, withKeys as P, vModelText as ne, vModelSelect as kt, onMounted as X, nextTick as _e, onBeforeUnmount as J, resolveDirective as U, computed as O, useTemplateRef as Mt, watch as Ie, vShow as ae, createVNode as Ee, getCurrentInstance as At, markRaw as Ge } from "vue";
import "tiny-emitter";
import { a as fe, _ as ge, G as Q, M as Ft, N as Tt, L as $t, O as Rt, Q as tt, V as re, i as lt, W as Ye, X as Le, Y as St } from "./main-DwE8E7HF.js";
...
const Zt = { class: "h-full flex items-center justify-center" }, Qt = ["placeholder", "aria-label", "disabled"], Xt = ["placeholder", "aria-label", "disabled"], Jt = {
  ...
  props: ["params"],
  setup(_) {
    const n = C("none"), e = _, a = oe("iApi"), l = Tt(), s = C(), { t: i } = N(), u = O(() => {
  ...
  setup(_) {
    const n = [re.OID, re.DOUBLE, re.SINGLE, re.INTEGER], e = oe("iApi"), a = lt(), l = fe(), s = O(() => l.mobileView), i = C(!s.value), u = C(), m = Mt("gridContainer"), { t: b, locale: w } = N(), y = () => At()?.proxy?.$forceUpdate(), L = _, c = C({
      ...
      r.length === 0 && ze(), ke.value = r.reduce((t, { featureCount: v }) => t + v, 0), ie.value = new Array(G.value.length).fill(0), r.forEach((t, v) => ie.value[v] += t.downloadedAttributes()), r.forEach((t, v) => {
        ...
        Promise.all(t).then((v) => {
          ...
          v.forEach((d, f) => {
            ...
            d.columns.forEach((g) => {
              c.value.fieldMap && c.value.fieldMap[g.data] ? (x.push({
                ...
                origAttr: g.data,
                mappedAttr: void 0
              }), p.columns.map(($) => $.data).includes(g.data) || p.columns.push(g);
            ...
            for (let g = 0; g < p.rows.length; g++)
              for (const [$] of Object.entries(p.rows[g]))
                e.ui.isPlainText(p.rows[g][$]) && (p.rows[g][$] = e.ui.escapeHtml(p.rows[g][$]));
          ...
          }), ue.value = Ge(p.rows), le.value = Ge(le.value), $e(), j.value = !1;
        }).catch((v) => {
    ...
    return X(() => {
      ...
      return E(), k("div", {
        ...
        F(o("div", null, [
          o("p", Ol, T(h(b)("grid.splash.error")), 1)
        ], 512), [
          [ae, te.value]
        ]),
        F(o("div", Ul, [
...
}), ka = /* @__PURE__ */ ge(Ea, [["__scopeId", "data-v-5e5c8acb"]]), Ma = /* @__PURE__ */ D({
  __name: "screen",
  props: {
    panel: {
      type: tt,
...
}), gr = /* @__PURE__ */ ge(Ma, [["__scopeId", "data-v-904e67ef"]]);
export {
  gr as default
};
