import { B as e, J as t, L as n, U as r, dt as i, lt as a, mt as o, o as s, ut as c, z as l } from "./main-Ll3BQWQ6.js";
...
var z = ["onClick"], Te = { class: "md-icon-small flex w-full" }, B = { class: "flex-1 truncate whitespace-nowrap overflow-hidden pr-4" }, Ee = /* @__PURE__ */ v({
	...
	props: ["params"],
	setup(t) {
		let n = t, { t: r } = I(), a = y("iApi"), o = w(), s = async () => {
			let t = n.params.data, r = t.rvUid, o = a.geo.layer.getLayer(r), s = be(n.params.layerCols, o), c = await o.getGraphic(t[s], { getAttribs: !0 });
			a.event.emit(e.DETAILS_TOGGLE, {
				data: c.attributes,
				uid: r,
				format: i.ESRI
			}, !0), n.params.isTeleport && a.scrollToInstance();
		};
		return S(() => {
	...
	setup(i) {
		...
		let s = [
			...
			c.SINGLE,
			c.INTEGER
		], u = y("iApi"), d = r(), ne = l(), p = f(() => ne.mobileView), v = w(!p.value), T = w(), k = le("gridContainer"), { t: L, locale: be } = I(), xe = () => ie()?.proxy?.$forceUpdate(), R = i, z = w({
			id: "dummy",
			layerIds: [],
			state: new te(),
			fieldMap: {}
		...
		}, Je = f(() => X.value.some((e) => e.layerState === o.LOADED && e.visibility)), Ye = (t) => {
			B.value = t.api;
			...
			n ||= u.geo.layer.getLayer(R.gridId)?.name || R.gridId, Ne.value = n, Jt(), q.value.length > 0 && Z.value.autoSizeAllColumns(), Ke(), qe(), Z.value.addEventListener("rowDataUpdated", Ke), W.value.push(u.event.on(e.FILTER_CHANGE, ({ uid: e, filterKey: t }) => {
				t !== a.GRID && e && X.value.map((e) => e.uid).includes(e) && Q();
			...
			We.value.push(e), await Promise.all(n), await Promise.all(X.value.map(async (e) => {
				e && e.visibility ? await e.getFilterOIDs([a.GRID], z.value.state.filterByExtent ? u.geo.map.getExtent() : void 0).then((t) => {
					ze.value[e.uid] = t;
				}) : ze.value[e.uid] = [];
			...
			X.value.filter((e) => e.mapLayer).forEach((e) => {
				if (!z.value.state.applyToMap) e.setSqlFilter(a.GRID, "");
				else {
					let t = ln(e.id);
					e.setSqlFilter(a.GRID, t);
				}
			});
		}, ln = (e) => {
