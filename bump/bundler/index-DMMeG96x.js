...
import "tiny-emitter";
import { F as k, P as A } from "./main-DDNyuLKS.js";
...
class nt extends k {
  ...
  async make(m) {
    ...
    if (s.isWebMercator())
      ...
      const i = new A("pole", { x: -96, y: 90 }), w = await this.$iApi.geo.proj.projectGeometry(s, i), e = this.$iApi.geo.map.mapPointToScreenPoint(w), c = {
        screenX: t.clientWidth / 2,
        screenY: t.clientHeight
      };
      o = Math.atan(
        (e.screenX - c.screenX) / (c.screenY - e.screenY)
      ) * 180 / Math.PI;
    ...
    let r = new n.Group();
    return n.loadSVGFromString(
      f,
      () => {
        r = new n.Group(h), r.positionX = l, r.getAngleFromPosition = a;
      },
      (d, i) => {
        h.push(i);
      }
    ), r.set(S(g, m || {})), Promise.resolve(r);
  }
}
export {
  nt as default
