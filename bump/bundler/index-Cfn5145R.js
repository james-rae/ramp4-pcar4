...
import "tiny-emitter";
import { F as k, P as A } from "./main-D3k1WGvN.js";
...
class Mt extends k {
  ...
  async make(n) {
    ...
    if (s.isWebMercator())
      ...
      const e = new A("pole", { x: -96, y: 90 }), w = await this.$iApi.geo.proj.projectGeometry(s, e), i = this.$iApi.geo.map.mapPointToScreenPoint(w), c = {
        screenX: t.clientWidth / 2,
        screenY: t.clientHeight
      };
      o = Math.atan(
        (i.screenX - c.screenX) / (c.screenY - i.screenY)
      ) * 180 / Math.PI;
      ...
      l = Math.min(Math.max(x, 0), 100), a = (p) => {
        ...
        return Math.atan((i.screenX - P) / (50 / 2 - i.screenY)) * (180 / Math.PI);
      };
    }
    const g = {
      top: 0,
